...
int wmain(int argc, wchar_t** argv) {
    if (argc != 4) {
        std::cerr << "usage: pup_frame105_lfr_probe <decoded_dat> <vfs> <output_dir>\n";
        return 2;
    }
    const std::filesystem::path output(argv[3]);
    const auto lfr = output / "oid55_frame105_source_packets.lfr";
    const auto csv = output / "oid55_frame105_source_ticks.csv";
