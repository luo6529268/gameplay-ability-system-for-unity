...
int wmain(int argc, wchar_t** argv) {
    if (argc != 4) {
        std::cerr << "usage: ddj_lfr_probe <decoded_dat> <vfs> <output_dir>\n";
        return 2;
    }
    const std::filesystem::path output(argv[3]);
    const auto lfr = output / "naruto_ddj_source_packets.lfr";
    const auto csv = output / "naruto_ddj_source_ticks.csv";
    ...
    if (!rows) return 6;
    rows << "tick,input,action,last_action_144,hp,mp,oid518_count\n";
