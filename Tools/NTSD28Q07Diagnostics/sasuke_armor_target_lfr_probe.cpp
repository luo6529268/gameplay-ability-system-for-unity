...
int wmain(int argc, wchar_t** argv) {
    if (argc != 4 && argc != 5) {
        std::cerr << "usage: sasuke_armor_target_lfr_probe <decoded_dat> <vfs> <output_dir> [target_x: 550|1200]\n";
        return 2;
    }
    ...
    const auto lfr = output / "sasuke_armor_target_source_packets.lfr";
    const auto csv = output / "sasuke_armor_target_source_ticks.csv";
    ...
    if (argc == 5) rows << ",target_hp,target_action,target_mp,target_runtime_armor_hp";
    rows << '\n';
    for (int tick = 1; tick <= 26; ++tick) {
        ...
        if (argc == 5) rows << ',' << target->current_hp << ',' << target->frame.action << ',' << target->current_mp << ',' << target->runtime_armor_hp;
        rows << '\n';
    }
    rows.close();
    if (!rows) return 9;
