...
struct WeaponReactionCase {
    int type=1, attacker_type=0, attacker_state=0, attacker_facing=0, target_facing=0;
...
static std::shared_ptr<const ntsd28::DatDocument> weapon_reaction_definition(bool attacker, const WeaponReactionCase& c) {
...
static void write_weapon_reaction_state(const ntsd28::BattleWorld28& world) {
    std::cout << "{\"raw\":[";
...
static void emit_weapon_reaction(const char* group, const WeaponReactionCase& c, int index) {
    ...
    for(int slot=0;slot<2;++slot) {
        ...
        request.definition=weapon_reaction_definition(slot==0,c);request.hp=slot==0?500:c.hp;request.mp=500;
    ...
    write_weapon_reaction_state(world);direct_crt_calls.clear();direct_synchronized_calls.clear();
    ...
    write_weapon_reaction_state(world);
    std::cout << ",\"audio\":[";
    ...
    std::cout << ",\"afterReleasedHoldFinalize\":";write_weapon_reaction_state(world);std::cout << "}\n";
}

int wmain(int,wchar_t**) {
    try {
        std::cout<<std::setprecision(std::numeric_limits<double>::max_digits10);int n=0;
        for(int type:{1,2,4,6})for(int face:{0,1})for(int state:{0,1002,2000})for(int y:{-10,0,3})for(int fall:{0,40,41})for(int dvy:{0,8}) {
            WeaponReactionCase c;c.type=type;c.target_facing=face;c.attacker_state=state;c.target_y=y;c.fall=fall;c.dvy=dvy;emit_weapon_reaction("base",c,n++);
        }
        for(int type:{1,2,4,6})for(int face:{0,1})for(int state:{0,1002,2000})for(int vx:{-8,0,8})for(int px:{-3,0,3})for(int dvx:{-5,0,5})for(int x:{90,110}) {
            WeaponReactionCase c;c.type=type;c.attacker_facing=face;c.attacker_state=state;c.target_vx=vx;c.pending_x=px;c.dvx=dvx;c.target_x=x;c.fall=60;emit_weapon_reaction("horizontal",c,n++);
        }
        for(int type:{1,2,4,6})for(int at:{0,3,4})for(int state:{1002,3000,3007})for(int cover:{0,2}) {
            WeaponReactionCase c;c.type=type;c.attacker_type=at;c.attacker_state=state;c.cover=cover;emit_weapon_reaction("post",c,n++);
        }
        for(int type:{1,2,4,6})for(int hp:{1,500})for(int injury:{-5,0,101})for(int bdefend:{0,100})for(int scale:{50,100})for(int weak:{0,1}) {
            WeaponReactionCase c;c.type=type;c.hp=hp;c.injury=injury;c.bdefend=bdefend;c.scale=scale;c.weak=weak;emit_weapon_reaction("resources",c,n++);
        }
        for(int type:{1,2,4,6})for(int dvx:{-5,0,5})for(int y:{-10,0,3}) {
            WeaponReactionCase c;c.type=type;c.attacker_facing=1;c.target_facing=1;c.dvx=dvx;c.target_y=y;emit_weapon_reaction("both_left",c,n++);
        }
        for(int type:{1,2,4,6})for(int y:{-10,0,20})for(int dvy:{0,8,-8})for(double py:{-2.5,10.0}) {
            WeaponReactionCase c;c.type=type;c.target_y=y;c.dvy=dvy;c.pending_y=py;emit_weapon_reaction("vertical",c,n++);
        }
        if(n!=2100)throw std::runtime_error("weapon reaction count changed");return 0;
