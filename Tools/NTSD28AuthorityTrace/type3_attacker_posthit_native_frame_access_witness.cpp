...
struct Type3Case {
    ...
    int selected;
    bool declared;
    int dvx = 0, state = 3000, cover = 0;
    bool armor = false;
    int target_type = 0;
};
static void emit_type3(int index, const Type3Case& c) {
    std::ostringstream ad, td;
    ad << "<bmp_begin>\nname: Type3PostHitAttacker\n<bmp_end>\n<frame> 20 initial\nstate: " << c.state
       << " wait: 100 next: 20 hit_Fj: " << c.selected << " cover: " << c.cover << "\n"
       << "itr:\nkind: 0 effect: 0 x: -100 y: -100 w: 300 h: 300 zwidth: 100 injury: 5 fall: 1 bdefend: 0 vrest: 1\nitr_end:\n<frame_end>\n";
    if(c.declared) ad << "<frame> " << c.selected << " selected\nstate: 0 wait: 41 next: " << c.selected << " dvx: " << c.dvx << "\n<frame_end>\n";
    td << "<bmp_begin>\nname: Type3PostHitTarget\n<bmp_end>\n";
    if(c.armor) td << "<armor>\ntype: 1 ratio: 15 decrease: 50 mp: 0 hp: 0 fall: -1 bdefend: -1 injury: -1 delay: -1 state: 4\n<armor_end>\n";
    td << "<frame> 10 initial\nstate: " << (c.armor?4:c.target_type==3?3005:0) << " wait: 100 next: 10\n"
    ...
    catalog.upsert_definition(77,3,"type3-attacker.dat",a);catalog.upsert_definition(78,c.target_type,"type3-target.dat",t);
    for(int slot : {0,70}) {
        ntsd28::SpawnRequest28 request;request.object_id=slot==0?77:78;request.object_type=slot==0?3:c.target_type;
    ...
    if(c.armor) world.entity(70)->runtime_armor_hp=3;
    ...
    std::cout << "{\"index\":" << index << ",\"params\":{\"group\":\"" << c.group << "\",\"selected\":" << c.selected
        << ",\"declared\":" << (c.declared?"true":"false") << ",\"selectedDvx\":" << c.dvx << ",\"attackerState\":" << c.state
        << ",\"cover\":" << c.cover << ",\"armorRequested\":" << (c.armor?"true":"false") << ",\"targetType\":" << c.target_type << ",\"seed\":42}"
    ...
    const auto result=c.armor ? world.resolve_ordinary_type1_armor_standard_hit(0,0,{}) : world.resolve_ordinary_unarmored_standard_hit(0,0);
    std::cout << ",\"status\":" << static_cast<int>(result.status) << ",\"message\":\"" << json_escape(result.message)
        ...
        << ",\"armorDecision\":" << static_cast<int>(result.armor_decision.kind)
        << ",\"activationAvailable\":" << (result.armor_activation.available?"true":"false")
        << ",\"activationMpCost\":" << result.armor_activation.mp_cost
        << ",\"armorBroken\":" << (result.armor_activation.armor_hp_broken?"true":"false")
...
int wmain() {
    try {
        ...
        emit_type3(0,{"implicit_low",69,false});
        emit_type3(1,{"declared_high",900,true,7});
        emit_type3(2,{"implicit_high",900,false});
        emit_type3(3,{"declared999",999,true,-9});
        emit_type3(4,{"missing999",999,false});
        emit_type3(5,{"out_of_range",1000,false});
        emit_type3(6,{"negative",-1,false});
        emit_type3(7,{"zero_fallback",0,false});
        emit_type3(8,{"cover2",900,true,7,3007,2});
        emit_type3(9,{"cover1",900,true,7,3007,1});
        emit_type3(10,{"literal550",77,true,550});
        emit_type3(11,{"type1_armor",900,true,7,3000,0,true});
        emit_type3(12,{"type3_target",900,true,7,3000,0,false,3});
        return 0;
