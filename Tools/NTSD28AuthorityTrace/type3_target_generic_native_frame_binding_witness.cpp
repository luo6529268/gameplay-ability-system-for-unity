...
static void emit_type3(int index, const Type3Case& c) {
    ...
    ad << "<bmp_begin>\nname: GenericType3Attacker\n<bmp_end>\n<frame> 20 initial\nstate: " << (c.pair?3005:0)
       << " wait: 100 next: 20\nitr:\nkind: 0 effect: 0 x: -100 y: -100 w: 300 h: 300 zwidth: 100 injury: 5 fall: 1 bdefend: 0 vrest: 1\nitr_end:\n<frame_end>\n";
    td << "<bmp_begin>\nname: GenericType3Target\n<bmp_end>\n<frame> 10 initial\nstate: 0 wait: 100 next: 10 hit_Fj: " << c.selected
       << " hit_Uj: " << c.selected << "\nbdy:\nkind: 0 x: -100 y: -100 w: 300 h: 300 zwidth: 100\nbdy_end:\n<frame_end>\n";
    if(c.declared) td << "<frame> " << c.selected << " selected\nstate: " << (c.pair?3005:0) << " wait: 41 next: " << c.selected << "\n<frame_end>\n";
    if(c.pair) {
        ad << "<frame> 13 latch\nstate: 0 wait: 100 next: 13 hit_Uj: 70\n<frame_end>\n<frame> 70 reset\nstate: 0 wait: 100 next: 70\n<frame_end>\n";
        td << "<frame> 11 latch\nstate: 0 wait: 100 next: 11 hit_Uj: 71\n<frame_end>\n<frame> 71 reset\nstate: 0 wait: 100 next: 71\n<frame_end>\n";
    }
    ...
    catalog.upsert_definition(77,c.attacker_type,"generic-attacker.dat",a);catalog.upsert_definition(78,3,"generic-target.dat",t);
    for(int slot : {0,70}) {
        ntsd28::SpawnRequest28 request;request.object_id=slot==0?77:78;request.object_type=slot==0?c.attacker_type:3;
    ...
    std::cout << "{\"index\":" << index << ",\"params\":{\"group\":\"" << c.group << "\",\"selected\":" << c.selected
        << ",\"declared\":" << (c.declared?"true":"false") << ",\"attackerType\":" << c.attacker_type
        << ",\"pair\":" << (c.pair?"true":"false") << ",\"targetType\":3,\"initialPendingCount\":1,\"seed\":42}"
...
int wmain() {
    try {
        ...
        emit_type3(0,{"fj_implicit69",69,false});
        emit_type3(1,{"fj_implicit900",900,false});
        emit_type3(2,{"fj_declared900",900,true});
        emit_type3(3,{"fj_declared999",999,true});
        emit_type3(4,{"fj_missing999",999,false});
        emit_type3(5,{"fj_1000",1000,false});
        emit_type3(6,{"fj_negative",-1,false});
        emit_type3(7,{"fj_zero_fallback",0,false});
        emit_type3(8,{"uj_zero_fallback",0,false,3});
        emit_type3(9,{"uj_matched_pair",900,true,3,true});
        emit_type3(10,{"fj_declared77",77,true});
        return 0;
