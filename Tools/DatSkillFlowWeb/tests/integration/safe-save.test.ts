...
function nativeClient(): PowerShellWindowsSafeFileClient {
...
async function fixture(label: string): Promise<{
    root: string;
    client: PowerShellWindowsSafeFileClient;
...
describe("recoverable safe-save protocol", windowsOnly, () => {
    it("binds overwrite challenges to content and consumes them once", async () => {
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof SafeSaveError && error.code === "challenge-content-mismatch",
        );
        await assert.rejects(
            service.overwrite(documentId, challenge.challengeId, content),
            (error: unknown) => error instanceof SafeSaveError && error.code === "challenge-invalid",
        );
    });
    ...
    it("rejects an external target mutation under the native overwrite transaction", async () => {
        ...
        await assert.rejects(
            service.overwrite(documentId, challenge.challengeId, content),
            (error: unknown) => error instanceof SafeSaveError && error.code === "external-change",
        );
    ...
    it("expires and consumes a challenge", async () => {
        ...
        await assert.rejects(
            service.overwrite(documentId, challenge.challengeId, content),
            (error: unknown) => error instanceof SafeSaveError && error.code === "challenge-expired",
        );
        await assert.rejects(
            service.overwrite(documentId, challenge.challengeId, content),
            (error: unknown) => error instanceof SafeSaveError && error.code === "challenge-invalid",
