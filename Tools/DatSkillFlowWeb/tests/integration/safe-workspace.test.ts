...
describe("opaque workspace and document registry", () => {
    it("allows one startup-only absolute root and rejects authorization after sealing", async () => {
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof WorkspaceSecurityError && error.code === "startup-root-already-authorized",
        );
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof WorkspaceSecurityError && error.code === "root-grant-disabled",
        );
    });
    ...
    it("rejects NUL, traversal, absolute, drive-relative, and oversized reads", async () => {
        ...
        for (const unsafePath of [
            ...
            await assert.rejects(
                ...
                (error: unknown) => error instanceof WorkspaceSecurityError,
                unsafePath,
            );
        }
        await assert.rejects(
            registry.openDocument(rootId, "large.dat"),
            (error: unknown) => error instanceof WorkspaceSecurityError && error.code === "read-too-large",
        );
    });
    ...
    it("rejects a symlink or junction escape during logical resolution", async (context) => {
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof WorkspaceSecurityError && error.code === "root-escape",
        );
    });
});
