...
function fileEntry(path: string, buildId: string, bytes: Buffer): Record<string, unknown> {
    return {
        path,
        buildId,
        size: bytes.length,
...
async function createManifestFixture(): Promise<ManifestFixture> {
...
async function writeManifest(fixture: ManifestFixture): Promise<void> {
...
describe("trusted build manifest execution", () => {
    ...
    it("retries only injected transient Windows pointer-sharing failures before pinning", async () => {
        ...
        const verified = await loadVerifiedBuildManifest({
            ...fixture,
            readFileImpl: async (filePath: string, options: string) => {
