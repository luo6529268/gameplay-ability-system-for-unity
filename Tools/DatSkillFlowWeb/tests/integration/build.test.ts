...
interface BuildManifest {
    ...
    serverEntry: string;
    testFiles: string[];
    runtimeAssets: Array<{ path: string; buildId: string; size: number; sha256: string }>;
    outputs: Array<{ path: string; buildId: string; size: number; sha256: string }>;
    clientFiles: Array<{ path: string; buildId: string; size: number; sha256: string }>;
}

async function readCurrentManifest(): Promise<BuildManifest> {
    let lastError: unknown;
    for (let attempt = 0; attempt < 32; attempt += 1) {
        try {
            return JSON.parse(await readFile(resolve(process.cwd(), "dist/build-manifest.json"), "utf8")) as BuildManifest;
        } catch (error) {
...
describe("native zero-dependency build", () => {
    ...
    it("keeps the current manifest parseable while a concurrent reader observes a unique build publication", async () => {
        const runBuild = (): Promise<void> => new Promise((resolveBuild, rejectBuild) => {
