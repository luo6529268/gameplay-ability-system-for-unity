...
async function serverFixture(
    ...
    const entry = (path: string, body: Buffer) => ({
        path,
        buildId,
        size: body.length,
...
async function post(origin: string, token: string | undefined, path: string, body: unknown, withOrigin = true): Promise<Response> {
...
async function getStatusWithHeaders(origin: string, path: string, headers: Readonly<Record<string, string>>): Promise<number> {
    const url = new URL(origin);
    return await new Promise<number>((resolveResponse, rejectResponse) => {
...
async function requestStatus(origin: string, path: string, method: string, headers: Readonly<Record<string, string>> = {}): Promise<number> {
    const url = new URL(origin);
    return await new Promise<number>((resolveResponse, rejectResponse) => {
...
describe("safe workspace HTTP API", () => {
    it("returns a no-store bootstrap token without inventing an unconfigured root capability", async () => {
        ...
        const body = JSON.parse(text) as { data: { rootId?: string; capabilities: string[]; stateToken: string } };
    ...
    it("bootstraps only the opaque startup root, seals it at listen, and keeps production grant disabled", async () => {
        ...
        await assert.rejects(
            ...
            (error: unknown) => error instanceof WorkspaceSecurityError && error.code === "startup-authorization-sealed",
        );
    ...
    it("requires exact Origin/Host and token for every state-changing route", async () => {
        ...
        const grantBody = await granted.json() as { data: { rootId: string } };
        const opened = await post(origin, token, "/api/documents/open", {
            rootId: grantBody.data.rootId,
            path: "source.dat",
        });
        assert.equal(opened.status, 200);
        const openBody = await opened.json() as { data: { documentId: string } };
        ...
        const readBody = await read.json() as { data: { contentBase64: string } };
        ...
        const challengeBody = await challengeResponse.json() as { data: { challengeId: string; contentSha256: string } };
    ...
    it("rejects oversized JSON bodies before buffering them", async () => {
        ...
        const url = new URL(origin);
        const status = await new Promise<number>((resolveResponse, rejectResponse) => {
            const outgoing = request({
                hostname: url.hostname,
                port: url.port,
