...
const openServers: Array<ReturnType<typeof createApplicationServer>> = [];
...
async function createStaticFixture(): Promise<StaticFixture> {
...
describe("loopback application server", () => {
    it("retries an ephemeral browser-blocked port and returns the next safe loopback port", async () => {
        ...
        const origin = await listenLoopback({} as ReturnType<typeof createApplicationServer>, 0, {
    ...
    it("fails explicitly after exhausting browser-blocked ephemeral port retries", async () => {
        ...
        await assert.rejects(
            listenLoopback({} as ReturnType<typeof createApplicationServer>, 0, {
    ...
    it("binds explicitly to 127.0.0.1 and serves same-origin health", async () => {
        ...
        const health = await response.json() as { ok: boolean; data: { status: string; host: string } };
