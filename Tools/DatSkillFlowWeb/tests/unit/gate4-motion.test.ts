...
interface MotionCpoint {
    readonly kind: number;
}

type MotionFrame = SimFrameDefinition & {
    readonly dvx?: number;
    readonly dvy?: number;
    readonly dvz?: number;
    readonly cpoints?: readonly MotionCpoint[];
};

type MotionDat = SimDatSeed & {
    readonly jumpHeight?: number;
    readonly jumpDistance?: number;
    readonly jumpDistanceZ?: number;
};

type MotionEntitySeed = SimEntitySeed & {
...
type MotionEntity = SimEntity & Required<Pick<MotionEntitySeed,
    "keyLeft" | "keyRight" | "blockBackZ" | "blockForwardZ" | "blockLeft" | "blockRight"
>>;
...
function frame(id: number, overrides: Partial<MotionFrame> = {}): SimFrameDefinition {
    return { id, state: 1, wait: 100, next: 0, ...overrides } as SimFrameDefinition;
}

function dat(oid: number, rawObjectType: number, frames: readonly SimFrameDefinition[], overrides: Partial<MotionDat> = {}): SimDatSeed {
    return { oid, rawObjectType, frames, ...overrides } as SimDatSeed;
}

function entity(stableId: string, slot: number, overrides: Partial<MotionEntitySeed> = {}): SimEntitySeed {
    return {
        stableId,
        slot,
        oid: slot + 1,
...
function at(state: SimulationState, slot: number): MotionEntity {
    const value = state.slots[slot];
    assert.ok(value, `expected active slot ${slot}`);
    return value as MotionEntity;
}
...
describe("Gate4A canonical motion contract", () => {
    it("preserves ordered cpoint kinds and exact frame dv fields", () => {
        ...
        const motion = value as MotionFrame;
    ...
    it("stores finite double jump stats plus directional keys and four block flags in canonical snapshots", () => {
        ...
        const resolved = state.catalog[20] as SimDatDefinition & MotionDat;
...
describe("Gate4A late jump initialization and opoint motion visibility", () => {
    it("initializes only explicit +/-212 from finite resolved DAT stats and leaves displacement to the next tick", () => {
        ...
        const invoke = frameTick as unknown as (subject: SimEntity, resolvedDat: SimDatDefinition) => { readonly entity: SimEntity };
        const make = (next: number, extra: Partial<MotionEntitySeed> = {}) => createSimulation({
    ...
    it("spawns an opoint with explicit velocity/reset flags but gives it first motion on the next tick", () => {
        ...
        const birth = [child.x, child.y, child.z] as const;
