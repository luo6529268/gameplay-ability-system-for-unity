...
function entity(overrides: Partial<SimEntitySeed> = {}): SimEntitySeed {
    return {
        stableId: "entity-0",
        slot: 0,
        rawObjectType: 0,
...
function onlyEntity(state: ReturnType<typeof createSimulation>) {
...
describe("Gate 2 authoritative frame_tick subset", () => {
    ...
    it("resolves next=999 to 212 iff airborne raw objType 0, otherwise to 0", () => {
        const resolve = (rawObjectType: number, yInt: number) => onlyEntity(stepSimulation(createSimulation({
            entities: [entity({
                rawObjectType,
                yInt,
