...
function wpoint(overrides: Partial<SimWpointDefinition> = {}): SimWpointDefinition {
    return {
        kind: 0,
        x: 0,
        y: 0,
...
function itr(overrides: Partial<SimItrDefinition> = {}): SimItrDefinition {
    return {
...
function frame(id: number, overrides: Partial<SimFrameDefinition> = {}): SimFrameDefinition {
    return { id, state: 0, wait: 100, next: 0, ...overrides };
}

function entity(stableId: string, slot: number, overrides: Partial<SimEntitySeed> = {}): SimEntitySeed {
...
describe("Gate3B2 wpoint data and held attack payload", () => {
    it("normalizes ordered wpoints with exact nine integer fields and zero defaults", () => {
        const [normalized] = normalizeFrames([frame(0, {
            wpoints: [wpoint({ x: 4 }), { kind: 3 } as SimWpointDefinition],
        })]);
...
describe("Gate3B2 pickup, link validation, and force drop", () => {
    ...
    it("strictly validates the narrow injected pickup envelope", () => {
        ...
        assert.throws(() => parsePickupInputs({ pickups: [{ kind: 3, pickerSlot: 0, weaponSlot: 1 }] as never }), /kind.*2 or 7/);
        assert.throws(() => parsePickupInputs({ pickups: [{ kind: 2, pickerSlot: 0, weaponSlot: 400 }] as never }), /0\.\.399/);
        assert.throws(() => parsePickupInputs({ pickups: [{ kind: 2, pickerSlot: 0, weaponSlot: 1, extra: 1 }] as never }), /exactly/);
        const oversized = new Array(401);
        Object.defineProperty(oversized, 0, { get: () => { throw new Error("pickup row accessed"); } });
        assert.throws(() => parsePickupInputs({ pickups: oversized as never }), /at most 400/);
        assert.throws(() => stepSimulation(createSimulation({ entities: [] }), { pickups: oversized as never }), /at most 400/);
    });
    ...
    it("accepts the 400-row host seam maximum sequentially without inventing a kind-2 already-linked guard", () => {
        ...
        const pickups = Array.from({ length: 400 }, () => ({ kind: 2 as const, pickerSlot: 0, weaponSlot: 5 }));
    ...
    it("keeps pickup input and held events canonical and replay-identical", () => {
        ...
        const input = { pickups: [{ kind: 2 as const, pickerSlot: 0, weaponSlot: 2 }] };
...
describe("Gate3B2 bounded canonical JSON", () => {
    it("rejects 15k nesting and cycles with controlled errors in normalization and canonical serialization", () => {
        ...
        for (const operation of [
            () => normalizeJsonObject({ deep } as never),
            () => canonicalJson({ deep }),
        ]) {
        ...
        cyclic.self = cyclic;
        assert.throws(() => normalizeJsonObject(cyclic as never), /cycle/i);
    ...
    it("rejects a flat node-budget attack before accessing its rows and keeps normal traces canonical", () => {
        ...
        assert.throws(() => normalizeJsonObject({ values: tooWide } as never), /node budget/i);
    ...
    it("rejects sparse arrays instead of letting holes bypass the global node budget", () => {
        ...
        for (const operation of [
            () => normalizeJsonObject({ values: sparseAggregate } as never),
        ...
        for (const value of [sparse, interiorHole]) {
            assert.throws(() => normalizeJsonObject({ value } as never), /sparse/i);
