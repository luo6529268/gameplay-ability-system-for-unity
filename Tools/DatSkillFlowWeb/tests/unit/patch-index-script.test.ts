...
function runScanner(args: readonly string[]) {
    return spawnSync(
        powershellExecutable,
        [
            "-NoProfile",
...
async function parseIndexAsync(outputPath: string) {
...
describe("build-patch-index.ps1", () => {
    ...
    it("scans special manifest names, keeps explicit types, reports tupe/tpye, and merges supplemental records", { skip: !powershellAvailable }, async () => {
        ...
        try {
            ...
            const packageIndex = index.packages.find((item: { relativeDirectory: string }) => item.relativeDirectory === "Pack Ω");
            ...
            assert.equal(packageIndex.datFiles.some((path: string) => path.includes("supplemental")), false);
            assert.equal(packageIndex.bmpFiles.some((path: string) => path.includes("supplemental")), false);

            assert.ok(packageIndex.records.length >= 8);
            assert.ok(packageIndex.records.every((record: Record<string, unknown>) => {
                return Object.keys(record).sort().join(",") === "file,logicalPath,manifestPath,manifestSource,oid,type";
            }));
            assert.ok(packageIndex.records.every((record: { manifestSource: string }) => {
                return record.manifestSource === "source" || record.manifestSource === "supplemental";
            }));
            const sourceTypes = packageIndex.records
                .filter((record: { manifestSource: string }) => record.manifestSource === "source")
                .map((record: { type: number }) => record.type)
                .sort((left: number, right: number) => left - right);
            assert.deepEqual(sourceTypes, [0, 1, 2, 3, 4, 5, 5]);
            const recoveredSource = packageIndex.records.find((record: { oid: number; type: number; manifestSource: string }) => {
                return record.oid === 7 && record.type === 0 && record.manifestSource === "source";
            });
            const recoveredSupplemental = packageIndex.records.find((record: { oid: number; type: number; manifestSource: string }) => {
            ...
            assert.ok(packageIndex.records.every((record: { logicalPath: string | null }) => {
            ...
            const diagnosticCodes = packageIndex.diagnostics.map((diagnostic: { code: string }) => diagnostic.code);
            ...
            assert.ok(packageIndex.diagnostics.every((diagnostic: Record<string, unknown>) => {
    ...
    it("enforces text-size and total-file limits", { skip: !powershellAvailable }, async () => {
        ...
        try {
            ...
            assert.ok(limitedIndex.diagnostics.some((diagnostic: { code: string }) => diagnostic.code === "manifest-too-large"));
