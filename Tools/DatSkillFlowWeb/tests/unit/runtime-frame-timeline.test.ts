...
import {
    ...
    type RuntimeFrameEntity,
    type RuntimeFrameTick,
} from "../../src/client/runtime-frame-timeline.js";

function entity(slot: number, frame: number, active?: boolean): RuntimeFrameEntity {
    return active === undefined ? { slot, frame } : { slot, frame, active };
}

function tick(tickIndex: number, ...entities: RuntimeFrameEntity[]): RuntimeFrameTick {
