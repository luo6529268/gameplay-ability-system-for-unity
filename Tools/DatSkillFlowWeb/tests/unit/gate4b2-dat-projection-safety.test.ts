...
const hostileKeys = ["__proto__", "constructor", "toString", "unknown_key"] as const;

function assertExactDto(value: object, expectedKeys: readonly string[]): void {
...
function prototypeSensitiveDat(): Buffer {
    return Buffer.from([
        "name: Safe Hero\n",
        "walking_speed: 4\n",
        "walking_speed: 5\n",
...
describe("Gate4B2 DatProjection prototype-sensitive keys", () => {
    ...
    it("keeps legal duplicate-last/alias semantics while preserving hostile CST bytes losslessly", () => {
        ...
        assert.equal(document.cst.frames[0]?.blocks.some((block) => block.fields.some((field) => hostileKeys.includes(field.key as typeof hostileKeys[number]))), true);
