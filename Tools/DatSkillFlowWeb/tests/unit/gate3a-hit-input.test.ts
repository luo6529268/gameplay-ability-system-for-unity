...
const world: SimWorldInputState = Object.freeze({ ppMode: 1, oid6DjaGuard: 0 });
...
function seed(overrides: Partial<SimEntitySeed> = {}): SimEntitySeed {
    return {
...
function canonicalEntity(overrides: Partial<SimEntitySeed> = {}): SimEntity {
...
describe("Gate3A HIT-02 eight ordinary combo wrappers", () => {
    ...
    it("resets every ordinary wrapper when another new key interrupts its pending sequence", () => {
        const combos = ["DRA", "DLA", "DUA", "DDA", "DRJ", "DLJ", "DUJ", "DDJ"] as const;
...
describe("Gate3A HIT-05 eligibility, canonical state, and auditable pass integration", () => {
    it("runs only active resolved DAT type-0 entities, with no hitStop/frameDelay entry gate", () => {
        for (const overrides of [{ active: false }, { rawObjectType: 3 }] as const) {
            const entity = canonicalEntity({
                ...overrides,
                cooldowns: { attack: 9 },
