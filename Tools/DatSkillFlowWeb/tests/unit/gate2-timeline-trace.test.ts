...
import type { SimEntitySeed, SimFrameDefinition } from "../../src/sim/index.js";

function frame(id: number, next: number): SimFrameDefinition {
    return { id, state: 1, wait: 0, next };
}

function seed(): SimEntitySeed {
    return {
...
describe("Gate 2 pure timeline controller", () => {
    ...
    it("captures a normalized immutable input script for later seek replay", () => {
        const callerOwnedInput: Record<string, string> = { key: "a" };
        const initial = createTimeline(createSimulation({ entities: [seed()] }));
        const tick1 = applyTimelineCommand(initial, { type: "step", input: callerOwnedInput });
        const originalTrace = serializeTickTrace(tick1.traces[0]!);

        callerOwnedInput.key = "mutated-after-step";
        const seek0 = applyTimelineCommand(tick1, { type: "seek", tick: 0 });
        const replayedTick1 = applyTimelineCommand(seek0, { type: "seek", tick: 1 });

        assert.equal(serializeTickTrace(replayedTick1.traces[0]!), originalTrace);
        assert.deepEqual(replayedTick1.script[0], { key: "a" });
        assert.equal(Object.isFrozen(replayedTick1.script[0]), true);
    });
});
...
describe("Gate 2 canonical per-tick trace", () => {
    ...
    it("replays byte-identically from the initial state and input script", () => {
        const initial = createSimulation({ entities: [seed()] });
        const script = [{ key: "a", power: 1 }, { power: 2, key: "b" }, {}] as const;
        const runtime = { collision: (current: { frame: number }) => ({ frame: current.frame }) };
