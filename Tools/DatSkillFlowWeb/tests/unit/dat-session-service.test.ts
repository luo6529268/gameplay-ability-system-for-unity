...
function fingerprint(bytes: Uint8Array, generation: number) {
    return {
...
class FakeNativeClient implements NativeSafeFileClient {
    bytes: Buffer;
    ...
    async saveAs(): Promise<never> { throw new Error("save is deferred"); }
    async overwrite(): Promise<never> { throw new Error("save is deferred"); }
}

function datSource(name = "Second Hero", pic = 2): Buffer {
    return Buffer.from([
...
async function fixture(bytes: Uint8Array, options: DatSessionServiceOptions = {}) {
...
async function rejectsCode(promise: Promise<unknown>, code: DatSessionError["code"]): Promise<void> {
    await assert.rejects(promise, (error: unknown) => error instanceof DatSessionError && error.code === code);
}

function assertNoPrivateProjectionData(view: DatSessionView, documentId: string): void {
...
describe("server-owned DAT session core", () => {
    ...
    it("fails closed when the trusted format selection is missing or contains no editable authority fields", async () => {
        ...
        await rejectsCode(plainFixture.service.openDocument(plainFixture.documentId, undefined as never), "invalid-request");
    ...
    it("fails closed when dispose races a deferred reload plus queued edit and close", async () => {
        ...
        Object.defineProperty(client, "read", {
            configurable: true,
            value: async (request: NativeReadRequest) => {
                markStarted();
                await readGate;
    ...
    it("reserves concurrent opens and keeps ID/tombstone lifecycle bounded", async () => {
        ...
        const sessionRejected = simultaneous.find((result): result is PromiseRejectedResult => result.status === "rejected");
        ...
        const byteRejected = byteRace.find((result): result is PromiseRejectedResult => result.status === "rejected");
    ...
    it("keeps persistence save explicitly deferred", async () => {
        ...
        assert.equal(typeof (service as unknown as { save?: unknown }).save, "undefined");
    });
});
