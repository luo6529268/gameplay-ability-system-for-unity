...
public static class DatSkillFlowSafeFileTransaction
{
    ...
    private static SafeFileHandle OpenExistingFile(string path, string root, bool shareDelete)
    {
        SafeFileHandle handle = OpenRaw(
            path,
            GENERIC_READ | FILE_READ_ATTRIBUTES | SYNCHRONIZE,
            FILE_SHARE_READ | (shareDelete ? FILE_SHARE_DELETE : 0),
            OPEN_EXISTING,
    ...
    {
        return OpenExistingFile(path, root, false);
    }

    private static SafeFileHandle CreateNewFile(string path, string root, bool shareDelete)
    {
        ...
        try
        {
            handle = OpenRaw(
                ...
                shareDelete ? FILE_SHARE_READ | FILE_SHARE_DELETE : 0,
                CREATE_NEW,
    ...
    {
        return CreateNewFile(path, root, false);
    }
        ...
        int maximumBytes)
    {
        using (Traversal traversal = Traverse(request))
        {
            ...
            try
            {
                target = OpenExistingFile(traversal.TargetPath, traversal.RootPath, true);
                ...
                replacement = CreateNewFile(replacementPath, traversal.RootPath, true);
                ...
                if (replaced)
                {
                    Snapshot oldAfterReplace = ReadSnapshot(target, FinalPath(target), maximumBytes);
                    Snapshot newAfterReplace = ReadSnapshot(replacement, FinalPath(replacement), maximumBytes);
                    if (!SamePath(oldAfterReplace.Path, backupPath)
                        || !SamePath(newAfterReplace.Path, traversal.TargetPath)
                        || !FingerprintMatches(oldAfterReplace.Fingerprint, oldSnapshot.Fingerprint)
                        || !FingerprintMatches(newAfterReplace.Fingerprint, newSnapshot.Fingerprint))
                    {
                        TransactionFailure identityFailure = new TransactionFailure(
                            "postcondition-failed",
                            "ReplaceFileW changed an unexpected file identity.",
                            0);
                        identityFailure.Recovery = Recovery(
                            traversal.RootPath,
                            traversal.TargetPath,
                            replacementPath,
                            backupPath,
