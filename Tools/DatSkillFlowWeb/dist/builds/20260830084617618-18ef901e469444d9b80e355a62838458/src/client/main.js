...
let playing = false;
let zoom = 1;
let presentationRate                    = 120;
let playbackMs = 0;
let presentationSample                                 ;
let presentationFrameRequest                    ;
...
let scenePositions                                     ;
let lastPreviewScenario                                                                                            ;
let currentActorHitAreas                                 = [];
...
function normalize(payload      )               {
    ...
    const initialPosition = (value         , fallback                 )                  => {
...
function positionedPreviewTick(tick                         = project?.nativeTicks[tickIndex])                         {
...
function syncPositionUi()       {
    const toggle = select                   ("position-mode");
    const reset = select                   ("reset-positions");
    if (toggle) {
...
function update()       { syncReadOnlyUi(); syncPositionUi(); requestPreviewRender(); }
...
function compactSkillRouteSummary(skill            )         {
...
function setEntryTableHeadings(labels                                           )       {
...
function renderActionContextDetail()       {
    const detail = select             ("action-context-detail");
...
function advancePresentation(nowMs        )       {
...
async function selectInternalStage(parent            , stage            )                {
    if (playing) setPlaying(false);
...
select                   ("presentation-rate")?.addEventListener("change", (event) => {
    const rate = Number((event.currentTarget                     ).value);
    if (!RENDER_CADENCE_RATES.includes(rate                     )) return;
...
function finishActorPositionInteraction(pointerId = actorPositionInteraction?.pointerId)       {
    actorPositionInteraction = undefined;
    if (pointerId !== undefined) {
        try { canvas?.releasePointerCapture(pointerId); } catch {}
    }
...
function beginActorPositionInteraction(event              )          {
...
function moveActorPositionInteraction(event              )          {
    const interaction = actorPositionInteraction;
...
function commitActorPositionInteraction(event              )          {
    const interaction = actorPositionInteraction;
    if (interaction === undefined || interaction.pointerId !== event.pointerId) return false;
