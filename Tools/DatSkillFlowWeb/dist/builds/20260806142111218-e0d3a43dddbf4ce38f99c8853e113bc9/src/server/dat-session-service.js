...
export class DatSessionService {
    ...
    async edit(input         )                          {
        ...
        return await this.#enqueue(request.sessionId, () => {
            ...
            if (capability.kind === "integer-pair") {
                return this.#editIntegerPair(session, capability, request.value);
            }
            const scalarValue = request.value;
            if (Array.isArray(scalarValue) || capability.kind !== typeof scalarValue) {
            ...
            if (capability.kind === "number" && capability.numericKind === "integer"
                && !isSignedInt32(scalarValue)) {
                throw new DatSessionError("invalid-request", "The numeric field requires a signed 32-bit integer.");
            }
            if (capability.currentValue === scalarValue) return this.#createView(session);

            const savepoint = session.document.createPatchSavepoint();
            const previousValue = capability.currentValue;
            const applied = session.document.apply(createSetScalarCommand("dat-session-edit", capability.field, scalarValue));
            if (!applied.applied) {
            ...
            capability.currentValue = scalarValue;
            session.revision += 1;
            try {
    ...
    #editIntegerPair(
        ...
        const currentValue = capability.currentValue;
        if (!Array.isArray(currentValue)) {
            throw new DatSessionError("invalid-request", "The pair field capability is invalid.");
        }
        ...
        const previousRevision = session.revision;
        const savepoint = session.document.createPatchSavepoint();
        const applied = session.document.apply(createSetIntegerPairCommand(
            "dat-session-pair-edit",
            capability.field,
            nextValue,
        ));
        if (!applied.applied) {
            if (applied.diagnostics.length === 0) return this.#createView(session);
            throw new DatSessionError("invalid-request", "The ITR pair edit is not supported.");
        }
        capability.currentValue = nextValue;
        session.revision += 1;
        try {
            return this.#createView(session);
        } catch (error) {
