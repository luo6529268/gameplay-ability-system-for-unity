...
describe("automatic DAT skill entries", () => {
    it("starts a selected jump frame from its authored entry chain instead of fabricating F212 velocity", () => {
        const frames = [
            frame(210, 0, "jump", { state: 4, wait: 1, next: 211 }),
            frame(211, 1, "jump", { state: 4, wait: 1, next: 212 }),
            frame(212, 2, "jump", { state: 4, wait: 1, next: 0 }),
            frame(213, 3, "dash", { state: 4, wait: 1, next: 214 }),
        ];
        ...
        assert.equal(authoredTraceStartFrame(frames, 211), 210);
        assert.equal(authoredTraceStartFrame(frames, 212), 210);
        assert.equal(authoredTraceStartFrame(frames, 213), 213);
    });

    it("does not cross labels, gaps, or non-authored recovery transitions when resolving a trace entry", () => {
