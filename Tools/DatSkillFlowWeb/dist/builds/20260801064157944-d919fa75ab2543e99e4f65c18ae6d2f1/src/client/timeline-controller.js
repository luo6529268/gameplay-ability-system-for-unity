...
function latestTraceSummary(controller                    )         {
    ...
    return `Tick ${trace.tickIndex}: ${trace.frameTransitions.length} transition${trace.frameTransitions.length === 1 ? "" : "s"}, ${trace.collisions.length} collision check${trace.collisions.length === 1 ? "" : "s"}, ${trace.lifecycle.length} lifecycle event${trace.lifecycle.length === 1 ? "" : "s"}; ${trace.snapshotDigest}.`;
}
