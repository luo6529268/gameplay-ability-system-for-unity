...
function drawEntity(
    ...
    const loaded = resource !== undefined;
    const frames = resource?.frames ?? input.project.frames;
    const ranges = resource?.ranges ?? input.project.ranges;
    const frame = loaded && entity === primary ? input.runtimeFrame : loaded ? lastFrameForId(frames, entity.frame) : undefined;
