...
export function parseNativeInt32Token(rawValue                     )                     {
    const rawText = typeof rawValue === "string"
        ? rawValue
        : Buffer.from(rawValue).toString("latin1");
    if (!/^[+-]?\d+$/.test(rawText)) return undefined;
    const parsed = BigInt(rawText.startsWith("+") ? rawText.slice(1) : rawText);
    if (parsed < INT32_MIN_BIGINT) return INT32_MIN;
    if (parsed > INT32_MAX_BIGINT) return INT32_MAX;
