...
export class WindowsReplaceFilePublisher                             {
    ...
    constructor(options                                     = {}) {
        ...
        if (options.scriptPath === undefined) {
            ...
            this.#runtimeAssetVerification = {
                staticRoot,
                manifestPath: resolve(staticRoot, "build-manifest.json"),
    ...
    async replace(request                )                         {
        const scriptPath = this.#runtimeAssetVerification === undefined
            ? this.#scriptPath
            : await verifyManifestOutput(this.#runtimeAssetVerification);
        const args = [
            "-NoProfile",
            "-NonInteractive",
            "-ExecutionPolicy",
