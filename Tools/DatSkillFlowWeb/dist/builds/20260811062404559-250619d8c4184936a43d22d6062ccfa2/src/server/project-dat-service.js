...
function previewDatPlaintext(bytes            )         {
    const input = Buffer.from(bytes);
    let offset = 0;
    if (input.length >= 3 && input[0] === 0xef && input[1] === 0xbb && input[2] === 0xbf) offset = 3;
    const plainBytes = input.subarray(offset);
    ...
    if (plaintext.cst.topFields.length > 0
        || plaintext.cst.spriteRanges.length > 0
        || plaintext.cst.frames.length > 0) {
        return Buffer.from(plainBytes);
    }
    return Buffer.from(LosslessDatDocument.fromEncrypted(input).emitPlaintext());
}
...
export class CppNativeDatPreviewRunner                                   {
    ...
    async preview(plaintext            , options                       = {})                   {
        ...
        const normalizedOptions = { rootOid, startFrame, initialFrame, ticks, inputPlan, catalogEntries };
        ...
        const pending = this.#run(plaintext, rootOid, startFrame, initialFrame, ticks, inputPlan, catalogEntries);
        ...
        try {
            ...
            if (catalogEntries.length > 0) {
                ...
                for (const entry of catalogEntries) {
                    ...
                    lines.push(`id: ${entry.oid} type: ${entry.type} file: ${fileName}\n`);
                }
