...
describe("native DAT preview output bounds", () => {
    ...
    it("loads the complete patch catalog and spawns the F346 thrown OID 466 entity", {
        ...
        const catalogEntries = await Promise.all(nativePatchObjects.map(async (object) => ({
            oid: object.oid,
            type: object.type,
            plaintext: LosslessDatDocument.fromEncrypted(
...
describe("Naruto project DAT HTTP vertical slice", () => {
    ...
    it("publishes the prepared session before verified entry warmup completes", async () => {
        ...
        primary.sealStartupAuthorization();
        let releaseWarmup                           ;
        const warmupGate = new Promise         ((resolveWarmup) => { releaseWarmup = resolveWarmup; });
        let warmupStarted             ;
        const warmupStartedPromise = new Promise      ((resolveStarted) => { warmupStarted = resolveStarted; });
        const project = await ProjectDatService.initialize({
            primaryRegistry: primary,
            dataTxtLogicalPath: dataTxtPath,
        ...
        const prepared = await Promise.race([
            ...
            new Promise       ((_resolve, reject) => setTimeout(() => reject(new Error("preparation blocked on warmup")), 1_000)),
        ]);
