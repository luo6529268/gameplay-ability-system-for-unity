// dat-skill-flow-build:20260801025323944-1c4795345fde44369a71e1747b65e39d
...
describe("loopback application server", () => {
    ...
    it("establishes per-process state-changing request token isolation", async () => {
        ...
        assert.notEqual(firstSecurity.token, secondSecurity.token);
        assert.equal(authorizeStateChangingRequest(first, firstSecurity.token), true);
        assert.equal(authorizeStateChangingRequest(first, secondSecurity.token), false);
