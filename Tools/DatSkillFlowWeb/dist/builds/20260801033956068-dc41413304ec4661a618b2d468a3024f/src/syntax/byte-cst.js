...
function scalarKindAndNumber(raw        )                                                       {
    ...
    if (/^[+-]?\d+$/.test(rawText)) {
        const number = Number.parseInt(rawText, 10);
        if (Number.isSafeInteger(number)) return { scalarKind: "number", numericValue: number };
    }
...
function parseFieldsOnLine(
    ...
    for (let index = 0; index < matches.length; index += 1) {
        ...
        const parsed = scalarKindAndNumber(rawValue);
        fields.push({
            key: match.key,
...
function parseFrameHeader(source            , span          )                     {
...
function parseSpriteRange(source        , line          )                                {
    ...
    const fileField              = {
        ...
        rawValue: Buffer.from(source.subarray(fileStart, fileEnd)),
        scalarKind: "string",
    };
    const fields = parseFieldsOnLine(source, { start: fileEnd, end: content.end }, new Set());
    return {
        frameLo,
        frameHi,
...
export function parseDatCst(input            )                 {
    ...
    for (const line of splitLines(source)) {
        ...
        if (currentFrame) {
            ...
            if (blockStart && blockTypes.has(blockStart[1]                )) {
                ...
                currentBlock.fields.push(...parseFieldsOnLine(
                    ...
                    content,
                    new Set([type]),
                    { frameOccurrence: currentFrame.occurrence, blockType: type, blockIndex: index },
                ));
                continue;
            }
        ...
        topFields.push(...parseFieldsOnLine(source, content, new Set()));
    }
