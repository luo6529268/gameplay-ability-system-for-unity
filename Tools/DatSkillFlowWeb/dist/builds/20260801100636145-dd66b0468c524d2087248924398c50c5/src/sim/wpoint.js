...
import { authoredFrame, MAX_WORLD_SLOTS } from "./catalog.js";
...
export function runHeldObjectPass(state                 , pass        )                 {
    ...
    for (let heldSlot = 0; heldSlot < MAX_WORLD_SLOTS; heldSlot++) {
        ...
        const holderFrame = authoredFrame(holder.frames, holder.frame);
        if (holderFrame !== undefined) {
            ...
            const caughtFrame = authoredFrame(held.frames, held.frame);
            ...
            if (caughtFrame !== undefined) {
                ...
                const xBase = holder.facing === 0
                    ? safeIntegerResult(holder.xInt - holderCx + wp.x, "held sync x base")
                    : safeIntegerResult(holderCx - wp.x + holder.xInt, "held sync x base");
                const xAdjustment = held.facing === 0 ? caughtCx - caughtWp.x : caughtWp.x - caughtCx;
                let yInt = safeIntegerResult(holder.yInt - holderCy + wp.y + caughtCy - caughtWp.y, "held sync yInt");
                let zInt = holder.zInt;
                if (wp.cover === 0) {
                    zInt = safeIntegerResult(zInt + 1, "held sync zInt");
                    yInt = safeIntegerResult(yInt - 1, "held sync yInt");
                } else {
                    zInt = safeIntegerResult(zInt - 1, "held sync zInt");
                    yInt = safeIntegerResult(yInt + 1, "held sync yInt");
                }
                const xInt = safeIntegerResult(xBase + xAdjustment, "held sync xInt");
        ...
        const heldFrame = authoredFrame(held.frames, held.frame);
        if (heldFrame !== undefined && (heldFrame.state === 12 || heldFrame.state === 10)) {
            ...
            held = Object.freeze({
                ...
                vx: (holder.hitCount === 1 ? holder.knockbackVx : holder.vx) / 3,
                y: held.y < -2 ? -2 : held.y,
            });
...
export function applyPickupInputs(state                 , pickups                           )                        {
    ...
    for (const pickup of pickups) {
        let picker = working.slots[pickup.pickerSlot];
        let held = working.slots[pickup.weaponSlot];
        ...
        held = Object.freeze({
            ...held,
            linkState: heldLink,
            targetIdx: pickup.kind === 7 ? picker.slot : held.targetIdx,
            holderIdx: picker.slot,
            holderCopy: picker.slot,
            unk364: picker.unk364,
            ...(held.rawObjectType === 6 && held.hp <= 0 ? { unk31C: 0 } : {}),
        });
        working = replaceSlot(working, picker.slot, picker);
        working = replaceSlot(working, held.slot, held);
    ...
    return Object.freeze({ state: working, events: Object.freeze(events), ruleIds: Object.freeze(ruleIds) });
}
...
export function validateHeldWeaponCaches(state                 )                        {
    ...
    for (let slot = 0; slot < MAX_WORLD_SLOTS; slot++) {
        ...
        if (holder === null || !holder.active || authoredFrame(holder.frames, holder.frame) === undefined
