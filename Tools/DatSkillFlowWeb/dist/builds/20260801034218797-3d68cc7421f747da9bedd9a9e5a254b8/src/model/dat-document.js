...
function replacementForCommand(command                  )                          {
    ...
    if (command.field.scalarKind === "number") {
        if (typeof command.value !== "number" || !Number.isSafeInteger(command.value)) {
            return dataDiagnostic("unsupported-edit", `Numeric field ${command.field.key} requires a safe integer.`);
        }
...
export class LosslessDatDocument {
        ...
        if (replacement.equals(command.field.rawValue)) return { applied: false, diagnostics: [] };
