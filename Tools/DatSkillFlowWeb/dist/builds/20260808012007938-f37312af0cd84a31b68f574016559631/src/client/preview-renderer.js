...
function drawBackgroundLayer(
    ...
    if (!assetId || (layer.cc > 0 && (tick.tick < layer.c1 || tick.tick > layer.c2))) return;
    ...
    const parallax = stageWidth > context.canvas.width
        ? -((parallaxWidth - context.canvas.width) * tick.cameraX) / (stageWidth - context.canvas.width)
        : 0;
    ...
    for (let tileX = x + layer.loop; layer.x + (tileX - x) < parallaxWidth; tileX += layer.loop) {
...
function drawShadow(
    ...
    if (!shadow || !assetId || entity.link < 0) return;
...
function drawEntity(
    ...
    const frame = isPrimary ? input.runtimeFrame ?? lastFrameForId(frames, entity.frame) : lastFrameForId(frames, entity.frame);
    const pic = number(entity.renderPic ?? frame?.pic ?? entity.pic, 999);
...
export function drawPreviewCanvas(input                    )                             {
    ...
    let geometry                             = [];
    const entities = [...input.tick.entities].sort((left, right) =>
        number(left.zInt ?? left.z) - number(right.zInt ?? right.z));
    for (const entity of entities) {
        drawShadow(context, input, entity, primary);
