// dat-skill-flow-build:20260801083404917-0eed8a3a29e8405e9a48d323fe1cf1a3
...
function spawnChild(
    ...
    const childDat = resolveDat(state.catalog, opoint.oid);
    if (childDat === undefined) {
        return undefined;
    }
    const slot = firstFreeSpawnSlot(state);
    if (slot < 0) {
        return undefined;
    }
    ...
    const facing        = facingMode === 0
        ...
        : facingMode === 1
            ? (parent.facing === 0 ? 1 : 0)
            : 0;
    const xInt = parent.facing === 0
        ? parent.xInt - (frame.centerx ?? 0) + opoint.x
        : parent.xInt + (frame.centerx ?? 0) - opoint.x;
    const yInt = parent.yInt - (frame.centery ?? 0) + opoint.y;
    const z = parent.z + 1;
    let vz = 0;
    ...
    const reset = resetSlotCooldowns(working, slot);
    working = replaceSlot(working, slot, child, {
        objectCount: working.objectCount + 1,
        nextSpawnOrdinal: ordinal + 1,
        attackRest: reset.attackRest,
        vrest: reset.vrest,
    });
...
export function processOpointSpawn(state                 , parentSlot        )                      {
    const parent = state.slots[parentSlot];
    if (parent === null || !parent.active) {
    ...
    for (const opoint of opoints) {
        ...
        for (let index = 0; index < decoded.count; index++) {
            const spawned = spawnChild(working, parentSlot, frame, opoint, decoded.mode);
            ...
            if (latestParent !== null && latestParent.rawObjectType === 3 && frame.state === 3003) {
                ...
                if (linkedSlot >= 0 && linkedSlot < working.slots.length && working.slots[linkedSlot]?.active === true) {
                    let vrest = setVrest(working.vrest, linkedSlot, child.slot, 10);
                    vrest = setVrest(vrest, child.slot, linkedSlot, 10);
                    working = freezeSimulationState({ ...working, vrest });
                }
            }
        ...
        if (spawnedSlots.length > 1) {
            ...
            for (let index = 0; index < spawnedSlots.length; index++) {
                ...
                for (let previous = 0; previous < index; previous++) {
                    ...
                    let vrest = setVrest(working.vrest, slot, other, 0x28);
                    vrest = setVrest(vrest, other, slot, 0x28);
                    working = freezeSimulationState({ ...working, vrest });
                }
            }
