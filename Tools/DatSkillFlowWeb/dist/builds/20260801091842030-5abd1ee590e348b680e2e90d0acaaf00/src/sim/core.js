...
function bindFrameSources(
                                                                                      ...
                                                                                      
  {
    ...
    let aggregateContentUnits = 0;
    const maxAggregateContentUnits = 1_000_000;
    const register = (
        frames                               ,
        identity                    = frames,
    ) => {
        ...
        const contentUnits = frames.length + frames.reduce((total, frame) => total + (frame.opoints?.length ?? 0), 0);
        aggregateContentUnits = requireSafeInteger(aggregateContentUnits + contentUnits, "frame source content budget");
        if (aggregateContentUnits > maxAggregateContentUnits) {
            throw new RangeError(`aggregate unique frame-source content exceeds ${maxAggregateContentUnits} units`);
        }
