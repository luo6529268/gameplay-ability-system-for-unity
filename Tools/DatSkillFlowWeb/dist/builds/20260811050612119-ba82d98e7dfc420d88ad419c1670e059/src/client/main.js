...
function actionStartIndex()         {
    ...
    const value = number(project?.nativeTrace.rootSkillStartedTick, -1);
    return value < 0 ? -1 : Math.min(last, Math.max(0, Math.trunc(value)));
}
function playbackEndIndex()         {
    const last = Math.max(0, (project?.nativeTicks.length ?? 1) - 1);
    const start = actionStartIndex();
    const end = Math.min(last, Math.max(0, Math.trunc(number(project?.nativeTrace.playbackEndTick, last))));
    return start < 0 || end < start ? last : end;
}
function progressEndIndex()         {
    ...
    return Math.min(last, Math.max(0, Math.trunc(value)));
}
