...
export class ProjectDatService {
    ...
    async #runPreview(
        ...
        try {
            ...
            return enrichNativePreview({ ...rawPreview, resources }, resources, objectTypes, binding.oid);
        } catch (error) {
    ...
    async #loadPreviewResources(
        ...
        const loaded = await Promise.all(auxiliary.map(async (object) => this.#openPreviewObject(binding, object)));
        resources.push(...loaded.filter((resource)                                       => resource !== undefined));
        return resources;
    }
    ...
    async #openPreviewObject(
        ...
        try {
            session = await opened.service.openDocument(opened.documentId, "encrypted");
            return await this.#projectPreviewObject(binding, object, session.projection);
        } catch {
            return undefined;
        } finally {
            if (session !== undefined) await opened.service.close(session.sessionId);
            opened.registry.closeDocument(opened.documentId);
        }
    }
