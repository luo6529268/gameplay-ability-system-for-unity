...
export function enrichNativePreview(
    ...
    const ticks = preview.ticks.map((tick) => {
        ...
        const enrichedEntities = tick.entities.map((entity) => {
            ...
            if (kind === "root" && rootSkillEndedTick === null && tick.tick > 0 && isRootEnded(entity, resource)) {
    ...
    const playbackEndTick = Math.max(rootSkillEndedTick ?? lastTick, projectileEndTick);
