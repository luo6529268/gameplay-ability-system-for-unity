...
export function doFrameJump(
    ...
    const definition = frameDefinition(entity, resolvedTarget);
    ...
    const hpCost = Math.floor(mp / 1000) * 10;
    ...
    const current = replaceEntity(entity, {
        ...
        facing,
        hp: ppModeEnabled ? entity.hp - hpCost : entity.hp,
        pp: ppModeEnabled ? entity.pp - ppCost : entity.pp,
        comboCountVic: ppModeEnabled ? entity.comboCountVic + hpCost : entity.comboCountVic,
        ppDisplay: ppModeEnabled ? entity.ppDisplay + ppCost : entity.ppDisplay,
...
export function postCooldownInput(entity           , world                    )                          {
    ...
    for (const wrapper of ORDINARY_COMBOS) {
        const definition = frameDefinition(current, current.frame);
        if (definition === undefined) {
            continue;
        }
    ...
    const djaDefinition = frameDefinition(current, current.frame);
    ...
    const directDefinition = frameDefinition(current, current.frame);
