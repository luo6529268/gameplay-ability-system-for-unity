...
import { isSignedInt32, parseNativeInt32Token,                                     } from "../syntax/byte-cst.js";
...
function nativeIntegerPrefix(rawValue            )                                  {
...
function nativeIntegerPair(rawValue            )                            {
...
function nativeNumberPrefix(rawValue            )         {
    const text = Buffer.from(rawValue).toString("latin1");
    ...
    const append = (fields                        , base                                   )       => {
        ...
        for (const field of fields) {
            ...
            if (semanticKind === "integer-pair") {
                ...
                descriptors.push({
                    field: integerPairValue === field.integerPairValue ? field : {
                        ...field,
                        scalarKind: "number",
                        integerPairValue,
                    },
            ...
            if (semanticKind === "string") {
                ...
                const numericField = field.scalarKind === "number" && field.numericValue === numericValue
                    ? field
                    : { ...field, scalarKind: "number"         , numericValue };
                descriptorField = numericField;
...
export class DatSessionService {
    ...
    #requireSessionForTrustedEmission(sessionId        )               {
        const session = this.#sessions.get(sessionId);
