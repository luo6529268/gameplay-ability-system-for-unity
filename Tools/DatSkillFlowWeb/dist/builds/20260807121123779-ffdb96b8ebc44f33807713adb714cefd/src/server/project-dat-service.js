...
export function previewDatProjection(bytes            )                {
    ...
    while (offset < input.length && (input[offset] === 0x09 || input[offset] === 0x0a
        || input[offset] === 0x0d || input[offset] === 0x20)) {
        offset += 1;
    }
    return (input[offset] === 0x3c
        ? LosslessDatDocument.fromPlaintext(input)
        : LosslessDatDocument.fromEncrypted(input)).projection;
}
