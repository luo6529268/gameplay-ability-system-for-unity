...
function syncReadOnlyUi()       {
    ...
    const progressEnd = number(project?.nativeTrace.progressEndTick, -1);
    const playbackEnd = number(project?.nativeTrace.playbackEndTick, Math.max(0, count - 1));
...
function step()       {
    const last = Math.min(
        Math.max(0, (project?.nativeTicks.length ?? 1) - 1),
        number(project?.nativeTrace.playbackEndTick, Math.max(0, (project?.nativeTicks.length ?? 1) - 1)),
    );
