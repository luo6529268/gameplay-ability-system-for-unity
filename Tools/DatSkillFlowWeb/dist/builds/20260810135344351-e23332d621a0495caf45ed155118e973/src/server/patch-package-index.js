    ...
    const packages = packagesRaw.map((item, index)                    => {
        ...
        const records = requireArray(raw.records, `packages[${index}].records`, MAX_PACKAGE_RECORDS)
            .map((recordValue, recordIndex)                   => {
                ...
                const logicalPath = requireLogicalPath(record.logicalPath, `packages[${index}].records[${recordIndex}].logicalPath`);
                if (!datSet.has(logicalPath.toLowerCase())) {
                    throw new Error(`Patch record is not present in datFiles: ${logicalPath}`);
                }
        ...
        const diagnostics = requireArray(raw.diagnostics ?? [], `packages[${index}].diagnostics`, MAX_PACKAGE_RECORDS)
            .map((diagnosticValue, diagnosticIndex) => {
        ...
        const distinctCharacterOids = new Set(records.filter((record) => record.type === 0).map((record) => record.oid));
        const hasConflict = distinctCharacterOids.size < records.filter((record) => record.type === 0).length;
