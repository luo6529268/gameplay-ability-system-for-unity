...
async function selectInternalStage(parent            , stage            )                {
    ...
    if (planned === undefined) {
        diagnostics .textContent = `F${stage.startFrame} 参数已显示；父动作 Trace 没有到达任何已记录的 hit_* 来源 Frame，因此没有伪造内部起点。`;
        render();
        return;
    }

    diagnostics .textContent = `已在 F${planned.route.sourceFrame} 后的 Native Tick ${planned.triggerTick} 注入 ${planned.route.key}，正在生成完整动作分支。`;
    await preview(planned.scenario);
    const runtimeTick = project.nativeTicks.findIndex((tick) => (
        primaryPreviewEntity(tick.entities)?.frame === stage.startFrame
    ));
    if (runtimeTick < 0) {
        diagnostics .textContent = `已从真实入口执行并在 F${planned.route.sourceFrame} 触发 ${planned.route.key}，但 Native Trace 未到达 F${stage.startFrame}；参数选择保持不变。`;
        render();
        return;
    }
    ...
    selectedBlock = { type: "frame" };
    renderedFrameKey = "";
    diagnostics .textContent = `已从“${parent.displayName}”真实入口执行 ${planned.route.key}，并定位到 F${stage.startFrame}。`;
    render();
}
