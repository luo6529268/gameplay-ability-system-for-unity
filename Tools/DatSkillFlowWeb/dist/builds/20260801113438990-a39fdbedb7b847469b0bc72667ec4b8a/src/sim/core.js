...
export function stepSimulation(
    ...
    const motionDelayedSlots = new Set        ();
    const motionSlots = working.slots.map((entity) => {
        if (entity === null || !entity.active) return entity;
        if (entity.frameDelay !== 0 && entity.rawObjectType === 0) motionDelayedSlots.add(entity.slot);
    ...
    for (let slot = 0; slot < MAX_WORLD_SLOTS; slot++) {
        ...
        if (working.slots[slot]?.active === true && !motionDelayedSlots.has(slot)) {
