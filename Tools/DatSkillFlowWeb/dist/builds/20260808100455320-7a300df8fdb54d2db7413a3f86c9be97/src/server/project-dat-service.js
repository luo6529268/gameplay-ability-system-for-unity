...
export class ProjectDatService {
    ...
    async prepareDefaultSession(
        ...
        const workers = Array.from({ length: Math.min(PREVIEW_WARMUP_CONCURRENCY, unique.length) }, async () => {
            while (nextIndex < unique.length) {
                ...
                nextIndex += 1;
                try {
                    await this.#previewRunner.preview(emission.plaintext, unique[index] );
                } catch {
                    failed += 1;
                } finally {
                    completed += 1;
    ...
    async #refreshCatalog()                {
        this.#sweepExpired();
        if (Date.now() < this.#catalogRefreshAfter) return;
        let prepared;
        try {
    ...
    async #createPreview(
        ...
        try {
            ...
            const objectTypes = new Map([
                ...this.#catalogObjects.map((object)                            => [object.oid, object.type]),
