...
function nextDistanceToFrame(graph                            , occurrence        )         {
    if (!graph) return -1;
...
function completeActionIndexForFrame(frame       )         {
    const selectedDistance = nextDistanceToFrame(currentFlow(), frame.occurrence);
    if (selectedDistance >= 0 && selectedSkillIndex >= 0) return selectedSkillIndex;
    const candidates = skillState.skills.flatMap((skill, index) => {
        if (skill.oid !== activeProjectOid()) return [];
        const distance = nextDistanceToFrame(skillFlow(skill.startFrame), frame.occurrence);
        return distance < 0 ? [] : [{ index, distance, skill }];
    });
    const containing = candidates.filter((candidate) => candidate.distance > 0)
        .sort((left, right) => right.distance - left.distance || left.skill.startFrame - right.skill.startFrame)[0];
    if (containing) return containing.index;
    const exact = candidates.find((candidate) => candidate.skill.startFrame === frame.frameId);
    if (exact) return exact.index;
    const graphMatch = skillState.skills.findIndex((skill) => skill.oid === activeProjectOid()
        && skillFlow(skill.startFrame)?.nodes.some((node) => node.kind === "frame" && node.occurrence === frame.occurrence));
    return graphMatch;
}
...
function renderFrameBrowser()         {
    ...
    if (!body) return 0;
    const rows = latestCatalogFrames().flatMap((frame)                        => {
        const actionIndex = completeActionIndexForFrame(frame);
        const action = skillState.skills[actionIndex];
        const haystack = [frame.frameId, frame.label, frame.state, action?.displayName ?? ""]
        ...
        label.textContent = frame.label || `frame_${frame.frameId}`;
        state.textContent = String(frame.state);
        owner.textContent = action?.displayName ?? "未找到入口";
        owner.title = action ? `从 F${action.startFrame} 播放完整动作后定位到 F${frame.frameId}` : "没有可安全播放的完整动作入口";
...
async function selectFrame(frameId        , occurrence        , refreshPreview         )                {
    ...
    if (refreshPreview) {
        ...
        if (frame) await previewFrameWithinCompleteAction(frame);
    } else {
        render();
    }
}
async function previewFrameWithinCompleteAction(frame       )                   {
    if (!project) return false;
    const actionIndex = completeActionIndexForFrame(frame);
    const skill = skillState.skills[actionIndex];
    if (!skill || skill.oid !== project.oid) {
        diagnostics .textContent = `F${frame.frameId} 未找到所属完整动作；为避免伪造运行状态，未从该 Frame 单独启动。`;
        render();
        return false;
    }
    ...
    if (runtimeTick < 0) {
        diagnostics .textContent = `完整动作“${skill.displayName}”已运行，但 Native Trace 没有到达 F${frame.frameId}。`;
        return false;
    }
