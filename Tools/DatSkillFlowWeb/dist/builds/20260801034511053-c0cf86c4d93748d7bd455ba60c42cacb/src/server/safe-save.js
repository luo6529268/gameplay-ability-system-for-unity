...
export class SafeSaveService {
    ...
    async overwrite(documentId        , challengeId        , bytes            )                           {
        ...
        if (challenge.contentSha256 !== contentHash(bytes)) {
        ...
        return await this.#locks.runExclusive(challenge.canonicalTarget, async () => {
            ...
            let replacementPath                    ;
            try {
                replacementPath = await this.#writeReplacement(target, bytes);
            } catch (error) {
            ...
            const recovery = await observeRecovery(target.canonicalPath, replacementPath, backupPath)                            ;
            if (!publication.ok) {
