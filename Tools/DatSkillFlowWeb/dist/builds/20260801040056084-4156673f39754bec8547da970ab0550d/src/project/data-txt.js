...
export class DataTxtDocument {
        ...
        if (replacement.equals(field.rawValue)) return [];
...
export function selectFirstSuccessfulDuplicates                                    (
    entries              ,
    loadSucceeded                       ,
)                 {
    ...
    for (const entry of entries) {
        ...
        if (loadSucceeded(entry)) selected.set(entry.id, entry);
    }
    return selected;
}

export async function selectFirstSuccessfulDuplicatesAsync                                    (
    entries              ,
    loadSucceeded                                          ,
)                          {
    const selected = new Map           ();
    for (const entry of entries) {
        if (selected.has(entry.id)) continue;
        if (await loadSucceeded(entry)) selected.set(entry.id, entry);
    }
    return selected;
}
