...
describe("server-owned DAT session core", () => {
    ...
    it("fails closed when dispose races a deferred reload plus queued edit and close", async () => {
        ...
        await rejectsCode(service.openDocument(documentId), "invalid-request");
    });
    ...
    it("reserves concurrent opens and keeps ID/tombstone lifecycle bounded", async () => {
        ...
        const simultaneous = await Promise.allSettled([
            sessionLimited.service.openDocument(sessionLimited.documentId),
            sessionLimited.service.openDocument(sessionLimited.documentId),
        ]);
        ...
        const byteRace = await Promise.allSettled([
            byteLimited.service.openDocument(byteLimited.documentId),
            byteLimited.service.openDocument(byteLimited.documentId),
        ]);
        ...
        for (let index = 0; index < 4; index += 1) {
            const view = await lifecycle.service.openDocument(lifecycle.documentId);
            expired.push(view);
            now += 1;
