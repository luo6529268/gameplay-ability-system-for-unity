// dat-skill-flow-build:20260801043453133-4b782ab47dcd43678125d2167a9a45d1
...
async function readCurrentManifestSnapshot(canonicalRoot, manifestPath) {
    let lastError;
    for (let attempt = 0; attempt < CURRENT_MANIFEST_READ_ATTEMPTS; attempt += 1) {
        try {
            ...
            return {
                ...
                manifest: parseBuildManifest(JSON.parse(await readFile(canonicalManifestPath, "utf8"))),
            };
        } catch (error) {
            if (error?.code !== "ENOENT" || attempt === CURRENT_MANIFEST_READ_ATTEMPTS - 1) {
                throw error;
            }
            lastError = error;
            await new Promise((resolveRetry) => setTimeout(resolveRetry, 2));
        }
    }
    throw lastError;
}

export async function verifyManifestOutput({ staticRoot, manifestPath, outputPath }) {
...
export async function loadVerifiedBuildManifest({ staticRoot, manifestPath }) {
    const canonicalRoot = await realpath(resolve(staticRoot));
    const { manifest } = await readCurrentManifestSnapshot(canonicalRoot, manifestPath);
    const canonicalPinnedManifestPath = requireContained(
        canonicalRoot,
        await realpath(join(canonicalRoot, ...manifest.clientRoot.split("/"), "build-manifest.json")),
        "pinnedManifestPath",
    );
    const pinnedManifest = parseBuildManifest(JSON.parse(await readFile(canonicalPinnedManifestPath, "utf8")));
...
export async function loadVerifiedRuntimeAsset({ staticRoot, manifestPath, outputPath }) {
