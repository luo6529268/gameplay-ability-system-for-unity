...
export interface BlackColorKeyMetadata {
    enabled: true;
    red: 0;
    green: 0;
    blue: 0;
...
export interface BmpMetadataResult {
    ok: boolean;
    width: number;
    height: number;
    storedHeight: number;
    ...
    colorKey: BlackColorKeyMetadata;
    diagnostics: DataDiagnostic[];
}

export interface SpriteRangeMetadata {
    frameLo: number;
    frameHi: number;
    file: string;
    w: number;
...
export type SpriteFrameResolution =
    | { render: false; reason: "pic-999" | "range-not-found" | "invalid-grid" }
    | {
        render: true;
        file: string;
        rangeIndex: number;
...
export const BLACK_COLORKEY: BlackColorKeyMetadata = Object.freeze({
    enabled: true,
    red: 0,
    green: 0,
    blue: 0,
...
function invalidBmp(message: string): BmpMetadataResult {
    return {
        ok: false,
        width: 0,
        height: 0,
...
export function parseBmpMetadata(input: Uint8Array): BmpMetadataResult {
...
export function findSpriteRange(
    ranges: readonly SpriteRangeMetadata[],
    picture: number,
): SpriteRangeMetadata | undefined {
