...
export interface BdyProjection {
    x: number;
    y: number;
    w: number;
    h: number;
...
export interface OPointProjection {
    kind: number;
    x: number;
    y: number;
    action: number;
...
export interface WPointProjection {
    kind: number;
    x: number;
    y: number;
    attacking: number;
...
export interface BPointProjection {
    x: number;
    y: number;
}

export interface CPointProjection {
    kind: number;
    x: number;
    y: number;
    injury: number;
...
export interface DatFrameProjection {
    ...
    itrs: ItrProjection[];
    bdys: BdyProjection[];
    opoints: OPointProjection[];
    wpoints: WPointProjection[];
    bpoints: BPointProjection[];
    cpoints: CPointProjection[];
}

export interface SpriteRangeProjection {
    frameLo: number;
    frameHi: number;
    file: string;
    w: number;
...
export interface DatTopProjection {
    name: string;
    head: string;
    small: string;
...
const topDefaults: DatTopProjection = {
    name: "",
    head: "",
    small: "",
    weapon_hit_sound: "",
...
const itrDefaults: ItrProjection = {
...
const bdyDefaults: BdyProjection = { x: 0, y: 0, w: 0, h: 0 };
const opointDefaults: OPointProjection = { kind: 0, x: 0, y: 0, action: 0, dvx: 0, dvy: 0, oid: 0, facing: 0 };
const wpointDefaults: WPointProjection = { kind: 0, x: 0, y: 0, attacking: 0, cover: 0, weaponact: 0, dvx: 0, dvy: 0, dvz: 0 };
const bpointDefaults: BPointProjection = { x: 0, y: 0 };
const cpointDefaults: CPointProjection = {
...
function projectItr(block: DatBlockCst): ItrProjection {
...
function projectBdy(block: DatBlockCst): BdyProjection {
...
function projectOpoint(block: DatBlockCst): OPointProjection {
...
function projectWpoint(block: DatBlockCst): WPointProjection {
...
function projectBpoint(block: DatBlockCst): BPointProjection {
...
function projectCpoint(block: DatBlockCst): CPointProjection {
...
function projectFrameField(result: DatFrameProjection, field: DatFieldCst): void {
    if (field.key === "sound") {
...
function projectFrame(frame: DatCstDocument["frames"][number]): DatFrameProjection {
...
function spriteRangeValue(range: DatSpriteRangeCst, key: string): number {
    let result = 0;
...
export class DatProjection {
    public readonly top: Readonly<DatTopProjection>;
    public readonly spriteRanges: readonly SpriteRangeProjection[];
    public readonly frames: readonly DatFrameProjection[];
    public readonly diagnostics: readonly DataDiagnostic[];
    private readonly frameIndex = new Map<number, DatFrameProjection>();

    public constructor(cst: DatCstDocument) {
        const top: DatTopProjection = { ...topDefaults };
    ...
    public getFrame(frameId: number): DatFrameProjection | undefined {
        return this.frameIndex.get(frameId);
    }
}

export function projectDatCst(cst: DatCstDocument): DatProjection {
