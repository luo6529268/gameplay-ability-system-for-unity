...
export interface FrameStructureLocator {
    readonly kind: "frame";
    readonly frameOccurrence: number;
}

export interface BlockStructureLocator {
    readonly kind: "block";
    readonly frameOccurrence: number;
...
export type DatStructureLocator = FrameStructureLocator | BlockStructureLocator;

export type DatStructureEdit =
    | { readonly operation: "copy-frame"; readonly target: FrameStructureLocator; readonly newFrameId: number }
    | { readonly operation: "delete-frame"; readonly target: FrameStructureLocator }
    | {
        readonly operation: "create-block" | "copy-block" | "delete-block";
        readonly target: BlockStructureLocator;
    };
...
function hasTrailingLineBreak(source: Buffer, end: number): boolean {
    return end > 0 && (source[end - 1] === 0x0a || source[end - 1] === 0x0d);
}

function requireFrame(document: DatCstDocument, locator: FrameStructureLocator): DatFrameCst {
...
function requireBlock(document: DatCstDocument, locator: BlockStructureLocator): {
    frame: DatFrameCst;
    block: DatBlockCst;
} {
...
function requireEditableEncoding(document: DatCstDocument): void {
...
export function canDeleteFrame(frame: DatFrameCst): boolean {
    return frame.closed && frame.blocks.every((block) => block.closed);
}

export function canCopyFrame(document: DatCstDocument, frame: DatFrameCst): boolean {
    return canDeleteFrame(frame) && hasTrailingLineBreak(document.source, frame.span.end);
}

export function canDeleteBlock(block: DatBlockCst): boolean {
    return block.closed;
}

export function canCopyBlock(document: DatCstDocument, block: DatBlockCst): boolean {
    return canDeleteBlock(block) && hasTrailingLineBreak(document.source, block.span.end);
}

export function applyDatStructureEdit(
    document: DatCstDocument,
    edit: DatStructureEdit,
): Buffer {
