...
export interface PresentationCameraState {
    ...
    readonly cameraVel: number;
    readonly cameraMaxOverride: number;
}

export type PresentationCameraSubjectKind = "primary" | "fallback" | "synthetic";

export interface PresentationCameraStep {
    readonly camera: PresentationCameraState;
    readonly targetX: number;
    readonly maximumX: number;
    readonly subjectKind: PresentationCameraSubjectKind;
    readonly subjectSlots: readonly number[];
}
...
function truncDivision(numerator: number, denominator: number, label: string): number {
    return safeInteger(Math.trunc(numerator / denominator), label);
}

export function createPresentationCamera(seed: Partial<PresentationCameraState> = {}): PresentationCameraState {
...
export function stepPresentationCamera(
    ...
    if (maximumX <= 0) {
        return Object.freeze({
            ...
            targetX: 0,
            maximumX,
            subjectKind: "synthetic" as const,
    ...
    let subjectKind: PresentationCameraSubjectKind = "primary";
