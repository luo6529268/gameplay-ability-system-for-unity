import { dataDiagnostic, type ByteSpan, type DataDiagnostic } from "./data-diagnostic.js";

export type ByteTokenKind =
    | "bom"
    | "line-break"
    | "whitespace"
    | "comment"
...
export type DatBlockType = "itr" | "bdy" | "opoint" | "wpoint" | "bpoint" | "cpoint";
export type DatScalarKind = "number" | "string" | "opaque";

export interface DatFieldCst {
    key: string;
    keySpan: ByteSpan;
    valueSpan: ByteSpan;
    rawValue: Buffer;
...
export interface DatSpriteRangeCst {
    frameLo: number;
    frameHi: number;
    file: string;
    fileField: DatFieldCst;
    fields: DatFieldCst[];
    span: ByteSpan;
}

export type DetectedByteEncoding = "bytes" | "utf8-bom" | "utf16le" | "utf16be" | "utf32le" | "utf32be";

export interface DatCstDocument {
    source: Buffer;
    encoding: DetectedByteEncoding;
    tokens: ByteToken[];
    topFields: DatFieldCst[];
    spriteRanges: DatSpriteRangeCst[];
    frames: DatFrameCst[];
...
const blockTypes = new Set<DatBlockType>(["itr", "bdy", "opoint", "wpoint", "bpoint", "cpoint"]);

function isAsciiIdentifierByte(value: number): boolean {
...
function isAsciiLetterOrUnderscore(value: number): boolean {
    return (value >= 0x41 && value <= 0x5a)
        || (value >= 0x61 && value <= 0x7a)
        || value === 0x5f;
}
...
function isOpaqueByte(value: number): boolean {
    return value === 0;
}

function ascii(source: Uint8Array, start: number, end: number): string {
    let result = "";
...
function detectEncoding(source: Uint8Array): DetectedByteEncoding {
    if (source.length >= 4) {
...
export function lexBytes(input: Uint8Array): ByteToken[] {
...
function splitLines(source: Uint8Array): LineSpan[] {
...
function trimHorizontal(source: Uint8Array, span: ByteSpan): ByteSpan {
...
function lineWithoutComment(source: Uint8Array, line: ByteSpan): ByteSpan {
...
function findFieldMatches(source: Uint8Array, span: ByteSpan): FieldMatch[] {
...
function scalarKindAndNumber(raw: Buffer): { scalarKind: DatScalarKind; numericValue?: number } {
...
type FieldParseScope = "top" | "sprite" | "frame" | "block";

const topAuthorityStringKeys = new Set([
    "name", "head", "small", "weapon_hit_sound", "weapon_drop_sound", "weapon_broken_sound",
]);

function isAuthorityStringField(key: string, scope: FieldParseScope): boolean {
...
function parseFieldsOnLine(
    source: Buffer,
    contentInput: ByteSpan,
    ignoredKeys: ReadonlySet<string>,
    context: { scope: FieldParseScope; frameOccurrence?: number; blockType?: DatBlockType; blockIndex?: number },
): DatFieldCst[] {
    const content = lineWithoutComment(source, contentInput);
    const matches = findFieldMatches(source, content);
    const fields: DatFieldCst[] = [];
...
export function parseDatCst(input: Uint8Array): DatCstDocument {
    const source = Buffer.from(input);
    ...
    const topFields: DatFieldCst[] = [];
    const spriteRanges: DatSpriteRangeCst[] = [];
    ...
    for (const line of splitLines(source)) {
        ...
        if (currentFrame) {
            ...
            if (blockStart && blockTypes.has(blockStart[1] as DatBlockType)) {
                if (currentBlock) {
                    diagnostics.push(dataDiagnostic("malformed-block", `Unclosed ${currentBlock.type} block.`, { span: currentBlock.span }));
                }
