...
const DEFAULT_WPOINT: SimWpointDefinition = Object.freeze({
    kind: 0,
    x: 0,
    y: 0,
    attacking: 0,
...
function exactSafeInteger(value: bigint, label: string): number {
...
export function firstWpoint(frame: { readonly wpoints?: readonly SimWpointDefinition[] } | undefined): SimWpointDefinition {
    return frame?.wpoints?.[0] ?? DEFAULT_WPOINT;
}

export function resolveHeldAttackPayload(
    heldItr: SimItrDefinition | undefined,
    held: SimEntity,
    holder: SimEntity | undefined,
    victimSlot: number,
): SimItrDefinition | undefined {
    ...
    const replacement: Record<string, number> = { ...heldItr, kind: 0 };
    if (payload !== undefined) {
        for (const key of itrPayloadKeys) replacement[key] = payload[key];
    }
    return Object.freeze(replacement) as unknown as SimItrDefinition;
}

function event(
    kind: HeldObjectEvent["kind"],
    holder: SimEntity | undefined,
    held: SimEntity | undefined,
    holderSlot: number,
    heldSlot: number,
    pass: 5 | 12 | null,
    ruleId: string,
...
function clearNormalRelease(holder: SimEntity, held: SimEntity): readonly [SimEntity, SimEntity] {
    const holderChanges: Partial<SimEntity> = { linkState: 0 };
...
function drawModulo(state: SimulationState, modulo: number): readonly [SimulationState, number] {
...
export interface HeldPassResult {
    readonly state: SimulationState;
    readonly events: readonly HeldObjectEvent[];
    readonly ruleIds: readonly string[];
}

export function runHeldObjectPass(state: SimulationState, pass: 5 | 12): HeldPassResult {
    let working = state;
    const events: HeldObjectEvent[] = [];
    const rules: string[] = [GATE3B2_WPOINT_RULE.heldPassOrder];
...
function requirePickupInteger(value: unknown, label: string): number {
...
export function parsePickupInputs(input: SimulationInput): readonly SimPickupInput[] {
    ...
    return Object.freeze(input.pickups.map((candidate, index) => {
        ...
        const record = candidate as SimJsonObject;
...
export function assertPickupInputCapacity(input: SimulationInput): void {
...
export interface WpointOperationResult {
    readonly state: SimulationState;
    readonly events: readonly HeldObjectEvent[];
    readonly ruleIds: readonly string[];
}

export function applyPickupInputs(state: SimulationState, pickups: readonly SimPickupInput[]): WpointOperationResult {
...
export function validateHeldWeaponCaches(state: SimulationState): WpointOperationResult {
    let working = state;
    const events: HeldObjectEvent[] = [];
...
export function validatePositiveLinks(state: SimulationState): WpointOperationResult {
    let working = state;
    const events: HeldObjectEvent[] = [];
    for (let slot = 0; slot < MAX_WORLD_SLOTS; slot++) {
        const holder = working.slots[slot];
...
export function forceDropHeldWeapon(state: SimulationState, holderSlot: number): SimulationState {
