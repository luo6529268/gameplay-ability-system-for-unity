...
export interface ProcessOpointResult {
...
const EMPTY_COOLDOWNS: SimInputCooldowns = Object.freeze({
    right: 0, left: 0, up: 0, down: 0, attack: 0, jump: 0, defend: 0,
});
const EMPTY_COMBOS: SimInputCombos = Object.freeze({
    DRA: 0, DLA: 0, DUA: 0, DDA: 0, DRJ: 0, DLJ: 0, DUJ: 0, DDJ: 0, DJA: 0,
});

function nextStableId(state: SimulationState, slot: number): { stableId: string; ordinal: number } {
...
function decodeFacing(facing: number): { count: number; mode: number } {
    if (facing > 10) {
...
function spawnChild(
    ...
    parentSlot: number,
    frame: SimFrameDefinition,
    opoint: SimOpointDefinition,
    facingMode: number,
    childDat: SimDatDefinition,
    slot: number,
): { state: SimulationState; child: SimEntity; event: OpointSpawnEvent; lifecycle: SlotLifecycleEvent } | undefined {
    const parent = state.slots[parentSlot];
    if (parent === null || !parent.active) {
        return undefined;
    }
    const { stableId, ordinal } = nextStableId(state, slot);
    const facing: 0 | 1 = facingMode === 0
        ? parent.facing
        : facingMode === 1
            ? (parent.facing === 0 ? 1 : 0)
            : 0;
    ...
    const child: SimEntity = Object.freeze({
        stableId,
        slot,
    ...
    const event: OpointSpawnEvent = Object.freeze({
        stableId,
        slot,
...
export function processOpointSpawn(
    state: SimulationState,
    parentSlot: number,
    runtime: Pick<SimulationRuntime, "onOpointAllocationAttempt" | "onOpointVrestOperation"> = {},
): ProcessOpointResult {
    const parent = state.slots[parentSlot];
    if (parent === null || !parent.active) {
    ...
    const spawns: OpointSpawnEvent[] = [];
    const slotLifecycle: SlotLifecycleEvent[] = [];
