...
function normalizeOpoint(candidate: SimOpointDefinition): SimOpointDefinition {
...
const wpointKeys = ["kind", "x", "y", "attacking", "cover", "weaponact", "dvx", "dvy", "dvz"] as const;

function normalizeWpoint(candidate: SimWpointDefinition): SimWpointDefinition {
    return Object.freeze(Object.fromEntries(wpointKeys.map((key) => [
        key,
        safeInteger(candidate[key] ?? 0, `wpoint.${key}`),
    ]))) as unknown as SimWpointDefinition;
}

const itrDefaults: SimItrDefinition = Object.freeze({
...
const itrKeys = Object.keys(itrDefaults) as readonly (keyof SimItrDefinition)[];

function normalizeItr(candidate: SimItrDefinition): SimItrDefinition {
    return Object.freeze(Object.fromEntries(itrKeys.map((key) => [
        key,
        safeInteger(candidate[key] ?? itrDefaults[key], `itr.${key}`),
    ]))) as unknown as SimItrDefinition;
}

export function normalizeFrames(frames: readonly SimFrameDefinition[]): readonly SimFrameDefinition[] {
    if (frames.length > MAX_FRAME_IDS) {
        throw new RangeError("frames must contain at most 600 definitions");
    }
    const definitions = new Map<number, SimFrameDefinition>();
...
const EMPTY_FRAME: SimFrameDefinition = Object.freeze({
    id: 0,
    state: 0,
    wait: 1,
    next: 0,
...
export function createDatCatalog(
    explicit: readonly SimDatSeed[] | undefined,
    legacySeeds: readonly SimEntitySeed[],
): readonly (SimDatDefinition | null)[] {
    const catalog: (SimDatDefinition | null)[] = Array(MAX_CATALOG_OIDS).fill(null);
