...
const cooldownKeys = ["right", "left", "up", "down", "attack", "jump", "defend"] as const;
const comboKeys = ["DRA", "DLA", "DUA", "DDA", "DRJ", "DLJ", "DUJ", "DDJ", "DJA"] as const;

function normalizeCooldowns(seed: Partial<SimInputCooldowns> | undefined): SimInputCooldowns {
    return Object.freeze(Object.fromEntries(cooldownKeys.map((key) => [
        key,
        requireSafeInteger(seed?.[key] ?? 0, `entity.cooldowns.${key}`),
    ]))) as unknown as SimInputCooldowns;
}

function normalizeCombos(seed: Partial<SimInputCombos> | undefined): SimInputCombos {
    ...
    return Object.freeze(Object.fromEntries(entries)) as unknown as SimInputCombos;
}

function normalizeEntity(
    seed: SimEntitySeed,
    frames: readonly SimFrameDefinition[],
    frameSourceIndex: number,
): SimEntity {
...
function bindFrameSources(
    catalog: readonly (SimDatDefinition | null)[],
    seeds: readonly SimEntitySeed[],
    explicitOids: ReadonlySet<number>,
    onCanonicalize?: () => void,
): {
    catalog: readonly (SimDatDefinition | null)[];
    frameSources: readonly (readonly SimFrameDefinition[])[];
    seedBindings: readonly { frames: readonly SimFrameDefinition[]; index: number }[];
} {
    const sources: (readonly SimFrameDefinition[])[] = [];
    const indices = new Map<string, number>();
    const identityIndices = new WeakMap<object, number>();
    const register = (
        frames: readonly SimFrameDefinition[],
        identity: readonly object[] = frames,
    ) => {
    ...
    const firstLegacyOid = new Set<number>();
...
function freeEntityAt(state: SimulationState, slot: number): SimulationState {
...
export function freeEntity(state: SimulationState, stableId: string): SimulationState {
...
export function replaySimulation(
    ...
    script: readonly SimulationInput[],
    runtime: SimulationRuntime = {},
): SimulationReplayResult {
    let state = initial;
    const traces = [];
