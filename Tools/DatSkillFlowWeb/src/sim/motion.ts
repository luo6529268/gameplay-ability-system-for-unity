...
import type { SimEntity } from "./types.js";

export interface MotionResult { readonly entity: SimEntity; readonly ruleIds: readonly string[]; }

function replace(entity: SimEntity, changes: Partial<SimEntity>): SimEntity {
    return Object.freeze({ ...entity, ...changes });
}

function snapshot(value: number, label: string): number {
    const result = Math.trunc(value);
    if (!Number.isSafeInteger(result)) throw new TypeError(`${label} must be a safe integer`);
    return result;
}

function applyFrameVelocity(value: number, velocity: number, direction: number): number {
...
function applyGroundFriction(velocity: number): number {
    if (velocity > 0.0001) {
...
export function runMotion(entity: SimEntity): MotionResult {
    const rules: string[] = [GATE4_MOTION_RULE.frameAdvanceGates];
