...
export function createSlots(entities: readonly SimEntity[]): readonly (SimEntity | null)[] {
    const slots: (SimEntity | null)[] = Array(MAX_WORLD_SLOTS).fill(null);
...
export function entitiesFromSlots(slots: readonly (SimEntity | null)[]): readonly SimEntity[] {
    return Object.freeze(slots.filter((entity): entity is SimEntity => entity !== null));
}
...
export function replaceSlot(
    ...
    changes: Partial<SimulationState> = {},
): SimulationState {
...
export function firstFreeSpawnSlot(state: SimulationState): number {
...
export function normalizeVrest(entries: readonly SimVrestEntry[] | undefined): readonly SimVrestEntry[] {
    const values = new Map<string, SimVrestEntry>();
...
export function vrestAt(state: SimulationState, fromSlot: number, toSlot: number): number {
    return state.vrest.find((entry) => entry.fromSlot === fromSlot && entry.toSlot === toSlot)?.ticks ?? 0;
}

export function resetSlotCooldowns(state: SimulationState, slot: number): Pick<SimulationState, "attackRest" | "vrest"> {
...
export function setVrest(
    entries: readonly SimVrestEntry[],
    fromSlot: number,
    toSlot: number,
    ticks: number,
): readonly SimVrestEntry[] {
...
export interface VrestBuilder {
    resetSlot(slot: number): void;
    set(fromSlot: number, toSlot: number, ticks: number): void;
    materialize(): readonly SimVrestEntry[];
}

export function createVrestBuilder(
    entries: readonly SimVrestEntry[],
    onOperation?: (kind: OpointVrestOperationKind) => void,
): VrestBuilder {
