...
export interface FrameTickResult {
    ...
    readonly transitions: readonly FrameTransitionEvent[];
...
function transitionEvent(
    entity: SimEntity,
    kind: FrameTransitionEvent["kind"],
    fromFrame: number,
    toFrame: number,
    rawNext: number | null,
): FrameTransitionEvent {
...
export function frameTick(entity: SimEntity, resolvedDat?: SimDatDefinition): FrameTickResult {
    let current = entity;
    const transitions: FrameTransitionEvent[] = [];
    ...
    if (current.attacking > definition.wait) {
        ...
        if (rawNext === 0) {
            ...
            let kind: FrameTransitionEvent["kind"];
