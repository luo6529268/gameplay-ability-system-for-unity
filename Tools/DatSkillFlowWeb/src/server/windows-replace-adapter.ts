...
export interface ReplacePublisher {
...
export interface WindowsReplaceFilePublisherOptions {
    scriptPath?: string;
    runtimeAsset?: VerifiedRuntimeAssetDescriptor;
    executable?: string;
...
interface RuntimeAssetVerification {
    staticRoot: string;
    manifestPath: string;
    outputPath: string;
}

const mappedErrors: Readonly<Record<number, { code: string; message: string }>> = Object.freeze({
    1175: {
...
export function mapReplaceFileError(win32Code: number): { code: string; message: string } {
...
function parseHelperOutput(stdout: string): { ok: boolean; win32Code?: number; message?: string } {
    const lines = stdout.trim().split(/\r?\n/).filter((line) => line.length > 0);
    const lastLine = lines.at(-1);
    if (lastLine === undefined) {
        throw new Error("ReplaceFileW helper returned no structured output.");
    }
    const value = JSON.parse(lastLine) as Record<string, unknown>;
    if (typeof value.ok !== "boolean") {
        throw new Error("ReplaceFileW helper returned an invalid result.");
    }
    if (!value.ok && (!Number.isSafeInteger(value.win32Code) || (value.win32Code as number) < 0)) {
...
export class WindowsReplaceFilePublisher implements ReplacePublisher {
    readonly #scriptPath: string;
    readonly #executable: string;
    readonly #execFile: ExecFileLike;
    readonly #verifiedRuntimeAssetPath?: Promise<string>;

    constructor(options: WindowsReplaceFilePublisherOptions = {}) {
        ...
        if (options.scriptPath === undefined && options.runtimeAsset === undefined) {
            ...
            const runtimeAssetVerification: RuntimeAssetVerification = {
                staticRoot,
    ...
    async replace(request: ReplaceRequest): Promise<ReplaceResult> {
        ...
        const result = await new Promise<{ error: Error | null; stdout: string; stderr: string }>((resolveResult) => {
        ...
        let helper: ReturnType<typeof parseHelperOutput>;
        try {
        ...
        const win32Code = helper.win32Code as number;
