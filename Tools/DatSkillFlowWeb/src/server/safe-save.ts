...
export interface SafeSaveServiceOptions {
...
export type SafeSaveErrorCode =
    | "content-too-large"
    | "overwrite-required"
    | "save-as-write-failed"
    | "challenge-invalid"
    | "challenge-expired"
    ...
    | "external-change"
    | "temp-write-failed"
    | "replace-invocation-failed"
    | "replace-failed"
    | "replace-result-inconsistent";

export class SafeSaveError extends Error {
    readonly code: SafeSaveErrorCode;
    readonly recovery?: RecoveryReport;
    readonly win32Code?: number;

    constructor(
        code: SafeSaveErrorCode,
        message: string,
        options: ErrorOptions & { recovery?: RecoveryReport; win32Code?: number } = {},
    ) {
...
export interface OverwriteResult {
    status: "overwritten";
    document: OpenedDocument;
    recovery: Required<RecoveryReport>;
}

interface ChallengeRecord extends OverwriteChallenge {
...
class TargetLockManager {
    ...
    async runExclusive<T>(targetKey: string, operation: () => Promise<T>): Promise<T> {
...
function ensureContentBound(bytes: Uint8Array): void {
...
function publicObservation(value: NativePathObservation): PathObservation {
    return { ...value };
}

function publicRecovery(value: NativeRecoveryReport | undefined): RecoveryReport | undefined {
...
function targetLockKey(rootId: string, logicalPath: string): string {
...
export class SafeSaveService {
    ...
    readonly #locks = new TargetLockManager();
    readonly #challenges = new Map<string, ChallengeRecord>();

    constructor(registry: WorkspaceRegistry, options: SafeSaveServiceOptions = {}) {
    ...
    async issueOverwriteChallenge(
        ...
        const challenge: ChallengeRecord = Object.freeze({
    ...
    async overwrite(documentId: string, challengeId: string, bytes: Uint8Array): Promise<OverwriteResult> {
        ensureContentBound(bytes);
        const stableBytes = Buffer.from(bytes);
    ...
    #mapOverwriteFailure(error: unknown): SafeSaveError {
        if (!(error instanceof NativeSafeFileError)) {
