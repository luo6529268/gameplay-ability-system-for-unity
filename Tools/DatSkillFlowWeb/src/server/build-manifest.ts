...
export interface BuildManifest {
    ...
    serverEntry: string;
    testFiles: string[];
    runtimeAssets: BuildFileEntry[];
    outputs: BuildFileEntry[];
    clientFiles: BuildFileEntry[];
}
...
function expectPortableRelativePath(value: unknown, path: ReadonlyArray<string | number>): string {
...
export const buildManifestSchema = validator<BuildManifest>((value) => {
    ...
    const outputByPath = new Map<string, BuildFileEntry>();
    ...
    const runtimeAssets = expectArray(record.runtimeAssets, ["runtimeAssets"]).map((entry, index) => (
        parseFileEntry(entry, ["runtimeAssets", index], buildId)
    ));
    const runtimeAssetPaths = new Set<string>();
    for (const [index, entry] of runtimeAssets.entries()) {
        if (!entry.path.startsWith(`${clientRoot}/runtime/`)) {
            fail(["runtimeAssets", index, "path"], "runtime asset is outside the current runtime root");
        }
        if (runtimeAssetPaths.has(entry.path)) {
            fail(["runtimeAssets", index, "path"], `duplicate runtime asset: ${entry.path}`);
        }
        runtimeAssetPaths.add(entry.path);
        const output = outputByPath.get(entry.path);
        if (output === undefined || output.size !== entry.size || output.sha256 !== entry.sha256) {
            fail(["runtimeAssets", index], "runtime asset is not an exact current-build output");
        }
    }
