...
export const STATE_CHANGE_TOKEN_HEADER = "x-dat-skill-flow-token" as const;
...
const applicationServers = new WeakSet<Server>();
const applicationServerWorkspaces = new WeakMap<Server, WorkspaceRegistry>();
...
export interface ApplicationServerOptions {
    staticRoot: string;
    manifestPath: string;
    staticConfiguration?: Promise<StaticConfiguration>;
...
interface StaticConfiguration {
    ...
    clientFileByPath: ReadonlyMap<string, CachedStaticFile>;
}

interface CachedStaticFile {
    entry: BuildFileEntry;
    body: Buffer;
}

export interface ApplicationServerSecurity {
    tokenHeader: typeof STATE_CHANGE_TOKEN_HEADER;
    token: string;
}

export function getApplicationServerSecurity(server: Server): ApplicationServerSecurity {
...
export function authorizeStateChangingRequest(server: Server, providedToken: string | undefined): boolean {
...
function portablePathToNative(filePath: string): string {
...
export async function loadPinnedStaticConfiguration(
    ...
    const clientFileByPath = new Map<string, CachedStaticFile>();
    for (const entry of verifiedManifest.clientFiles) {
...
async function loadStaticConfiguration(options: ApplicationServerOptions): Promise<StaticConfiguration> {
...
function decodeRequestPath(rawTarget: string): string {
...
function exactStateChangingOrigin(server: Server, request: IncomingMessage): boolean {
...
function safeRecoveryDetails(error: SafeSaveError): Record<string, unknown> {
    const observation = (value: PathObservation | undefined): Record<string, unknown> | undefined => (
        value === undefined
            ? undefined
            : {
...
function sendApiError(request: IncomingMessage, response: ServerResponse, error: unknown): void {
...
function expectedRequestOrigin(server: Server): string | undefined {
...
function hasTrustedRequestOrigin(server: Server, request: IncomingMessage): boolean {
    const expectedOrigin = expectedRequestOrigin(server);
...
function advertisesRequestBody(request: IncomingMessage): boolean {
...
async function handleApiRequest(
    ...
    if (readMatch !== null) {
        ...
        try {
            const result = await workspace.readDocument(readMatch[1] as string);
            sendJson(request, response, 200, {
                ok: true,
                data: {
    ...
    if (!exactStateChangingOrigin(server, request)
        || !authorizeStateChangingRequest(server, request.headers[STATE_CHANGE_TOKEN_HEADER] as string | undefined)) {
...
export function createApplicationServer(options: ApplicationServerOptions): Server {
    ...
    const getStaticConfiguration = (): Promise<StaticConfiguration> => {
...
export interface LoopbackListenOptions {
    maxEphemeralListenAttempts?: number;
    isFetchBlockedPort?: (port: number) => boolean;
    listen?: (server: Server, port: number) => Promise<AddressInfo | null>;
...
async function listenOnLoopback(server: Server, port: number): Promise<AddressInfo | null> {
