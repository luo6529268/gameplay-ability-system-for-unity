...
interface CadenceProject extends PreviewProject {
    ...
    readonly packageId: string;
    readonly packageLabel: string;
    readonly nativeTicks: readonly PreviewTick[];
    readonly nativeTrace: Json;
...
const status = select<HTMLElement>("cadence-status");
const summary = select<HTMLElement>("cadence-summary");
const packageSelect = select<HTMLSelectElement>("cadence-package");
const characterSelect = select<HTMLSelectElement>("cadence-character");
const skillSelect = select<HTMLSelectElement>("cadence-skill");
const replayButton = select<HTMLButtonElement>("cadence-replay");
const playButton = select<HTMLButtonElement>("cadence-play");
const resetButton = select<HTMLButtonElement>("cadence-reset");
const speedSelect = select<HTMLSelectElement>("cadence-speed");
const loopToggle = select<HTMLInputElement>("cadence-loop");

const panes: readonly CadencePane[] = Object.freeze(
    Array.from(document.querySelectorAll<HTMLElement>("[data-cadence-rate]")).flatMap((pane): CadencePane[] => {
        const rate = Number(pane.dataset.cadenceRate);
        const canvas = pane.querySelector<HTMLCanvasElement>("canvas");
        const readout = pane.querySelector<HTMLElement>(".pane-readout");
        return RENDER_CADENCE_RATES.includes(rate as RenderCadenceRate) && canvas !== null && readout !== null
            ? [{ rate: rate as RenderCadenceRate, canvas, readout }]
            : [];
    }),
);
...
let project: CadenceProject | undefined;
let catalogChoices: readonly CatalogChoice[] = [];
let skills: readonly SkillEntry[] = [];
let selectedSkill: SkillEntry | undefined;
let playbackMs = 0;
let playing = false;
let playbackSpeed = 1;
...
function setStatus(value: string, state: "connected" | "loading" | "error" = "connected"): void {
...
function responseSession(payload: Json): Json {
    const data = record(payload.data);
    return record(data.document ?? data.session ?? data.project ?? data);
}

function responsePreview(payload: Json): Json {
    const data = record(payload.data);
...
function normalizeStage(value: unknown): PreviewStage | undefined {
...
function normalizeTicks(value: unknown): readonly PreviewTick[] {
...
function normalizePreviewObjects(value: unknown): readonly PreviewObject[] {
    return Object.freeze(list(value).flatMap((value): PreviewObject[] => {
...
function normalizeNativePreview(payload: Json): NativePreviewPayload {
...
function normalizeProject(payload: Json): CadenceProject {
    const data = record(payload.data);
    const session = responseSession(payload);
...
function previewProject(value: CadenceProject): PreviewProject {
    return value;
}

function populatePackages(): CatalogChoice | undefined {
...
function populateCharacters(packageId: string): CatalogChoice | undefined {
...
function populateSkills(): SkillEntry | undefined {
...
async function openCharacter(choice: CatalogChoice): Promise<void> {
...
function selectedSkillFromUi(): SkillEntry | undefined {
    return skills.find((skill) => skill.id === skillSelect?.value);
}

function movementSummary(ticks: readonly PreviewTick[]): string {
...
async function replaySelectedSkill(): Promise<void> {
    if (project === undefined) return;
    const skill = selectedSkillFromUi();
    if (skill === undefined) return;
    selectedSkill = skill;
    const scenario: SkillPreviewScenario = buildSkillPreviewScenario(project.frames, skill);
...
function drawEmptyPane(pane: CadencePane, message: string): void {
...
function drawPane(pane: CadencePane): void {
...
function animationFrame(nowMs: number): void {
