...
export type FieldCapability = Json & {
    fieldId: string;
    key: string;
    kind: "number" | "string" | "integer-pair";
    scope: "top" | "sprite" | "frame" | "block";
    occurrence: number;
    value: unknown;
    frameId?: number;
...
export interface FrameStructureCapability {
    capabilityId: string;
    frameId: number;
    occurrence: number;
    canCopy: boolean;
...
export interface BlockSelection { type: "frame" | OverlayType; index?: number; }

export const allOverlayTypes = Object.freeze(Object.keys(OVERLAY_COLORS) as OverlayType[]);
export const frameFieldLabels: Readonly<Record<string, string>> = Object.freeze({
...
export const record = (value: unknown): Json => value !== null && typeof value === "object" ? value as Json : {};
...
export function blockLabel(type: OverlayType): string {
    return type === "itr" || type === "bdy" ? type.toUpperCase() : type;
}

export function parseBlockSelection(value: string): BlockSelection {
    if (value === "frame") return { type: "frame" };
    const [type, rawIndex] = value.split(":");
    return allOverlayTypes.includes(type as OverlayType)
        ? { type: type as OverlayType, index: Number(rawIndex) }
        : { type: "frame" };
}
