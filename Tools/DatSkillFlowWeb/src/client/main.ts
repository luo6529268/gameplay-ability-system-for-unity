...
type TickEntity = PreviewEntity;
type NativeTick = PreviewTick;
...
const playButton = select<HTMLButtonElement>("play-toggle");
...
function normalize(payload: Json): ProjectState {
    ...
    const fieldCapabilities = list(session.fields ?? data.fields).flatMap((value): FieldCapability[] => {
    ...
    const structures = list(session.structureCapabilities ?? data.structureCapabilities).flatMap((value) => {
        ...
        const blocks = list(item.blocks).flatMap((blockValue) => {
            ...
            return blockCapabilityId ? [{
                ...
                blockType: text(block.blockType) as FrameStructureCapability["blocks"][number]["blockType"],
    ...
    return {
        ...
        revision: (session.revision ?? data.revision ?? "-") as string | number,
...
function syncPreviewBusy(busy: boolean): void {
    const panel = select<HTMLElement>("preview-panel");
...
function fieldCapability(frame: Frame, key: string, block: BlockSelection): FieldCapability | undefined {
    ...
    if (block.type === "frame") return findFrameFieldCapability(project?.fields ?? [], frame, key) as FieldCapability | undefined;
...
function frameFieldLocatorKey(frameId: number, occurrence: number, key: string): string {
    return `${frameId}:${occurrence}:${key}`;
}
function currentFrameFieldIndex(): ReadonlyMap<string, FieldCapability> {
...
function existingFrameField(frame: Frame, key: string): FieldCapability | undefined {
...
function setText(id: string, value: string): void {
    const element = select<HTMLElement>(id);
    if (element) element.textContent = value;
}
function reportOperation(operation: Promise<unknown>, fallback: string): void {
...
function currentStructureCapability(): FrameStructureCapability | undefined {
    const frame = currentFrame();
    return project?.structures.find((candidate) => candidate.occurrence === frame?.occurrence);
}
function currentBlockStructureCapability(): FrameStructureCapability["blocks"][number] | undefined {
...
function activeProjectOid(): number {
...
function currentFlow(): SkillFlowGraph | undefined {
    const skill = selectedSkill();
...
function syncFlowEdgeEditor(
    ...
    editableFields = graph === undefined ? new Map<string, string>() : editableFlowFields(graph),
): void {
    const target = select<HTMLSelectElement>("flow-edge-target");
    const apply = select<HTMLButtonElement>("apply-flow-edge");
    const readout = select<HTMLElement>("flow-edge-readout");
...
function renderFlow(): void {
    ...
    const edgesByFrom = new Map<string, SkillFlowEdge[]>();
    ...
    disposeFlowSvg?.();
    const svg = select<SVGSVGElement>("flow-svg");
    if (svg) {
...
function createFieldRow(frame: Frame, block: BlockSelection, key: string, rawValue: unknown): HTMLElement {
...
function createScalarInput(capability: FieldCapability | undefined, key: string, rawValue: unknown): HTMLInputElement {
...
function createPairInputs(capability: FieldCapability, rawValue: unknown): HTMLElement {
    ...
    const inputs = [0, 1].map((index) => {
        ...
        input.addEventListener("input", () => {
            const raw: [string, string] = [inputs[0]!.value, inputs[1]!.value];
            ...
            const value: [number, number] | undefined = valid ? [inputs[0]!.valueAsNumber, inputs[1]!.valueAsNumber] : undefined;
...
function renderFields(): void {
    ...
    for (const group of groups) {
        ...
        for (const key of group.keys) {
            const rawValue = selectedBlock.type === "frame" ? frame[key as keyof Frame] : blockEntries(frame, selectedBlock).find(([candidate]) => candidate === key)?.[1];
...
async function applyBatchEdits(
    edits: readonly { fieldId: string; value: number | string | readonly [number, number] }[],
...
async function applyStructureEdit(
    operation: "copy-frame" | "delete-frame" | "create-block" | "copy-block" | "delete-block",
): Promise<void> {
    const frame = currentFrame();
    ...
    await runExclusiveAction("edit", async () => {
        ...
        if (operation === "copy-frame") {
            ...
            if (selectedFrame && oldBlock.type !== "frame") {
                const collection = selectedFrame[blockCollections[oldBlock.type]] as unknown;
...
select("apply-session")?.addEventListener("click", () => select<HTMLFormElement>("frame-editor")?.requestSubmit());
...
function canvasPoint(event: PointerEvent | KeyboardEvent): { x: number; y: number } {
...
function canvasCapabilities(hit: OverlayGeometry): Readonly<Record<string, FieldCapability>> {
    const frame = currentFrame();
    if (!frame) return {};
    const block: BlockSelection = { type: hit.type, index: hit.index };
...
function canvasBlockValues(hit: OverlayGeometry): { x: number; y: number; w?: number; h?: number } {
    const frame = currentFrame();
    if (!frame) return { x: 0, y: 0 };
    const values = frame[blockCollections[hit.type]] as unknown;
...
function finishCanvasInteraction(pointerId = canvasInteraction?.pointerId): void {
    canvasInteraction = undefined;
...
canvas?.addEventListener("pointermove", (event) => {
    ...
    if (!interaction || interaction.pointerId !== event.pointerId) return;
    const point = canvasPoint(event);
    const grid: 1 | 4 = select<HTMLInputElement>("grid-four")?.checked ? 4 : 1;
...
canvas?.addEventListener("pointerup", (event) => {
    ...
    const next = interaction.handle && interaction.values.w !== undefined && interaction.values.h !== undefined
        ? resizeDatRect(interaction.values as { x: number; y: number; w: number; h: number }, interaction.handle, dx, dy, interaction.mirror)
...
document.querySelectorAll<HTMLButtonElement>("[data-tab-target]").forEach((button) => button.addEventListener("click", () => {
    document.querySelectorAll<HTMLElement>("[data-mobile-panel]").forEach((panel) => panel.classList.toggle("is-mobile-active", panel.id === button.dataset.tabTarget));
    document.querySelectorAll<HTMLButtonElement>("[data-tab-target]").forEach((candidate) => candidate.ariaPressed = String(candidate === button));
}));
