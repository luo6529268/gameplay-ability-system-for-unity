...
export interface FlowSvgOptions {
    readonly editableFieldIds: ReadonlyMap<string, string>;
...
function center(position: FlowNodePosition): { x: number; y: number } {
    return {
...
function positionMap(layout: ReturnType<typeof layoutSkillFlow>): ReadonlyMap<string, FlowNodePosition> {
    return new Map(layout.nodes.map((position) => [position.node.id, position]));
}

function edgePath(from: FlowNodePosition, to: FlowNodePosition): string {
...
function appendTitle(parent: SVGElement, value: string): void {
