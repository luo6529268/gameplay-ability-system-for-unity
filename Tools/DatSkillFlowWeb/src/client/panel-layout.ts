...
export interface PanelLayout extends PanelWidths {
    ...
    readonly middleMinimum: number;
    readonly leftMinimum: number;
    readonly leftMaximum: number;
    readonly rightMinimum: number;
    readonly rightMaximum: number;
}
...
export const COMPACT_DEFAULT_PANEL_WIDTHS = { left: 230, right: 286 } as const;
export const WIDE_DEFAULT_PANEL_WIDTHS = { left: 286, right: 330 } as const;
...
export function defaultPanelWidths(containerWidth: number): PanelWidths {
    return containerWidth <= COMPACT_PANEL_MAXIMUM
        ? COMPACT_DEFAULT_PANEL_WIDTHS
        : WIDE_DEFAULT_PANEL_WIDTHS;
}

export function middleMinimumForWidth(containerWidth: number): number {
    return containerWidth <= COMPACT_PANEL_MAXIMUM
