...
namespace NTSD.Test
{
    public sealed class NTSD28Q06DamageSpecialKindsEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-DAMAGE-REMAINING-SPECIAL-KINDS-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 2, 4, 7, 10, 16 };
        ...
        public void ImpactAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        [TestCase(-1)]
        [TestCase(4)]
        public void CandidateRepresentativesMatchSource(int isolatedIndex)
        {
            var differences = new List<string>();
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .Where(row => isolatedIndex < 0 ? RepresentativeIndices.Contains((int)row["index"]) : (int)row["index"] == isolatedIndex))
            {
                var world = CreateWorld(row, BattleRuntimeProfile.Authority400,
                    BattleEcsCharacterFrameAdvancePassMode.DataOriented, false);
                try
                {
                    string label = "candidate " + row["index"];
                    Compare(world, row["before"], label + " before", differences);
                    ...
                    if (range.Count != 1)
                    {
                        ...
                        File.WriteAllText(Output + "candidate-missing-" + row["index"] + ".json", JsonConvert.SerializeObject(new
                        {
                            count = range.Count, differences, capture = Capture(world),
                            pair = BattleHitCandidatePairSnapshotFactory.Capture(attacker, target, world),
                            targetCount = target.Runtime.HitCandidateCount,
                            attackerCount = attacker.Runtime.HitCandidateCount,
                            attackerNearest = attacker.Runtime.HitCandidateNearestDistance,
                            zDelta = target.Runtime.ZInt - attacker.Runtime.ZInt,
                            attackerSuppressed = attacker.Runtime.SuppressCollisionCandidateUntilTick,
                            targetSuppressed = target.Runtime.SuppressCollisionCandidateUntilTick,
                            attackerFrame = attacker.Frame.N, attackerSnapshot = attacker.Frame.Prev2,
                            targetFrame = target.Frame.N, targetSnapshot = target.Frame.Prev2,
                            pairAllowed = typeof(BruteForceSceneQuery).GetMethod("CandidateCollectionPairAllowed", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[] { attacker, target }),
                            carrier = typeof(BruteForceSceneQuery).GetMethod("IsCandidateAttackerCarrierForCurrentTick", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(query, new object[] { attacker }),
                            coarse = typeof(BruteForceSceneQuery).GetMethod("PassesReleaseCoarsePrefilter", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(query, new object[] { attacker, attacker.Frame.D, attacker.GetCollisionFrameData(), target, target.Frame.D, target.GetCollisionFrameData() }),
                            itrAllowed = typeof(BruteForceSceneQuery).GetMethod("ItrAllowedForFormalCollection", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[] { attacker, attacker.Frame.D, attacker.GetCollisionFrameData(), attacker.GetCollisionFrameData().itrs[0], target, target.Frame.D }),
                            itrs = attacker.GetCollisionFrameData()?.itrs,
                            bodies = target.GetCollisionFrameData()?.bodies
                        }, Formatting.Indented));
                    }
                    Assert.That(range.Count, Is.EqualTo(1), label);
            ...
            File.WriteAllText(Output + "candidate.json", JsonConvert.SerializeObject(new { cases, differences }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(isolatedIndex < 0 ? RepresentativeIndices.Length : 1));
            Assert.That(differences, Is.Empty, string.Join("\n", differences.Take(12)));
        }

        [TestCase(250, 1)]
        [TestCase(1100, 0)]
        public void ExplicitType3OffsetCollisionUsesLogicZ(int targetZ, int expectedCandidates)
        {
            var row = File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .First(value => (int)value["index"] == 4);
            var world = CreateWorld(row, BattleRuntimeProfile.Authority400,
                BattleEcsCharacterFrameAdvancePassMode.DataOriented, false);
            try
            {
                var attacker = world.FindEntityByRuntimeSlotForQuery(0);
                var target = world.FindEntityByRuntimeSlotForQuery(70);
                target.Runtime.SetPosition(target.Runtime.X, target.Runtime.Y, targetZ);
                target.Runtime.SyncIntegerPosition();
                target.Runtime.Type3VisualZOffset = 850;
                var query = (BruteForceSceneQuery)world.SceneQuery;
                query.FormalCollectorMode = CollisionFormalCollectorMode.ForceBruteForce;
                world.CaptureCollisionFrameSnapshotsAll();
                world.CollectCollisionCandidatesAll();
                Assert.That(query.TryGetCollisionCandidateRange(attacker, out CollisionCandidateRange range), Is.True);
                Assert.That(range.Count, Is.EqualTo(expectedCandidates), "rawZ=" + targetZ);
            }
            finally { Shutdown(world, false); }
        }
        ...
        [TestCase(120, 120)]
        [TestCase(300, 201)]
        public void Type3StageBoundsUsePreciseLogicZ(int inputZ, int expectedZ)
        {
            var row = File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).First();
            var world = CreateWorld(row, BattleRuntimeProfile.Authority400,
                BattleEcsCharacterFrameAdvancePassMode.DataOriented, false);
            try
            {
                var target = world.FindEntityByRuntimeSlotForQuery(70);
                target.Runtime.SetPosition(target.Runtime.X, target.Runtime.Y, inputZ);
                target.Runtime.SyncIntegerPosition();
                target.Runtime.Type3VisualZOffset = 30;
                target.ApplyPreFrameZBounds(100f, 200f);
                Assert.That(target.Runtime.Z, Is.EqualTo(expectedZ), "precise Z");
                Assert.That(target.Runtime.ZInt, Is.EqualTo(expectedZ), "integer Z");
            }
            finally { Shutdown(world, false); }
        }
        ...
        [TestCase(206, 6)]
        [TestCase(220, 10)]
        public void Type3ConsumersUseStoredIntegerZ(int inputZ, int offset)
        {
            var row = File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .First(value => (int)value["index"] == 4);
            var world = CreateWorld(row, BattleRuntimeProfile.Authority400,
            ...
            try
            {
                ...
                target.Runtime.SetPosition(target.Runtime.X, target.Runtime.Y, inputZ);
                target.Runtime.SyncIntegerPosition();
                target.Runtime.Type3VisualZOffset = offset;
                var entityLogic = typeof(LF2Entity).GetMethod("GetFrameLogicZInt", BindingFlags.Static | BindingFlags.NonPublic);
                var weaponLogic = typeof(LF2WeaponFrameLogicResolver).GetMethod("GetFrameLogicZInt", BindingFlags.Static | BindingFlags.NonPublic);
                Assert.That(entityLogic, Is.Not.Null);
                Assert.That(weaponLogic, Is.Not.Null);
                int displayZ = (int)target.GetDisplayZ();
                int entityZ = (int)entityLogic.Invoke(null, new object[] { target });
                int weaponZ = (int)weaponLogic.Invoke(null, new object[] { target });
                Assert.That(new[] { displayZ, entityZ, weaponZ }, Is.EqualTo(new[] { inputZ, inputZ, inputZ }),
                    "sprite/entity/weapon Z consumers");
            }
            finally { Shutdown(world, false); }
        }

        [Test]
        public void State1002ThrowingTailMatchesSource()
        {
            var differences = new List<string>();
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "throw-source/first.jsonl").Select(JObject.Parse))
            {
                var world = CreateWorld(row, BattleRuntimeProfile.Authority400,
                    BattleEcsCharacterFrameAdvancePassMode.DataOriented, false);
                try
                {
                    string label = "state1002 " + row["index"];
                    Compare(world, row["before"], label + " before", differences);
                    ...
                    bool applied = world.DamageWriter.TryApplyCurrentDatTargetHit(
                        world, attacker, target, attacker.Frame.D.itrs[0], default);
                    if (!applied || (int)row["status"] != 0)
                        differences.Add(label + " apply status differs");
                    Compare(world, row["after"], label + " impact", differences);
                    CompareJson(row["calls"], observer.Capture(), label + " calls", differences);
                    if (world.Rng.CallCount != legacyCalls)
                        differences.Add(label + " legacy RNG consumed");
                    ...
                    CompareJson(row["followingCalls"], observer.Capture(), label + " following calls", differences);
                    cases++;
                }
                finally { Shutdown(world, false); }
            }
            File.WriteAllText(Output + "throw-unity-red.json", JsonConvert.SerializeObject(
                new { cases, differences }, Formatting.Indented));
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    var attacker = world.FindEntityByRuntimeSlotForQuery(0);
                    var target = world.FindEntityByRuntimeSlotForQuery(70);
                    bool applied = world.DamageWriter.TryApplyCurrentDatTargetHit(world, attacker, target, attacker.Frame.D.itrs[0], default);
                    if (applied != (bool)row["applied"]) immediateDifferences.Add(label + " applied differs");
                    Compare(world, row["after"], label + " impact", immediateDifferences);
            ...
            File.WriteAllText(Output + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual impact transaction and following full tick; native raw50 mapped fields plus descriptor/B2/RNG. Source previousXYZ have no asserted Unity carrier; not platform or image parity."
            }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 17 : 6));
        ...
        internal static void RunThrowMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            var differences = new List<string>();
            int cases = 0;
            int borrowers = renderer ? LF2ObjectPool.Instance.ActiveObjectCountForAcceptance : 0;
            foreach (var row in File.ReadLines(Output + "throw-source/first.jsonl").Select(JObject.Parse)
                .Where(value => (int)value["index"] != 1))
            {
                var world = CreateWorld(row, profile, mode, renderer);
                try
                {
                    string label = "throw " + profile + "/" + row["index"];
                    Compare(world, row["before"], label + " before", differences);
                    var observer = new Observer();
                    world.NativeRandom.SetDiagnosticCallObserver(observer);
                    var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            ...
            File.WriteAllText(Output + "throw-" + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json",
                JsonConvert.SerializeObject(new { cases, differences }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(3));
            Assert.That(differences, Is.Empty, string.Join("\n", differences.Take(16)));
        }

        [TestCase(BattleRuntimeProfile.Authority400, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        [TestCase(BattleRuntimeProfile.MobileExtended, BattleEcsCharacterFrameAdvancePassMode.Legacy)]
        public void ImpactRepresentativesSurviveLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).Where(row => RepresentativeIndices.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    string[] expected = ExecuteImpactAndTwoTicks(world);
                    ...
                    Assert.That(ExecuteImpactAndTwoTicks(world), Is.EqualTo(expected), "replay case " + row["index"]);
                    cases++;
                }
                finally { Shutdown(world, false); }
            }
            ...
            File.WriteAllText(Output + "representative-replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Same-world impact physics transaction plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }

        private static string[] ExecuteImpactAndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var target = world.FindEntityByRuntimeSlotForQuery(70);
            world.DamageWriter.TryApplyCurrentDatTargetHit(world, attacker, target, attacker.Frame.D.itrs[0], default);
        ...
        private static JObject Capture(SimulationWorld world)
        {
            ...
            foreach (int slot in new[] { 0, 1, 2, 70 })
            {
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                int targetType = (int)row["params"]["type"];
                int targetOid = (int)row["params"]["oid"];
                int attackerType = row["params"]["attackerType"]?.Value<int>() ?? 0;
                int attackerInitialAction = attackerType == (int)LF2ObjectType.ThrowWeapon ? 20 : 10;
                var definitions = new[] { Definition(77, (string)row["attackerDat"]), Definition(targetOid, (string)row["targetDat"]), Definition(79, (string)row["ownerDat"]) };
                world.PrepareRuntimeDataCatalogForBattle(new[]
                {
                    new ObjectDefinition(77, attackerType, "impact-attacker.dat"), new ObjectDefinition(targetOid, targetType, "impact-target.dat"),
                    new ObjectDefinition(79, 0, "impact-owner.dat")
                }, id => id == 77 ? definitions[0] : id == targetOid ? definitions[1] : id == 79 ? definitions[2] : null);
                int[] slots = { 0, 1, 2, 70 };
                var entities = new LF2Entity[slots.Length];
                for (int i = 0; i < slots.Length; i++)
                {
                    int oid = slots[i] == 0 ? 77 : slots[i] == 70 ? targetOid : 79;
                    var task = new OPointCreateTask
                    {
                        ...
                        opoint = new ObjectPoint { oid = oid, action = slots[i] == 0 ? attackerInitialAction : 10 }
                    };
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06DamageSpecialKindsPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_DamageSpecialKindsPlay.request";
        private const string Result = "Temp/NTSD28_Q06_DamageSpecialKindsPlay.result.json";
        static NTSD28Q06DamageSpecialKindsPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06DamageSpecialKindsEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 6;
                    NTSD28Q06DamageSpecialKindsEditorTests.RunThrowMatrix(profile, mode, renderer);
                    passedCases += 3;
                }
            ...
            string status = errors.Count == 0 && passedCases == 18 && unchanged && borrowersAfter == borrowers ? "PASS" : "FAIL";
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; special-kind6 plus state1002 throw3 representatives including missing selected frame on Authority/DataOriented/renderer and Mobile/Legacy/logic. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
