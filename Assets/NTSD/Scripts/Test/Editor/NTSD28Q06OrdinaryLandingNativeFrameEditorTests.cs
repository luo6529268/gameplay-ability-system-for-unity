...
namespace NTSD.Test
{
    public sealed class NTSD28Q06OrdinaryLandingNativeFrameEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-ORDINARY-LANDING-NATIVE-FRAME-BINDING-001/";
        ...
        public void OrdinaryLandingAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            Assert.That(cases, Is.EqualTo(186));
        ...
        [TestCase(BattleRuntimeProfile.MobileExtended, BattleEcsCharacterFrameAdvancePassMode.Legacy)]
        [TestCase(BattleRuntimeProfile.MobileExtended, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        public void OrdinaryLandingSurvivesLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse))
            {
            ...
            Assert.That(cases, Is.EqualTo(186));
            File.WriteAllText(Output + "replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                cases, replayedTicks = cases * 2,
                scope = "Same-world ordinary landing physics transaction plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06OrdinaryLandingPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_OrdinaryLandingPlay.request";
        private const string Result = "Temp/NTSD28_Q06_OrdinaryLandingPlay.result.json";
        static NTSD28Q06OrdinaryLandingPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
                ...
                foreach (bool renderer in new[] { false, true })
                {
                    try
                    {
                        NTSD28Q06OrdinaryLandingNativeFrameEditorTests.RunMatrix(profile, mode, renderer);
                        passedCases += 186;
                    }
            ...
            string status = errors.Count == 0 && passedCases == 1488 && unchanged && borrowersAfter == borrowers ? "PASS" : "FAIL";
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; synthetic source186 in both profiles/physics paths/factories. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
