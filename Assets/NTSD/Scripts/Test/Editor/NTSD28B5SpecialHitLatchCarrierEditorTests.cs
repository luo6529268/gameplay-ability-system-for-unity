...
{
    public sealed class NTSD28B5SpecialHitLatchCarrierEditorTests
    {
        [Test]
        public void Lifecycle_DefaultFalse_InputResetPreserves_FullResetClears()
        {
            var runtime = new NTSDEntityRuntime();
            Assert.That(runtime.SpecialHitLatch0EB, Is.False);

            runtime.SpecialHitLatch0EB = true;
            runtime.ResetInputState();
            Assert.That(runtime.SpecialHitLatch0EB, Is.True);
        ...
        [Test]
        public void CanonicalCopy_PreservesLatchWithoutAliasingHitConfirm2()
        {
            var source = new NTSDEntityRuntime
            {
                SpecialHitLatch0EB = true,
                HitConfirm2 = 7,
            };
            ...
            Assert.That(destination.HitConfirm2, Is.EqualTo(7));

            source.SpecialHitLatch0EB = false;
            Assert.That(destination.SpecialHitLatch0EB, Is.True);
        }

        [Test]
        public void EntityRuntimeSnapshot_RoundTripsLatch()
        {
        ...
        [Test]
        public void ChecksumAndParity_TrackLatch()
        {
            SimulationWorld world = World(out LF2Character entity);
        ...
        {
            var source = new NTSDEntityRuntime { SpecialHitLatch0EB = true };
            var destination = new NTSDEntityRuntime();
            SimulationWorld world = World(out LF2Character entity);
            entity.Runtime.SpecialHitLatch0EB = true;
            ...
            Assert.That(BattleWorldEntityRuntimeSnapshotBuffer.CurrentSchemaVersion, Is.EqualTo(15));
