...
{
    [InitializeOnLoad]
    internal static class NTSD28Q06PlatformMotionPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_PlatformMotionPlay.request";
        private const string Result = "Temp/NTSD28_Q06_PlatformMotionPlay.result.json";

        static NTSD28Q06PlatformMotionPlayProbe()
        {
        ...
        private static void Poll()
        {
            ...
            int passed = 0;
            foreach (int index in new[] { 0, 20, 107, 109 })
            {
                try
                {
                    if (index >= 100) NTSD28Q06FrameMotionTailEditorTests.VerifyRendererForPlay(index - 100);
                    else NTSD28Q06PlatformTransactionEditorTests.VerifyRendererForPlay(index);
                    passed++;
                }
            ...
            {
                status = errors.Count == 0 && passed == 4 && unchanged && borrowers == after ? "PASS" : "FAIL",
                passedCases = passed, errors, sceneChecksumUnchanged = unchanged,
                rendererBorrowersBefore = borrowers, rendererBorrowersAfter = after,
                scope = "Actual pooled Renderer platform candidate/motion/physics and frame-motion tail source representatives; logic values only, not sprite/shadow visual parity."
            }, Formatting.Indented));
