...
namespace NTSD.Test
{
    public sealed class NTSD28Q06ImpactNativeFrameBindingEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-IMPACT-NATIVE-FRAME-BINDING-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 2, 4, 12, 16, 18 };
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    bool applied = world.DamageWriter.TryApplyNativeImpact(world, attacker, target, attacker.Frame.D.itrs[0]);
            ...
            File.WriteAllText(Output + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual impact transaction and following full tick; native raw47 mapped fields plus descriptor/B2/RNG. Source previousXYZ have no asserted Unity carrier; not platform or image parity."
            }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 20 : 6));
        ...
        private static string[] ExecuteImpactAndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var target = world.FindEntityByRuntimeSlotForQuery(70);
            world.DamageWriter.TryApplyNativeImpact(world, attacker, target, attacker.Frame.D.itrs[0]);
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                var definitions = new[] { Definition(77, (string)row["attackerDat"]), Definition(78, (string)row["targetDat"]), Definition(79, (string)row["ownerDat"]) };
                world.PrepareRuntimeDataCatalogForBattle(new[]
                {
                    new ObjectDefinition(77, 0, "impact-attacker.dat"), new ObjectDefinition(78, targetType, "impact-target.dat"),
                    new ObjectDefinition(79, 0, "impact-owner.dat")
                }, id => id == 77 ? definitions[0] : id == 78 ? definitions[1] : id == 79 ? definitions[2] : null);
                int[] slots = { 0, 1, 2, 70 };
                var entities = new LF2Entity[slots.Length];
                for (int i = 0; i < slots.Length; i++)
                {
                    int oid = slots[i] == 0 ? 77 : slots[i] == 70 ? 78 : 79;
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06ImpactPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_ImpactPlay.request";
        private const string Result = "Temp/NTSD28_Q06_ImpactPlay.result.json";
        static NTSD28Q06ImpactPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06ImpactNativeFrameBindingEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 6;
                }
            ...
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; impact6 representatives on Authority/DataOriented/renderer and Mobile/Legacy/logic. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
