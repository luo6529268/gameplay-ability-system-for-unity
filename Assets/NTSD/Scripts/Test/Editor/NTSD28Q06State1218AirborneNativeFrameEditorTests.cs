...
namespace NTSD.Test
{
    public sealed class NTSD28Q06State1218AirborneNativeFrameEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-STATE1218-AIRBORNE-NATIVE-FRAME-BINDING-001/";
        ...
        public void State1218AirborneAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            int[] smoke = { 0, 4, 8, 15, 19, 32, 36, 44 };
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    Assert.That(world.NativeResourcePhase12, Is.EqualTo((int)row["config"]["resourcePhase12"]));
            ...
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 55 : 8));
        ...
        [TestCase(BattleRuntimeProfile.Authority400, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        [TestCase(BattleRuntimeProfile.MobileExtended, BattleEcsCharacterFrameAdvancePassMode.Legacy)]
        public void State1218AirborneRepresentativesSurviveLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).Where(row => new[] { 0, 4, 8, 15, 19, 32, 36, 44 }.Contains((int)row["index"])))
            {
            ...
            File.WriteAllText(Output + "representative-replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Same-world state12/18 airborne physics transaction plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                world.Runtime.NativeWorldClock.ResourcePhase12 = (int)row["config"]["resourcePhase12"];
                world.Runtime.NativeWorldClock.ResourcePhase3 = (int)row["config"]["resourcePhase3"];
