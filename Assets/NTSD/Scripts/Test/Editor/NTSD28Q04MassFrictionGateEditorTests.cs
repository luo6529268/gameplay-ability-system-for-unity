...
namespace NTSD.Test.Editor
{
    ...
    {
        [Test]
        public void CoreMatchesNativeWithCurrentContext()
        {
            ...
            foreach (string row in rows)
            {
                ...
                var context = new CharacterMechanicsContext(runtime, null, 0, 0, 1.7);
        ...
        [TestCase(100, true)]
        [TestCase(150, true)]
        public void RealCharacterAndEcsUseSameGroundedRule(int objectId, bool ecs)
        {
            var world = new SimulationWorld();
            LF2Character character = CreateCharacter(world, 0, objectId);
            try
            {
                ...
                Assert.That(character.ObjectId, Is.EqualTo(objectId));
            }
            finally { world.Unregister(character); }
        }

        internal static LF2Character CreateCharacter(SimulationWorld world, int slot, int objectId = 0)
        {
            ...
            var character = new LF2Character { Name = data.name, ObjectId = objectId };
