...
namespace NTSD.Test
{
    public sealed class NTSD28Q06FirstBdyNativeFrameBindingEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-FIRST-BDY-NATIVE-FRAME-BINDING-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 3, 4, 6 };
        ...
        public void FirstBodyAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer, bool allRepresentatives = false)
        {
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    Assert.That(query.TryGetCollisionCandidateRange(attacker, out CollisionCandidateRange range), Is.True, label);
                    Assert.That(range.Count, Is.EqualTo(1), label);
                    int capturedCandidateCount = range.Count;
                    world.PostInteractionTickAll(0);
                    var diagnostics = world.BattleHitExecutionPlanDiagnosticsForDiagnostics;
                    Compare(world, row["after"], label + " first body", immediateDifferences);
                    ...
                    actual.Add(new { index = (int)row["index"], candidateCount = capturedCandidateCount, firstBodyAttempts = diagnostics.ObservedFirstBodyResponseAttemptCount, diagnostics, before, after, following = Capture(world) });
                    cases++;
                }
                finally
                {
            ...
            File.WriteAllText(Output + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual shared first-body candidate response plus following full tick; native raw47 mapped fields plus descriptor/B2/RNG. Source previousXYZ have no asserted Unity carrier; not platform or image parity."
            }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(allRepresentatives || profile == BattleRuntimeProfile.Authority400 && !renderer ? 8 : 4));
        ...
        [Test]
        public void FirstBodyRepresentativesSurviveLocalSnapshotReplay()
        {
            int cases = 0;
            var results = new List<object>();
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .Where(row => RepresentativeIndices.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    string[] expected = ExecuteFirstBodyAndTwoTicks(world);
                    Assert.That(world.TryRestoreBattleStateSnapshot(identity, snapshot, out var failure), Is.True,
                        "case " + row["index"] + " restore: " + failure);
                    ...
                    string[] replayed = ExecuteFirstBodyAndTwoTicks(world);
                    Assert.That(replayed, Is.EqualTo(expected), "replay case " + row["index"]);
                    results.Add(new { index = (int)row["index"], expected, replayed });
                    cases++;
                }
                catch (Exception exception)
                {
                    File.WriteAllText(Output + "representative-replay-failure.json", JsonConvert.SerializeObject(new
                    { index = (int)row["index"], cases, error = exception.ToString(), results }, Formatting.Indented));
                    throw;
                }
                finally { Shutdown(world, false); }
            }
            Assert.That(cases, Is.EqualTo(4));
            File.WriteAllText(Output + "representative-replay.json", JsonConvert.SerializeObject(new
            { cases, replayedTicks = cases * 2, results }, Formatting.Indented));
        }

        private static string[] ExecuteFirstBodyAndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var query = (BruteForceSceneQuery)world.SceneQuery;
            query.FormalCollectorMode = CollisionFormalCollectorMode.ForceBruteForce;
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                var definitions = new[] { Definition(31980, (string)row["attackerDat"]), Definition(31981, (string)row["targetDat"]) };
                world.PrepareRuntimeDataCatalogForBattle(new[]
                {
                    new ObjectDefinition(31980, 0, "firstbody-attacker.dat"), new ObjectDefinition(31981, 0, "firstbody-target.dat")
                }, id => id == 31980 ? definitions[0] : id == 31981 ? definitions[1] : null);
                int[] slots = { 0, 70 };
                var entities = new LF2Entity[slots.Length];
                for (int i = 0; i < slots.Length; i++)
                {
                    int oid = slots[i] == 0 ? 31980 : 31981;
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06FirstBdyPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_FirstBdyPlay.request";
        private const string Result = "Temp/NTSD28_Q06_FirstBdyPlay.result.json";
        static NTSD28Q06FirstBdyPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06FirstBdyNativeFrameBindingEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 4;
                }
            ...
            string status = errors.Count == 0 && passedCases == 8 && unchanged && borrowersAfter == borrowers ? "PASS" : "FAIL";
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; first-body4 on Authority/DataOriented/renderer and Mobile/Legacy/logic. No physical-key or pixel parity claim; separate Q05 shutdown result required."
            }, Formatting.Indented));
