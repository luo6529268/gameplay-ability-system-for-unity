...
namespace NTSD.Test
{
    public sealed class NTSD28Q06EffectOverrideNativeFrameAccessEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-EFFECT-OVERRIDE-NATIVE-FRAME-ACCESS-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 5, 10, 11, 12, 15 };
        ...
        public void EffectOverrideAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 16 : 6));
        ...
        [TestCase(BattleRuntimeProfile.Authority400, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        public void EffectRepresentativesSurviveLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).Where(row => RepresentativeIndices.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    string[] expected = ExecuteEffectAndTwoTicks(world);
                    ...
                    Assert.That(ExecuteEffectAndTwoTicks(world), Is.EqualTo(expected), "replay case " + row["index"]);
                    cases++;
                }
            ...
            File.WriteAllText(Output + "representative-replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Same-world standard damage and effect override transaction plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }

        private static string[] ExecuteEffectAndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var target = world.FindEntityByRuntimeSlotForQuery(70);
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06EffectPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_EffectPlay.request";
        private const string Result = "Temp/NTSD28_Q06_EffectPlay.result.json";
        static NTSD28Q06EffectPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06EffectOverrideNativeFrameAccessEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 6;
                    var hitPlanMode = renderer ? BattleHitExecutionPlanMode.ShadowCompare : BattleHitExecutionPlanMode.DataOriented;
                    NTSD28Q06StandardHitFall80PreservationEditorTests.RunCapturedHitPlan(hitPlanMode,
                        NTSD28Q06StandardHitFall80PreservationEditorTests.Output + "source/first.jsonl", renderer);
                    NTSD28Q06StandardHitFall80PreservationEditorTests.RunCapturedHitPlan(hitPlanMode,
                        "artifacts/diagnostics/NTSD28-Q06-STANDARD-HIT-PENDING-Y-PROJECTION-001/source/first.jsonl", renderer);
                    passedCases += 5;
                }
            ...
            string status = errors.Count == 0 && passedCases == 22 && unchanged && borrowersAfter == borrowers ? "PASS" : "FAIL";
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; Effect6 representatives on Authority/DataOriented/renderer and Mobile/Legacy/logic; standard3 and vertical2 captured candidate cases on Shadow/renderer and DataOriented/logic. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
