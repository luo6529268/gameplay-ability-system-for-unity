...
{
    [InitializeOnLoad]
    internal static class NTSD28Q06CpointSettlementRemainderPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_CpointSettlementRemainderPlay.request";
        private const string Result = "Temp/NTSD28_Q06_CpointSettlementRemainderPlay.result.json";

        static NTSD28Q06CpointSettlementRemainderPlayProbe()
        {
        ...
        private static void Poll()
        {
            ...
            int passed = 0;
            foreach (int index in new[] { 5, 9, 22 })
            {
                try
                {
                    NTSD28Q06CpointSettlementRemainderEditorTests.VerifyRendererForPlay(index);
                    passed++;
                }
            ...
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual pooled Renderer CPoint settlement/release and following tick; logic values, not physical key or visual parity."
            }, Formatting.Indented));
