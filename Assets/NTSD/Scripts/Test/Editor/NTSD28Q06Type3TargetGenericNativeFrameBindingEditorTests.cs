...
namespace NTSD.Test
{
    public sealed class NTSD28Q06Type3TargetGenericNativeFrameBindingEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-TYPE3-TARGET-GENERIC-NATIVE-FRAME-BINDING-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 2, 4, 8, 9 };
        ...
        public void Type3TargetAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        public void CapturedGenericAndPairMatchSource(BattleHitExecutionPlanMode mode)
        {
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .Where(row => (int)row["index"] == 0 || (int)row["index"] == 9))
            {
                ...
                try
                {
                    ...
                    if ((int)row["params"]["attackerType"] == 0) world.PostInteractionTickAll(0);
                    else world.ObjectInteractionTickAll(0);
                    ...
                    if (mode == BattleHitExecutionPlanMode.ShadowCompare && (int)row["index"] == 0)
                        Assert.That(diagnostics.ObservedWriterEffectCount, Is.GreaterThan(0));
                    for (int i = 0; i < 2; i++)
                    {
                        var entity = world.FindEntityByRuntimeSlotForQuery(i == 0 ? 0 : 70);
                        var expected = row["after"]["entities"][i];
                        Assert.That(entity.Frame.N, Is.EqualTo((int)expected["raw"]["frame"]["action"]));
                        Assert.That(entity.AttackingCounter, Is.EqualTo((int)expected["raw"]["frame"]["frameCounter"]));
                        Assert.That(entity.HitCount, Is.EqualTo((int)expected["pendingCount"]));
                        Assert.That(entity.KnockbackVx, Is.EqualTo((double)expected["pendingX"]));
                        Assert.That(entity.KnockbackVy, Is.EqualTo((double)expected["pendingY"]));
                        Assert.That(entity.KnockbackVz, Is.EqualTo((double)expected["pendingZ"]));
                        Assert.That(entity.Runtime.HP, Is.EqualTo((int)expected["raw"]["vitals"]["currentHp"]));
                    }
                }
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            File.WriteAllText(Output + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual generic Type3 target response, post-selection pair reset and following full tick; native raw47 mapped fields plus descriptor/B2/RNG. Source previousXYZ have no asserted Unity carrier; not platform or image parity."
            }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 11 : 5));
        ...
        [TestCase(BattleRuntimeProfile.Authority400, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        public void Type3TargetRepresentativesSurviveLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).Where(row => RepresentativeIndices.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    string[] expected = ExecuteType3TargetAndTwoTicks(world);
                    ...
                    Assert.That(ExecuteType3TargetAndTwoTicks(world), Is.EqualTo(expected), "replay case " + row["index"]);
                    cases++;
                }
                finally { Shutdown(world, false); }
            }
            ...
            File.WriteAllText(Output + "representative-replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Same-world generic Type3 target response and post-selection pair transaction plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }

        private static string[] ExecuteType3TargetAndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var target = world.FindEntityByRuntimeSlotForQuery(70);
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                {
                    new ObjectDefinition(77, (int)row["params"]["attackerType"], "generic-attacker.dat"), new ObjectDefinition(78, 3, "generic-target.dat")
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06Type3TargetPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_Type3TargetPlay.request";
        private const string Result = "Temp/NTSD28_Q06_Type3TargetPlay.result.json";
        static NTSD28Q06Type3TargetPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06Type3TargetGenericNativeFrameBindingEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 5;
                }
            ...
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; Type3 target5 generic/pair representatives on Authority/DataOriented/renderer and Mobile/Legacy/logic. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
