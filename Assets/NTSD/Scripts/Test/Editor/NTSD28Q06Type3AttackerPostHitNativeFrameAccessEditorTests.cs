...
namespace NTSD.Test
{
    public sealed class NTSD28Q06Type3AttackerPostHitNativeFrameAccessEditorTests
    {
        internal const string Output = "artifacts/diagnostics/NTSD28-Q06-TYPE3-ATTACKER-POSTHIT-NATIVE-FRAME-ACCESS-001/";
        ...
        private static readonly int[] RepresentativeIndices = { 0, 1, 4, 9, 11, 12 };
        ...
        public void Type3PostHitAndFollowingTickMatchSource(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
        ...
        public void CapturedThreeRoutesMatchSource(BattleHitExecutionPlanMode mode)
        {
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse)
                .Where(row => new[] { 1, 11, 12 }.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    if (mode == BattleHitExecutionPlanMode.ShadowCompare && (int)row["params"]["targetType"] == 0)
                        Assert.That(diagnostics.ObservedWriterEffectCount, Is.GreaterThan(0), "case " + row["index"]);
                    var expected = row["after"]["entities"][0];
                    Assert.That(attacker.Frame.N, Is.EqualTo((int)expected["raw"]["frame"]["action"]));
                    Assert.That(attacker.AttackingCounter, Is.EqualTo((int)expected["raw"]["frame"]["frameCounter"]));
                    Assert.That(attacker.Frame.D != null, Is.EqualTo((bool)expected["available"]));
                    Assert.That(attacker.Frame.D?.wait ?? 0, Is.EqualTo((int)expected["wait"]));
                    Assert.That(attacker.Runtime.Vx, Is.EqualTo((double)expected["raw"]["motion"]["x"]));
                    Assert.That(attacker.Runtime.Vz, Is.EqualTo((double)expected["raw"]["motion"]["z"]));
        ...
        internal static void RunMatrix(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            foreach (var row in rows)
            {
                ...
                try
                {
                    ...
                    var attacker = world.FindEntityByRuntimeSlotForQuery(0);
                    var target = world.FindEntityByRuntimeSlotForQuery(70);
                    bool applied = (int)row["params"]["targetType"] == 3
                        ? world.DamageWriter.ApplySpecialAttackDamage(world, attacker, target, attacker.Frame.D.itrs[0])
                        : world.DamageWriter.ApplyStandardCharacterDamage(world, attacker, target, ((LF2Character)target).HitCounters, attacker.Frame.D.itrs[0]);
            ...
            File.WriteAllText(Output + profile + "-" + mode + (renderer ? "-renderer" : "") + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Type3 attacker post-hit in type0 unarmored, type0 armor reduced and type3 unarmored writer transactions and following full tick; native raw47 mapped fields plus descriptor/B2/RNG. Source previousXYZ have no asserted Unity carrier; not platform or image parity."
            }, Formatting.Indented));
            Assert.That(cases, Is.EqualTo(profile == BattleRuntimeProfile.Authority400 && !renderer ? 13 : 6));
        ...
        [TestCase(BattleRuntimeProfile.Authority400, BattleEcsCharacterFrameAdvancePassMode.DataOriented)]
        public void Type3RepresentativesSurviveLocalSnapshotReplay(BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode)
        {
            int cases = 0;
            foreach (var row in File.ReadLines(Output + "source/first.jsonl").Select(JObject.Parse).Where(row => RepresentativeIndices.Contains((int)row["index"])))
            {
                ...
                try
                {
                    ...
                    string[] expected = ExecuteType3AndTwoTicks(world);
                    ...
                    Assert.That(ExecuteType3AndTwoTicks(world), Is.EqualTo(expected), "replay case " + row["index"]);
                    cases++;
                }
                finally { Shutdown(world, false); }
            }
            ...
            File.WriteAllText(Output + "representative-replay-" + profile + "-" + mode + ".json", JsonConvert.SerializeObject(new
            {
                ...
                scope = "Same-world Type3 attacker post-hit transaction across three routes plus two complete ticks; not cross-world or renderer validation."
            }, Formatting.Indented));
        }

        private static string[] ExecuteType3AndTwoTicks(SimulationWorld world)
        {
            var attacker = world.FindEntityByRuntimeSlotForQuery(0);
            var target = world.FindEntityByRuntimeSlotForQuery(70);
            bool applied = target.GetCurrentDataObjectTypeForSimulation() == 3
                ? world.DamageWriter.ApplySpecialAttackDamage(world, attacker, target, attacker.Frame.D.itrs[0])
                : world.DamageWriter.ApplyStandardCharacterDamage(world, attacker, target, ((LF2Character)target).HitCounters, attacker.Frame.D.itrs[0]);
        ...
        private static SimulationWorld CreateWorld(JObject row, BattleRuntimeProfile profile, BattleEcsCharacterFrameAdvancePassMode mode, bool renderer)
        {
            ...
            try
            {
                ...
                {
                    new ObjectDefinition(77, 3, "type3-attacker.dat"), new ObjectDefinition(78, (int)row["params"]["targetType"], "type3-target.dat")
    ...
    [InitializeOnLoad]
    internal static class NTSD28Q06Type3PostHitPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_Type3PostHitPlay.request";
        private const string Result = "Temp/NTSD28_Q06_Type3PostHitPlay.result.json";
        static NTSD28Q06Type3PostHitPlayProbe() { EditorApplication.update += Poll; }
        ...
        private static void Poll()
        {
            ...
            foreach (var profile in new[] { BattleRuntimeProfile.Authority400, BattleRuntimeProfile.MobileExtended })
            {
                ...
                try
                {
                    NTSD28Q06Type3AttackerPostHitNativeFrameAccessEditorTests.RunMatrix(profile, mode, renderer);
                    passedCases += 6;
                }
            ...
            string status = errors.Count == 0 && passedCases == 12 && unchanged && borrowersAfter == borrowers ? "PASS" : "FAIL";
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Real Scene preserved; Type3 attacker post-hit6 representatives (standard type0, armor reduced type0, type3 target) on Authority/DataOriented/renderer and Mobile/Legacy/logic. Physical-key and image parity not claimed; separate Q05 shutdown result required."
            }, Formatting.Indented));
