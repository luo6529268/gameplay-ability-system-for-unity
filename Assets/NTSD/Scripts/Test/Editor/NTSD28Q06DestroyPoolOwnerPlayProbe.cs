...
{
    [InitializeOnLoad]
    internal static class NTSD28Q06DestroyPoolOwnerPlayProbe
    {
        private const string Request = "Temp/NTSD28_Q06_DestroyPoolOwnerPlay.request";
        private const string Result = "Temp/NTSD28_Q06_DestroyPoolOwnerPlay.result.json";

        static NTSD28Q06DestroyPoolOwnerPlayProbe()
        {
        ...
        private static void Poll()
        {
            ...
            foreach (int index in new[] { 0, 1, 5 })
            {
                try
                {
                    NTSD28Q06DestroyPoolOwnerEditorTests.VerifyForPlay(index);
                    passed++;
                }
            ...
            File.WriteAllText(Result, JsonConvert.SerializeObject(new
            {
                ...
                scope = "Actual pooled Renderer destruction for three logic shell types in independent Worlds; scene checksum and global renderer borrowers preserved."
            }, Formatting.Indented));
