...
namespace NTSD.Animation
{
    ...
    public class CharacterStates : MMSingleton<CharacterStates>
    {
        ...
        public delegate bool StateHandler(LF2CharacterAnimator character, string eventType, object eventData);
        ...
        /// <summary>
        /// 获取当前逻辑帧的下一帧 ID
        /// 允许状态处理器动态决定下一帧去哪里 (例如根据输入跳转不同帧)
        /// </summary>
        public int? GetNextFrameId(LF2CharacterAnimator character)
        {
            int currentState = character.CurrentFrame.state;
            int? nextFrameId = null;

            // 1. 询问当前状态处理器
            if (stateHandlers.TryGetValue(currentState, out StateHandler specificHandler))
            {
                // 通过事件数据传递 ref 类型的 nextFrameId
                var data = new NextFrameData { nextFrameId = null };
                specificHandler(character, "get_next_frame", data);
                nextFrameId = data.nextFrameId;
            }

            // 2. 询问通用处理器
            if (!nextFrameId.HasValue && genericHandler != null)
            {
                var data = new NextFrameData { nextFrameId = null };
                genericHandler(character, "get_next_frame", data);
                nextFrameId = data.nextFrameId;
            }
        ...
        /// </summary>
        private bool GenericStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "hit":
        ...
        /// </summary>
        private bool HandleGenericHit(LF2CharacterAnimator target, object eventData)
        {
            if (target == null || target.ps == null || target.CurrentFrame == null) return false;
            ...
            if (target.CurrentFrame.state == LF2States.Lying) return false;

            InteractionArea itr = evt.itr;
            LF2CharacterAnimator attacker = evt.attacker;
        ...
        /// </summary>
        private bool HandleGenericFrame(LF2CharacterAnimator character)
        {
        ...
        /// </summary>
        private bool HandleGenericTU(LF2CharacterAnimator character)
        {
            // 1. 消失效果状态机 (FLF:56-82)
            // TODO: 需要特效系统
        ...
        /// </summary>
        private bool HandleGenericTransit(LF2CharacterAnimator character)
        {
        ...
        /// </summary>
        private bool HandleGenericStateExit(LF2CharacterAnimator character, string combo)
        {
        ...
        /// </list>
        /// </summary>
        private bool HandleGenericCombo(LF2CharacterAnimator character, string combo)
        {
            ...
            int targetFrame = character.CurrentFrame.Hit[tag];
            if (targetFrame <= 0)
                return false;
        ...
        private (int dx, int dz) GetMoveInput(LF2CharacterAnimator character)
        {
            int dx = 0, dz = 0;
        ...
        /// 处理角色的静止、基础按键响应
        /// </summary>
        private bool StandingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
                        ...
                        case null:
                            // 检查是否有实际方向输入
                            {
                                ...
                                if (hasDx || hasDz)
                                {
                                    ...
                                    var ps = character.ps;
                                    DIRECTION dir = character.unitActions.dir;
        ...
        /// </list>
        /// </summary>
        private bool WalkingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            var (dx, dz) = GetMoveInput(character);
            ...
            switch (eventType)
            {
                ...
                case "TU":
                    ...
                    if (character.unitActions != null && character._Character != null)
                    {
                        ...
                        // 斜向移动时的速度补偿系数 (约 0.7)
                        var xfactor = 1 - (dz != 0 ? 1 : 0) * (2f / 7f);

                        if (dx != 0) ps.vx = (int)dir * characterData.walking_speed * xfactor;
                        ps.vz = dz * characterData.walking_speedz;
                ...
                case "combo":
                    // 行走中的输入处理
                    string comboKey = eventData as string;
                    DIRECTION currentDir = character.unitActions.dir;

                    // 1. 处理转向
                    if (dx != 0 && dx != (int)currentDir)
                    {
                        DIRECTION newDir = currentDir == DIRECTION.RIGHT ? DIRECTION.LEFT : DIRECTION.RIGHT;
        ...
        /// </summary>
        private bool RunningStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            var (dx, dz) = GetMoveInput(character);
            ...
            switch (eventType)
            {
                ...
                case "TU":
                    ...
                    {
                        DIRECTION dir = character.unitActions.dir;
                        var xfactor = 1 - (dz != 0 ? 1 : 0) * (1f / 7f);
                        var characterData = character._FrameDataWrapper.characterData;
                        var ps = character.ps;

                        ps.vx = xfactor * (int)dir * characterData.running_speed;
                        ps.vz = dz * characterData.running_speedz;
                    }
                    return false;
        ...
        /// </summary>
        private bool AttackStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
                ...
                case "TU":
                    ...
                    // 对应 FLF:511-547
                    var frameDataTU = character.CurrentFrame;
        ...
        /// </summary>
        private bool JumpStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
                    ...
                    if (character.PreviousFrameId == LF2StandardFrames.JumpAttack ||
                        character.PreviousFrameId == LF2StandardFrames.JumpAttack + 1)
                    {
                        character.StateMem["attlock"] = 2;
                    }
                    return false;

                case "TU":
                    // 1. 起跳速度设置 (Frame 211 -> 212)
                    if (character.StateMem.TryGetValue("frameTU", out object StateValue) && (bool)StateValue)
                    {
                        character.StateMem["frameTU"] = false;
                        if (character.NextFrameId == LF2StandardFrames.JumpingAir &&
                            character.PreviousFrameId == LF2StandardFrames.JumpingUp)
                        {
                            ...
                            // 记录地面高度，重置相对高度
                            ps.groundY = character.unitActions != null ? character.unitActions.transform.position.y : character.transform.position.y;
                            ps.y = 0;

                            // 应用跳跃速度
                            ps.vx = dx * (characterData.jump_distance - 1);
                            ps.vz = dz * (characterData.jump_distancez - 1);
                            ps.vy = characterData.jump_height;
                        }
                    }

                    // 2. 更新攻击锁定计时器
                    if (character.StateMem.TryGetValue("attlock", out object lockVal))
                    {
                        int val = (int)lockVal;
                        if (val > 0) character.StateMem["attlock"] = val - 1;
                    }
                    return false;
        ...
        /// </summary>
        private bool DashStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_entry":
                    // 从奔跑或蹲下进入冲刺时，设置初始冲刺速度
                    if ((character.PreviousFrameId >= LF2StandardFrames.RunningStart &&
                         character.PreviousFrameId <= LF2StandardFrames.RunningEnd) ||
                        character.PreviousFrameId == LF2StandardFrames.Crouch)
                    {
                        float dirFactor = (character.CurrentFrameId == LF2StandardFrames.DashForward) ? 1f : -1f;
                        // TODO: 应用 dash_distance 等数据到 ps.vx
                    }
                    return false;
        ...
        /// </summary>
        private bool DefendingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// </summary>
        private bool InjuredStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_entry":
        ...
        /// </summary>
        private bool FallingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 所有技能共用这个处理器
        /// </summary>
        private bool AbilityStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
                    ...
                    {
                        // 遍历所有 ITR 区域
                        foreach (var itr in frameData.itrs)
                        {
        ...
        /// - 109: 背面爬起结束
        /// </summary>
        private bool RowingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "TU":
        ...
        /// </summary>
        private bool BrokenDefendStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame_force":
                case "TU_force":
                    ...
                    if (frameData.dvx != 0)
                    {
                        ...
                        // float velocityX = character.ps.vx;
                        // int dirH = character.unitActions.dir == DIRECTION.RIGHT ? 1 : -1;
                        //
                        // // 检查速度方向是否与朝向方向相反
                        // int velocityDir = velocityX > 0 ? 1 : -1;
                        // if (velocityDir != dirH)
                        // {
                        //     float avx = Mathf.Abs(velocityX);  // 绝对速度
                        //     int dirx = 2 * velocityDir;         // 方向因子（使用2倍确保明确方向值）
                        //
                        //     // 检查是否在空中 或 速度小于帧定义的dvx
                        //     bool isInAir = character.ps.y < 0;  // FLF Y-axis: 负数为空中
                        //     if (isInAir || avx < frameData.dvx)
                        //     {
                        //         // 强制设置速度为 dvx * dirx
                        //         character.ps.vx = dirx * frameData.dvx;
                        //     }
                        //
                        //     // dvx为负数时的特殊处理（向朝向方向相反移动）
                        //     if (frameData.dvx < 0)
                        //     {
                        //         character.ps.vx -= dirx;
                        //     }
                        // }
                    }
                    return false;
        ...
        /// </summary>
        private bool CatchingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_entry":
                ...
                case "frame":
                    ...
                    // TODO: 需要实现抓取系统后取消注释
                    // if (character.Catching != null && frameData.cpoint != null)
                    // {
                    //     // 计算抓点世界坐标
                    //     Vector3 cpointWorldPos = character.transform.TransformPoint(
                    //         new Vector3(frameData.cpoint.x, frameData.cpoint.y, frameData.cpoint.z)
                    //     );
                    //
                    //     // 通知被抓对象更新位置
                    //     character.Catching.caught_b(
                    //         cpointWorldPos,           // 抓点世界坐标
                    //         frameData.cpoint,         // cpoint数据
                ...
                case "TU":
                    ...
                    //
                    //         var cpoint = character.CurrentFrame.cpoint;
                    //
                    //         // ==================== 处理伤害 ====================
                    //         // 对应 FLF Line 632-637
                    //         if (cpoint.injury != 0)
        ...
        /// 5. 抓取状态验证（双向检查）
        /// </summary>
        private bool BeingCaughtStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_exit":
        ...
        /// - 强制丢弃武器
        /// </summary>
        private bool FrozenStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_exit":
        ...
        ///   4. count = -1: 销毁对象
        /// </summary>
        private bool LyingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_entry":
        ...
        /// </summary>
        private bool MixedStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
                    ...
                    // 帧54: 空中轻武器投掷结束 → 在空中时返回跳跃状态
                    if (frameId == LF2StandardFrames.SkyLgtWpThw3)
                    {
                        var frameData = character.CurrentFrame;
                        if (frameData.next == LF2StandardFrames.LoopToStart &&
                            character.ps.y < 0)
                    ...
                    if (character.NextFrameId == LF2StandardFrames.Crouch)  // 215
                    {
                        ...
                        if (comboKey == "jump")
                        {
                            var (dx, dz) = GetMoveInput(character);
                            {
                                ...
                                else if ((character.ps.vx > 0 ? 1 : -1) == character.ps.GetDir())
                                {
                                    character.TransitionToFrame(LF2StandardFrames.DashForward, 10);  // 213
                                    return true;
                                }
        ...
        /// </summary>
        private bool Injured2StateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
        ...
        /// - 燃烧状态21/22不会伤害队友
        /// </summary>
        private bool BurningStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// - 可能由Firen特定技能触发
        /// </summary>
        private bool FirenSpecificStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "TU":
        ...
        /// 出现次数：16
        /// </summary>
        private bool ChargingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "state_entry":
        ...
        /// 出现次数：128（合计）
        /// </summary>
        private bool WeaponStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 出现次数：190（合计）
        /// </summary>
        private bool ProjectileStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 这是使用最频繁的状态之一（307次）
        /// </summary>
        private bool ObjectFlyingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 出现次数：152
        /// </summary>
        private bool ObjectExpandingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 出现次数：47
        /// </summary>
        private bool EffectPlayingStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
        ...
        /// 出现次数：2
        /// </summary>
        private bool SpecialEffectStateHandler(LF2CharacterAnimator character, string eventType, object eventData)
        {
            switch (eventType)
            {
                case "frame":
