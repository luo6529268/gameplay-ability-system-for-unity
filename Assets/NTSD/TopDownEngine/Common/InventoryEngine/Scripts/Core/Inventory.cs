...
namespace MoreMountains.InventoryEngine
{
    ...
    /// 基础库存类。
    /// 将处理存储物品、保存和加载其内容、添加物品、移除物品、装备物品等。
    /// </summary>
    public class Inventory : MonoBehaviour, MMEventListener<MMInventoryEvent>, MMEventListener<MMGameEvent>
	{
        ...
        [Tooltip("库存组件就像库存的数据库和控制器部分。" +
			"它不会在屏幕上显示任何内容，您还需要一个库存显示组件。" +
			"在这里，您可以决定是否要在检视器中输出调试内容（对调试很有用）。")]
		...
		public int NumberOfStackableSlots(string searchedItemID, int maxStackSize)
		{
			...
			while (i < Content.Length)
			{
				if (InventoryItem.IsNull(Content[i]))
				{
					if (numberOfStackableSlots < int.MaxValue)
					{
...
;
        ...
        /// 返回可以添加到此库存中特定物品的最大值，而不超出物品上定义的最大数量。
        /// </summary>
        /// <param name="itemToAdd"></param>
        ...
        /// 返回库存中所有与指定名称匹配的物品的列表
        /// </summary>
        /// <returns>一个符合搜索条件的物品列表</returns>
		...
		public virtual int OnGetInventoryItemIndexByID(string ItemID) 
		{
			for (int i = 0; i < Content.Length; i++) 
			{
				if (!InventoryItem.IsNull(Content[i])) 
		...
		public virtual void DestoryItemByID(string ItemID) 
		{
			int index = OnGetInventoryItemIndexByID(ItemID);
			if (index != -1)
				DestroyItem(index);
		}
