...
namespace MoreMountains.TopDownEngine.Editors
{
    ...
    public class SAP2DPathfinderEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            {
                pathfinder.EditorSettings = (SAP2DEditorSettings)Resources.Load("SAP2DSettings/SAP2DEditorSettings");
        ...
        {
            pathfinder.AddGrid();
            SAP_GraphDrawer newDrawer = new SAP_GraphDrawer();
        ...
        private void DrawGridsList()
        {
            if (pathfinder.gridsCount == 0)
            {
                EditorGUILayout.HelpBox("SAP2D Manager 没有任何网格。" + 
                    "\n你应该通过在检视器窗口点击鼠标右键并选择‘添加新网格’来创建一个新的网格。", MessageType.Info);
                return;
            }
