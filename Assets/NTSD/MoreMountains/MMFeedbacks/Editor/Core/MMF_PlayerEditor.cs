...
namespace MoreMountains.Feedbacks
{
    ...
    /// 一个自定义编辑器，显示可折叠的MMFeedback列表、用于添加更多内容的下拉列表，以及用于在运行时测试反馈的测试按钮
    /// </summary>
	...
	public class MMF_PlayerEditor : Editor
	{
		...
		protected const string _inactiveMessage = "所有MMFeedback，包括这个，目前都被禁用。这是通过脚本完成的，通过更改MMFeedback的值。GlobalMMFeedbacksActive布尔值。现在这个值已经设置为false。将其设置为true将允许MMFeedback再次播放。";
		protected const string _instructionsMessage = "从“添加反馈”下拉列表中选择反馈并自定义它们。请记住，如果您不使用自动初始化（唤醒或启动），则需要通过脚本初始化它们。";
