...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks
	{
        ...
        /// <summary>
        /// 如果为真，当反馈播放时，检视器不会刷新，这可以节省性能，但例如反馈检视器中的进度条看起来就不会那么平滑。
        /// </summary>
        [Tooltip("如果为真，当反馈播放时，检视器不会刷新，这可以节省性能，但例如反馈检视器中的进度条看起来就不会那么平滑")]
        public bool PerformanceMode = false;

        /// <summary>
        /// 如果为真，在禁用时将对所有反馈调用StopFeedbacks
        /// </summary>
        [Tooltip("如果为真，在禁用时将对所有反馈调用StopFeedbacks")]
        ...
        /// 播放所有反馈，指定位置和强度。这个位置可以被每个反馈使用，并考虑到激发粒子或播放声音。
        ///	反馈强度是每个反馈可以用来降低其强度的一个因素，通常你会想要定义衰减基于时间或距离(使用较低的远离玩家的反馈强度值)。
        ///	另外，你可以强制反馈反向播放，忽略其当前状态
        /// </summary>
		...
		public virtual bool IsAllowedToPlay(Vector3 position)
		{
            ...
            // if we're already playing and can't play while already playing, we're not allowed to play   如果我们已经在玩了，而且不能在玩的时候玩，我们就不能玩了
		...
		{
            // if no pause was found, we just play all feedbacks at once 如果没有发现暂停，我们就立即播放所有反馈
            int count = FeedbacksList.Count;
			for (int i = 0; i < count; i++)
			{
        ...
        /// <summary>
        /// Computes new random duration multipliers on all feedbacks if needed 如果需要，计算所有反馈的新随机持续时间乘数
        /// </summary>
