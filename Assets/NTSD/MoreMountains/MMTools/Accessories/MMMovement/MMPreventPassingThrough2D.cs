...
namespace MoreMountains.Tools
{
    ...
    /// 通过在每次移动后向后投射一条射线，防止快速移动的对象穿过碰撞器。
    /// </summary>
    [AddComponentMenu("ThirdParty/More Mountains/Tools/Movement/MM Prevent Passing Through 2D")]
	public class MMPreventPassingThrough2D : MonoBehaviour 
	{

        [MMInformation("<size=13>通过在每次移动后向后投射一条射线，防止快速移动的对象穿过碰撞器。</size>",
        ...
        /// the bounds adjustment variable <summary>
        /// 边界调整变量
        /// </summary>
        public float SkinWidth = 0.1f;
        /// whether or not to reposition the rb if hitting a trigger collider  <summary>
        /// 是否在触发器碰撞器触发时重新定位刚体（Rigidbody）。
        /// </summary>
        public bool RepositionRigidbodyIfHitTrigger = true;
        /// whether or not to reposition the rb if hitting a non trigger collider <summary>
        /// 是否在与非触发器（non-trigger）碰撞器发生碰撞时重新定位刚体（Rigidbody）。
        /// </summary>
