...
{
	[Serializable]
	[AddComponentMenu("ThirdParty/More Mountains/Tools/Vision/MM Cone Of Vision 2D")]
	public class MMConeOfVision2D : MonoBehaviour
	{
		...
		protected virtual void DrawMesh()
		{
			...
			float centerAngle = OnGetCameraDirection();
			...
			{
				float angle = centerAngle + stepsAngle * i + EulerAngles.y - VisionAngle / 2f;
        ...
        float OnGetCameraDirection() 
		{
            ...
            mousePosition.z = _MainCamera.nearClipPlane; // 设置 Z 坐标为相机的近裁剪面
            Vector3 worldMousePosition = _MainCamera.ScreenToWorldPoint(mousePosition);
            Vector3 directionToMouse = (worldMousePosition - this.transform.position).normalized;

            // 计算鼠标方向的角度
            float centerAngle = Mathf.Atan2(directionToMouse.y, directionToMouse.x) * Mathf.Rad2Deg;
			return centerAngle;
        }
