...
{	
    ...
    /// 您可以使用各种方法从对象池中取出对象。
	/// OriginalOrder - 原始顺序：按照对象被添加到池中的原始顺序来获取对象。
	/// 2. OriginalOrderSequential - 原始顺序顺序：按照对象被添加到池中的原始顺序依次获取对象。
	/// 3. RandomBetweenObjects - 随机对象间：在对象之间随机选择一个对象来获取。
	/// 4. RandomPoolSizeBased - 基于池大小的随机：根据池的大小随机选择一个对象来获取。
    /// </summary>
    ...
    public class MMMultipleObjectPooler : MMObjectPooler
	{
		...
		[MMInformation("<size=13>MultipleObjectPooler 是一个对象储备库，由生成器（Spawner）使用。当被请求时，它会根据您选择的池化方法返回池中的一个对象（理想情况下是未激活的）。</size>" +
            "\n<size=13>-原始顺序（OriginalOrder）将按照您在检视器中设置的顺序（从上到下）生成对象。</size>" +
            "\n<size=13>-原始顺序顺序（OriginalOrderSequential）将做同样的事情，但是在移动到下一个对象之前会先清空每个对象池。</size>" +
            "\n<size=13>-随机对象间（RandomBetweenObjects）将从池中随机选择一个对象，但忽略其池大小，每个对象被选中的机会均等。</size>" +
            "\n<size=13>-基于池大小的随机（PoolSizeBased）会根据其池大小概率从池中随机选择一个对象（池越大，被选中的机会越高）。</size>", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
        /// the chosen pooling method
        [Tooltip("所选择的池化方法")]
        public MMPoolingMethods PoolingMethod = MMPoolingMethods.RandomPoolSizeBased;
		[MMInformation("如果您设置`CanPoolSameObjectTwice`为`false`，池化器将尝试防止同一个对象被池化两次，以避免重复。" +
			"“这只会影响随机池化方法，不影响有序池化。”", MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		...
		[Tooltip("是否同一个对象可以连续被池化两次。如果您设置`CanPoolSameObjectTwice`为`false`，" +
			"池化器将尝试防止同一个对象被重复池化以避免重复。这将只影响随机池化方法，不影响有序池化。")]
        ...
        [Tooltip("一个独特的名称，应该与您想要一起使用的所有MMMultipleObjectPoolers匹配")]
        ...
        [Tooltip("如果CanPoolSameObjectTwice设置为false，这决定了我们将迭代多少次以尝试找到不同的对象")]
