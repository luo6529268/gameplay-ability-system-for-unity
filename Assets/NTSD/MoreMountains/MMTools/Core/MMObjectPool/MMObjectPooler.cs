...
namespace MoreMountains.Tools
{
    ...
    /// 一个基类，根据使用情况（简单、多个对象池化器）进行扩展，并被生成器作为接口使用。
	/// 仍然处理共同的事情，比如单例和在start时初始化。
	/// 不要将此类添加到预制体上，否则不会有任何效果。相反，应该添加SimpleObjectPooler或MultipleObjectPooler。
    /// </summary>
    public abstract class MMObjectPooler : MonoBehaviour
	{
		...
		[Tooltip("如果为真，则池将尝试在找到具有相同名称的池时不创建新的等待池。")]
		...
		[Tooltip("如果为真，则所有等待和活动对象将被重新组合在一个空的游戏对象下。否则，它们将仅位于层级结构的顶级。")]
