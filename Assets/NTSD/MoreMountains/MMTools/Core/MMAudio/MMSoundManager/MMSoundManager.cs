...
namespace MoreMountains.Tools
{
	...
	/// 特点：
	/// -播放/停止/暂停/恢复/释放声音
	/// -完全控制：循环、音量、音调、立体声、空间混合、旁路、优先级、混响、多普勒级别、扩散、滚降模式、距离
	/// -支持2D和3D空间音效
	/// -内置音频源池，自动回收音频源以实现最大性能
	/// -内置音频混音器和组，带有现成的音轨（主音轨、音乐、音效、UI），并可根据需要在更多组上播放
	/// -停止/暂停/恢复/释放整个音轨
	/// -一次性停止/暂停/恢复/释放所有声音
	/// -静音/设置整个音轨的音量
	...
	/// -声音淡入/淡出
	/// -音轨淡入/淡出
	/// -独奏模式：播放一个声音时将一个或所有音轨静音，然后在之后自动取消静音
	/// -PlayOptions结构体
	/// -选择让声音在场景加载时持续存在，或从一个场景到另一个场景
	/// -音轨的检视器控件（音量、静音、取消静音、播放、暂停、停止、恢复、释放、声音数量）
	/// -MMSfxEvents
	/// -MMSoundManagerEvents：静音音轨、控制音轨、保存、加载、重置、停止持续声音
	/// </summary>
	[AddComponentMenu("ThirdParty/More Mountains/Tools/Audio/MM Sound Manager")]
		...
		MMEventListener<MMSoundManagerTrackFadeEvent>
	{
		...
		[Header("Pool")]
        /// 音频源池的大小，一批准备就绪且可循环使用的音频源。应大致等于你预期同时播放的最大声音数量
        [Tooltip("音频源池的大小，一批准备就绪且可循环使用的音频源。应大致等于你预期同时播放的最大声音数量")]
        public int AudioSourcePoolSize = 10;
        /// 池是否可以扩展（按需创建新的音频源）。在理想情况下，你希望避免这样做，并拥有足够大的池，以避免在运行时创建音频源的高昂成本。
        [Tooltip("池是否可以扩展（按需创建新的音频源）。在理想情况下，你希望避免这样做，并拥有足够大的池，以避免在运行时创建音频源的高昂成本。")]
