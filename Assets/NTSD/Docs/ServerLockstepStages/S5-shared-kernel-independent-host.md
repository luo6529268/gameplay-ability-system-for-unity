...
- GP-09 product behavior was confirmed on 2026-08-29: online battle does not pause; individual surrender/exit is an idempotent room/session lifecycle command, follows missing-neutral and the existing 30-second ownership barrier to Server AI, does not end the whole match, and remains reconnectable through GP-06. The original global one-shot mask consumer audit remains an evidence task, not a product-choice blocker.
...
- shared package/ABI consumable by Unity and Server;
...
| Same package/runtime | Unity Mono/IL2CPP and .NET Server consume same owner | Not started |
...
- 2026-08-29: GP-09 online no-pause and individual surrender/exit lifecycle were frozen as product behavior; implementation and original one-shot consumer evidence remain pending.
