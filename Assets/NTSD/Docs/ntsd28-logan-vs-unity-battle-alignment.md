...
当前游标：`EXECUTION=ACTIVE / ACTIVE_BATCH=BATCH-03 / ACTIVE_Q=Q06 / LAST_VERIFIED=LIFECYCLE_SCOPED_CHECKS_SELF_CHECK_PENDING / NEXT_TASK=NTSD28-Q06-NATIVE-WEAPON-PIECE-TRANSACTION-001 / CURRENT_VERSIONS=15,23,26,2,2_FRAME_CAMPAIGN_PENDING / SCENE_USER_CHANGE_PRESERVED / FULL_ALIGNMENT_INCOMPLETE`。
...
| 06 / Q06 | B6/B7及资源owner：精确producer/consumer与slot尾部接线 | Q05 | IN_PROGRESS / DISPLAY_PROGRESSION_PARTIAL | 审计NTSD28-Q06-RESOURCE-MP-FIRST-DIFFERENCE-AUDIT-001已限定通过；HP/MP已134 tests、3768/2028 native、SelfCheck/Play限定通过；下一NTSD28-Q06-NATIVE-FRAME-RUNTIME-READER-MIGRATION-001；Native访问器/资源内部资格已223+1/SelfCheck/Play限定通过，其余reader未迁移；OPoint出生值已3716/329/212/SelfCheck/双路径Play限定通过，display仍缺其余出生初值；HP/MP内部隐式零帧资格已补、完整高位动作tick未验，post未接；新增frame int/double分reader motion、chp/cmp内容消费；接收同源neutral trace tick3 currentMp200/201差异（MP bootstrap已校正，原因已由native四模式对照和当前Unity capture确认：缺少mode恢复门槛；完整MP事务已接，Q08正式mode注入待）；拆包接CPoint资源/settlement、OPoint完整materializer、+2F8 AI、revival reserve/join、HP/baseMax、C25资源/terminal/pieces/生灭。基础数值合同与规则夹具可先验证；mode/frontend和整场证据分别留Q08/Q12。每个子包实际接通即触发回访，不等整组结束。 | R02、R04～R13、R16 |
...
| Plan / prepared batch / next | NTSD28-PRIORITY-DEPENDENCIES-20260913 / BATCH-03 / Q06-native-frame-runtime-reader-migration |
| Active Q / Task / Change | Q06 / NTSD28-Q06-NATIVE-FRAME-TRANSACTION-INTEGRATION-001 / READY_FOR_EXACT_PRECHANGE_RECORD；快照Native descriptor已244/SelfCheck/Play限定VERIFIED，NativeAccessor及invalid fixture已VERIFIED，HP/MP恢复限定资源事务VERIFIED；父零帧全部reader和display未完。 |
...
| 已知内容/schema基线 | Unity旧138仅迁移前基线，D-023正式330目录可构建但尚未部署。runtime/aggregate/checksum/character/base版本13/21/24/2/2已联合验证；trace3/raw-source2/50字段44绑定6MISSING。 |
...
| 下一步最窄动作 | 从真实frame绑定/direct/next/快照FrameDataId链核对Native accessor迁移，先精确code-path/Record再写；继续按输入、hit/CPoint/held、生成/表现等分组。保留旧查询给未迁移/非战斗，不全局替换或为清grep改dead代码。 |
| 最近验证 | Native accessor原函数81行/63合法、223/223＋旧接口1/1、完整SelfCheck、Scene两资源owner六例/校验恢复4→4/关闭全0/两帧Stopped；CS0/dirtyfalse/root14/用户Scene SHA不变。未验高动作完整tick，其余reader仍旧。证据artifacts/diagnostics/NTSD28-Q06-NATIVE-FRAME-ACCESSOR-RESOURCE-ADMISSION-001/REPORT.md。 |
