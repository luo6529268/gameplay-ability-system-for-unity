...
最新验证（2026-07-14）：fresh `dotnet build Assembly-CSharp.csproj /v:minimal /m:1` 为 **0 errors / 42 warnings**；随后 fresh Unity batchmode `BattleRuntimeSelfCheck` 再次完整通过，`ntsd_selfcheck_unity.log` 结尾明确记录 `[BattleRuntimeSelfCheck] 战斗运行时自检通过。`。本轮新增通过的针对性断言为 `CheckKind15CharacterWhirlwind` 与 `CheckKind16CharacterSideEffects`。因此 T0/T1/T2/T3/T4/T5/T6 均已完成 Unity 运行时验收；剩余主线优先级收敛为 T7/T8 → T9。
