...
> 2026-09-26 Q07减伤首差新包 `NTSD28-Q07-SASUKE-ARMOR-TARGET-FIRST-DIFF-001 / PLANNED`：正式/暂存OID87 `sus.dat`同SHA，type-1护甲为静态候选；先在原Sasuke千本输入下把目标改OID87/初始MP300，以X550重叠和X1200对照验证正式命中/护甲选中，再走根EXE与原Editor raw。两诊断脚本已准确声明Task/Change/Ledger，尚未写代码。未测先不补生产；Q07/D-024开放。

> 2026-09-26 Q07下一检查顺序：本轮只读发现正式资源事务的减伤与cpoint受伤/投掷调用，在Unity相应writer未见同调用；这是静态候选，不能当首差。先选正式可达、非满PP、同seed输入的减伤或持有命中及未命中对照，根EXE/原Editor逐tick后才建独立生产修复包。入口与边界见`NTSD28-Q07-SASUKE-NEEDLE-TARGET-HIT-FIRST-DIFF-001/ADJACENT-RESOURCE-BRANCH-AUDIT-20260926.md`；Q07/D-024继续开放。

> 2026-09-26 最新Q07通用资源修复 `NTSD28-Q07-STANDARD-HIT-RESOURCE-TRANSACTION-001 / FOCUSED_TEST_PASS`：共享未护甲角色受击路径按正式版在HP/credit后调用资源事务，以物理攻击源算injury/suppression、两跳owner收奖励，写当前PP而非旧MP。原Editor RED2/2→GREEN2/2、纯核8/8，正式根EXE同初态X550 1428/1428、X1200 2100/2100映射实体字段同值；前后raw仅X550佐助tick16–26 MP改变。较宽测试24中一项旧反射夹具参数异常未归因、不称全绿。自然Battle Scene Play、护甲/减伤其它分支与正式EXE GPU待独立出口；Q07/D-024/总目标继续开放。详同ID验收报告；下方PLANNED是实施前记录。

> 2026-09-26 Q07下一包 `NTSD28-Q07-STANDARD-HIT-RESOURCE-TRANSACTION-001 / PLANNED`：三脚本Task/Change/Ledger已在实施前建立；以X550正式tick16 MP500/Unity400首差为RED，修共享type0未护甲受击资源事务及PP载体，再保留前轨复跑X550/X1200原Editor raw。非战斗、DAT数据、图片、Scene/相机不动；Q07/D-024保持开放。

> 2026-09-26 Q07最新 `NTSD28-Q07-SASUKE-NEEDLE-TARGET-HIT-FIRST-DIFF-001 / VERIFIED_SCOPED_DIAGNOSTIC`：源/根正式EXE/原Editor同输入X550命中与X1200未命中26tick均PASS；X1200实体2100/2100同值，X550的1428比较仅佐助MP在tick16–26不同（正式500、Unity400）。目标HP500→360/action186与四OID440生命周期一致。原生共享资源奖励调用在Unity通用受击路径缺失；下一必须独立Task/Change作通用结算修复，保留前轨并再跑两例。Q07/D-024开放，正式EXE同世界GPU/物理Play未被这项证明；下方PLANNED是历史。

> 2026-09-26 Q07佐助有目标首差新包 `NTSD28-Q07-SASUKE-NEEDLE-TARGET-HIT-FIRST-DIFF-001 / PLANNED`：先基于formal action2 ITR与原Battle自然四OID440，做X550重叠假设/X1200对照；准确两个诊断脚本的Task/Change/Ledger已在代码前建。不得以几何静态重叠代替真实候选/命中，先正式LFR/根EXE与原Unity完整Driver逐tick，若首差再另立生产包。历史v3恢复文件及后续增量保留，DAT/Scene/非战斗不改；Q07/D-024开放。

> 2026-09-26 Q07下一有目标碰撞入口：正式/暂存OID440 `c/sasu/a/chi.dat`同SHA，动作2为完整kind0 ITR；原Battle物理佐助L/D/J既有结果自然产生四个OID440/action2。Lee五OID204动作20–23无ITR，不能用作发起命中验收。下一只在有明确目标几何/非重叠控制及同种子输入下找候选→命中首差，再做自然Play/正式EXE；当前仅静态入口和旧Play出生证据，命中未测。见`artifacts/diagnostics/NTSD28-Q07-SASUKE-NEEDLE-ITR-ENTRY-AUDIT-20260926.md`。三份v3文档此前已恢复且NUL-free，后续增量不得被旧候选覆盖；Q07/D-024/总目标开放。

> Q07最新限定出口：`NTSD28-Q07-SASUKE-NEEDLE-CAMERA-PIXEL-001 / VERIFIED`原Editor第四轮物理L/D/J自然四OID440、tick18正式`chi.png/pic1`四命令及全黑基线ROI新增424像素重复通过；既有有序关闭owner完成后，idempotent报告`AlreadyStopped / RuntimeMapCleared`，剩余World对象/槽/池借用0/0/0。Editor退出、Console0 C#错、Menu/Battle/两配置SHA稳定。只关Unity子体组相机+关闭子包，正式EXE GPU和Q07整组未关；此前同ID`RUNTIME_PENDING`为历史。详同ID验收报告。

> 当前Q07限定出口：`NTSD28-Q07-SASUKE-NEEDLE-CAMERA-PIXEL-001 / RUNTIME_PENDING`原Editor正式内容物理L/D/J第三轮四自然OID440、四个分离投影中心的`chi.png/pic1`中央命令，tick18出生前全黑ROI中新增424非黑像素；原图目检及Pillow独立复数一致。首轮失败和第二轮重叠ROI记录保留。Editor编译0 C#错/退出EditMode/相机恢复/四保护SHA不变；后退出池计数和正式EXE GPU同画面未测，Q07/总目标开放。详本包ACCEPTANCE。三份v3恢复已完成且新增记录不得被旧候选覆盖。

> 2026-09-26 Q07当前精确执行 `NTSD28-Q07-SASUKE-NEEDLE-CAMERA-PIXEL-001 / PLANNED`：新opt-in Editor观察者拟配合现有佐助物理L/D/J探针，在原Battle Scene自然四OID440出生前后读中央相机PNG与实体命令；Task/Change/Ledger已在脚本前登记。生产、DAT、Scene、相机配置和非战斗不动；Unity像素不等于正式EXE同画面，Q07仍开放。

> 2026-09-26 Q07旧资产退场门：非空正式根的Battle选中路径已有原Editor限定证据，但空根Menu预热旧DAT/图片并依赖其完成后显示模式/选人；直接删旧文件或即时拒绝空根会改变保留的Menu/loading行为。521旧文件及meta仍不删、不移，旧动态reader已按owner定位但未退场；详`artifacts/diagnostics/NTSD28-Q07-LEGACY-MENU-FALLBACK-BOUNDARY-20260926.md`。继续Q07战斗内容/自然表现，不借资源清理改非战斗流程。

> 2026-09-26 Q07六图格动作入口下一步：已只读排除四份正式DAT中的目标帧字面/编码转移及已检playable相邻state85、`next=999`、state12物理路径；不能把目录6/6或强制初始动作称作自然到帧。动态关系、碰撞、AI、外部初态仍待真实写者与普通输入见证；随后再做根正式EXE/原Unity同态像素。详`artifacts/diagnostics/NTSD28-Q07-SIX-BOUNDARY-WRITER-EXCLUSION-20260926.md`。Q07/BATCH-04及总目标开放，未改战斗脚本/资源/Scene。

> 2026-09-26 最新 Q09/P-08：原 Battle Scene 正式 Ita HP500/166 受控中央相机A/B已证body后1x3标记及3个独立复核红像素，Editor非Play/四保护SHA与清理稳定。两份探针假阴性保留。Legacy live body在`LF2ObjectRenderer`，`BattleEntityOverlayRenderer`仅测试自检调用；下一若实现Legacy，需独立声明生产调用、池/释放/有序关闭，并验自然低血量和正式EXE同状态。总表P-08仍`CENTRAL_CONTROLLED_GPU_PASS / NATURAL_LEGACY_EXE_PENDING`，Q09与总目标开放。证据：`artifacts/diagnostics/NTSD28-Q09-BPOINT-BLEED-SCENE-PIXEL-001/ACCEPTANCE.md`与`LEGACY-OWNER-AUDIT.md`。

> 2026-09-26 Q09/P-08 `NTSD28-Q09-BPOINT-BLEED-CENTRAL-001 / FOCUSED_TEST_PASS / RUNTIME_PENDING`：正式当前318个bpoint所需的中央有序红色1x3标记已接入快照→命令→内置白纹理→网格，阈值用基础HP三分之一、不按OID特判。原Editor编译0错，新例2/2、相邻32/32、网格1/1、低于阈值1/1；Ledger860/20/差异及四保护SHA稳定。`NTSD28-Q09-TRUSTED-COMMAND-TEST-CONSTRUCTOR-001`独立修复四个旧反射夹具参数，首轮28/32保留。下一需原Battle Scene自然Ita/Sasu低血量像素、Legacy出口及正式EXE同状态；P-08/Q09/总目标开放。详 `artifacts/diagnostics/NTSD28-Q09-BPOINT-BLEED-CENTRAL-001/ACCEPTANCE.md`。

> 2026-09-26 Q09相邻测试门 `NTSD28-Q09-TRUSTED-COMMAND-TEST-CONSTRUCTOR-001 / IN_PROGRESS`：原Editor P-08新增2/2 PASS，相邻32例28/32；四个旧resolver测试在trusted-command旧23参反射签名处失败。脚本前另建Task/Change/Ledger，下一仅补末四参数并聚焦重跑，保护P-08生产与测试hunk。Play/EXE和P-08/Q09聚合仍待。

> 2026-09-26 Q09/P-08 `NTSD28-Q09-BPOINT-BLEED-CENTRAL-001 / IN_PROGRESS`：按正式当前318个bpoint和配对playable规则，脚本前已建Task/Change/Ledger。下一仅在已声明五个脚本中接中央有序低血量实心标记，使用正式缺省1x3红/基础HP三分之一，不按OID特判、不改DAT。原Editor定向测试/Play、Legacy和正式EXE画面仍待，Q09与总目标开放。

> 2026-09-26 Q09 `NTSD28-Q09-COM-LABEL-TEST-CONSTRUCTOR-001 / FOCUSED_TEST_PASS`：旧反射夹具补bool签名/false实参，原Editor确切单例1/1及八例整类8/8通过、编译0错，四保护SHA稳定。Q09平台阴影受控完整Driver已有10C=-5/中央Shadow命令，但当前相机捕获ROI被其它命令包围，独占像素0仍不能归属；下一需有效隔离见证，不得改写为GPU PASS。Q07/Q09/总目标开放。下方PLANNED为历史。

> 2026-09-26 下一Q09测试夹具 `NTSD28-Q09-COM-LABEL-TEST-CONSTRUCTOR-001 / PLANNED`：当前com-label构造函数末项是可选bool，但反射按精确签名查找，旧六参数测试失败于绘制前。Task/Change/Ledger/STATE/对齐总表已登记；只在既有Editor测试加七参数与false并跑该例、随后必要时七例，保护已有阴影测试hunk。阴影像素仍单独开放。

> 2026-09-26 Q07 当前 `NTSD28-Q07-KO-FEED-PRODUCTION-ROW-WITNESS-001 / FOCUSED_TEST_PASS`：原Editor同初态鸣人38tick生产Driver行轨迹1/1 PASS，tick8事件time7、行tick8–36可见、tick37/38隐藏，逻辑尾记录仍1，退出对象/槽/借用0；测试帮助器仅可选项目mode Asset+隔离EditMode池，默认旧调用不变。失败三轮/终态CSV与job均存。下一回 Q07 非例外像素与其它自然技能/资源读者，Q07/Q09/总目标开放。四保护文件SHA未变。

> 2026-09-26 Q07 `NTSD28-Q07-KO-FEED-PRODUCTION-ROW-WITNESS-001 / CODE_WRITTEN`：默认关闭的项目模式Asset回放参数与38tick单例Editor见证已写，下一在原项目Editor编译/发现/运行该精确测试，检查事件tick8/time7及发布行tick36/37分界；失败保留CSV再判首差。无生产、DAT、Scene或非战斗改动，下方PLANNED为旧状态。

> 2026-09-26 Q07 当前执行 `NTSD28-Q07-KO-FEED-PRODUCTION-ROW-WITNESS-001 / PLANNED`：正式事件tick8/time7、源码行窗口及项目模式Asset输入已核；下一用原项目Editor的同初态鸣人回放延长到tick38，观测生产Driver每tick KO事件/发布行。仅测试帮助器可选Asset入口与单个新测试，已预登记Task/Change/Ledger；不改生产/DAT/Scene/非战斗，Q07/Q09仍开放。下方Q09夹具9/9结论保持。

> 2026-09-26 当前游标：`NTSD28-Q09-PROJECT-MODE-TEST-FIXTURES-001 / FOCUSED_TEST_PASS`，原Editor编译0错，受影响9/9定向EditMode通过；模式来源是现有 `ProjectBattleModeConfig` Asset，排除mode DAT未回迁。首轮图标临时夹具缺SPARK的正式system.dat/PNG已在同测试文件补齐，失败与终态JSON均保留。Q07共享KO时钟的Q09行消费者测试现在可运行，但精确生产可见末tick/正式像素仍需另验；Q07/Q09/总目标开放。双Scene、GameConfig、模式Asset磁盘哈希未变。下方CODE_WRITTEN为历史阶段。

> Q09 当前精确包 `NTSD28-Q09-PROJECT-MODE-TEST-FIXTURES-001 / CODE_WRITTEN`：三处旧Editor测试已改用现有ProjectBattleModeConfig快照；图标夹具只复制正式PNG，排除ID变体是内存克隆。原Editor编译/聚焦待；Q07共享KO时钟原Battle Scene Menu→物理J→KO→350→选角复验PASS、池借用0、Scene/GameConfig哈希稳定。生产/Asset/DAT/Scene/非战斗未改，Q07/Q09/总目标开放；下方PLANNED为改前登记。

> Q07 当前精确包 `NTSD28-Q07-NATIVE-KNOCKOUT-EVENT-CLOCK-001 / FOCUSED_TEST_PASS`：共享KO时间读既有C24 `NativeFrameSequence`；原Editor编译0错、前/后C24 2/2、Q08事件3/3、Q10声音5/5，鸣人同初态30tick KO完成tick8/正式事件时间7同值，raw/domain/input-RNG各30/30不变。Q09行投影旧夹具在SetUp读取用户排除mode DAT失败，不恢复原DAT；行末可见帧、更新的Play探针/正式像素待验。DAT/PNG/Scene/非战斗未动，Q07/总目标开放。详同ID ACCEPTANCE；下方CODE_WRITTEN为改前记录。

> Q07 当前诊断 `NTSD28-Q07-NARUTO-PUNCH-KO-EVENT-TRACE-001 / VERIFIED_SCOPED_DIAGNOSTIC`：原Editor同初态30tick PASS，旧三轨各30/30不变；正式/Unity KO同在完成tick8出生，归属及类型五字段同值，事件时间正式7/Unity8首差。正式源以 C24 `sequence_` 计时，Unity writer误用 host `CurrentTickIndex`；生产已有 `NativeFrameSequence`，须另建共享修复包覆盖 C24 前后KO路径和feed/快照/校验。旧521不删，DAT/图片/生产/Scene/非战斗本诊断未改；Q07/R09/R10/R18和总目标开放。详同ID ACCEPTANCE。下方CODE_WRITTEN为改前记录。

> Q07鸣人同种子RNG `NTSD28-Q07-NARUTO-PUNCH-SEEDED-RNG-TRACE-001 / VERIFIED_SCOPED_SEEDED_SOURCE_UNITY_RNG_STATE_PARITY`：正式配对源36例作者/KO和旧字段及LFR字节不变；首例原Editor raw的CRT+同步七字段30tick 210/210同值，tick8 CRT state2524509468/calls3002。根EXE LFR playback默认seed0仍不得宣称全CRT状态同值；只是该seed下paired source/Unity状态无首差。首次链接/路径失败保留，第二次编译/运行PASS。Q08已有真实Menu物理J→KO归属独立证据，无需重测相同条件；下一Q07优先审自然技能/生命周期、旧动态reader与非例外可见表现，R17/Q09承担画面。DAT/生产/Scene/非战斗不改，旧521不删；Q07/R09/R10/R18及总目标开放。详同ID REPORT；下条PLANNED已过时。

> Q07当前精确包 `NTSD28-Q07-NARUTO-PUNCH-SEEDED-RNG-TRACE-001 / PLANNED`：同种子seed682973786配对源与Unity的30tick CRT状态尚未比；根EXE LFR默认seed0是格式/回放限制，不据此修生产。已在改脚本前登记Task/Change/Ledger，下一仅给现有Tools鸣人36例诊断CSV加源RNG观察列并新目录运行，与保留的原Editor run2逐tick首差。旧521资源不删，Q08已另证物理J/KO，不重复探针；Q07/R09/R10/R18及总目标开放。

> Q07鸣人frame513同条件原Editor对照 `NTSD28-Q07-NARUTO-PUNCH-UNITY-RAW-FIRST-DIFF-001 / VERIFIED_SCOPED_EDITMODE_NARUTO_PUNCH_ENTITY_INPUT_PARITY`：原Editor 30tick raw/domain/input-RNG run2 PASS，正式EXE LFR对照的槽位集30/30、实体1560/1560、输入1140/1140、相位30/30、同步RNG30/30同值；tick8 action513致目标action186/HP-10。run1夹具校验FAIL保存，run2修正谓词后PASS；根LFR CRT默认seed0与源/Unity种子682973786不同，不报完整CRT同值。仅Editor诊断脚本及证据，生产/DAT/图片/Scene/非战斗未动。下一Q07独立验证同条件Battle Scene物理键及KO事件归属/可见画面；Q07/R09/R10/R18和总目标开放，详同ID ACCEPTANCE。下条PLANNED已过时。

> Q07下一包 `NTSD28-Q07-NARUTO-PUNCH-UNITY-RAW-FIRST-DIFF-001 / PLANNED`：正式鸣人frame513 LFR已备，现只给既有原Editor raw-capture诊断加X525/两Naruto/seed682973786/J@完成tick2/30tick严格schema，输出同初态实体/domain/input-RNG 首差；Task/Change/Ledger已先登记。不得用旧Q08物理J Play结果冒充同条件比较，也不得改DAT/生产/Scene/非战斗或其它Editor项目。Q07开放。

> Q07鸣人普通出拳正式源/发行 `NTSD28-Q07-NARUTO-PUNCH-FORMAL-HIT-001 / VERIFIED_SCOPED_FORMAL_SOURCE_RELEASE_AUTHORED_PUNCH_HIT`：36个有界普通攻击/站位例均自然形成作者帧与归属KO；首例X525、攻击tick2x1，frame513于tick8造成HP10→-10与0/1/0/0归属。根正式EXE同LFR无头回放PASS，30tick动作/目标HP与动作/输入阶段0差异。仅新增Tools探针及证据，无DAT/Unity生产/Scene/非战斗改动。下一须**独立**原Editor同seed、双方完整初态及逐tick输入包；Q08旧物理J证据不能直接比较。Q07/R09/R10/R18和总目标开放，详同ID REPORT。

> Q07源侧结果 `NTSD28-Q07-RASENGAN-ACTION35-TARGET-SOURCE-001 / VERIFIED_SCOPED_NEGATIVE_CONSUMER_GATE`：鸣人自然frame254→OID434/action35的kind5 ITR在11位置/605tick中有五位置10条候选，正式普通hit分类器10/10因无有效holder/wpoint强度替换拒绝；frame254 `attacking:0`，要求1..9，目标HP始终500。最终两扫描同SHA、未输出有效目标LFR，根EXE只核身份；Unity同条件未跑。下一个实际可消费自然hit候选先审已有Q08鸣人frame513物理J/KO，再独立立项。DAT/生产/Scene/非战斗未动，Q07/R09/R12/R18开放。详同ID REPORT。

> Q07 Lee下一碰撞见证更正（2026-09-26）：本普通J→L五OID204子体动作20–23的正式/暂存 `a/cha/cha.dat` 同SHA且均无 `itr`，正式playable候选收集会跳过无ITR源帧。不要继续把目标移入这五子体路径来测试其主动几何命中；先选自然可达且带ITR的动作。目标攻击子体和其他交互未证；Q07/D-024及总目标开放。仅只读复核，详同ID生命周期Task。

> Q07 Lee无目标OID204生命周期 `NTSD28-Q07-LEE-CHILD-LIFECYCLE-TRACE-001 / VERIFIED_SCOPED_NO_TARGET_CHILD_BIRTH_TO_DESPAWN_TRACE`：根正式EXE与原Editor保存trace的45完成tick，五子slots51–55 tick6出生、tick6–13各8tick动作20/20/21/21/22/22/23/23、tick14五条正式despawn code1000及Unity快照death、后续无残留；45槽位集/640字段全同。此限定关闭当前无目标出生至退场，不闭碰撞/交互、内部回收原因、物理输入或Q07总出口。未跑新Unity/EXE也未改代码资源；详同ID REPORT/JSON。

> Q07当前资源字节复核 `NTSD28-Q07-CURRENT-SELECTED-CONTENT-BYTE-GATE-001 / VERIFIED_SCOPED_SELECTED_BATTLE_BYTE_GATE`：非排除DAT338/353及PNG1031/1255的当前公共路径SHA差异均0；缺15 DAT属用户例外/保留Menu/stage暂缓，缺224 PNG为排除背景110与精确114 sprite。114中106受保护或例外、8为所检playable目录声明未选读；活动战斗WORDS0..5/SPARK七图逐SHA在位。根EXE GUI和其它读者未证，8行保持不确定，不能盲拷贝/删旧文件；下一转自然技能/生命周期与旧动态读者。Q07/R17/总目标继续开放，详同ID REPORT。

> Q07 Lee背景合成世界相机 `NTSD28-Q07-LEE-CHILD-COMPOSED-WORLD-CAMERA-001 / VERIFIED_SCOPED_UNITY_COMPOSED_WORLD_CAMERA_PIXEL`：原Editor正式内容/保存Battle Scene普通J→L五OID204出生、五中央命令；保留相机原culling/clear/HDR/MSAA的tick11/12双PNG，首子体ROI600变化、574与隔离捕获同坐标，原项目背景清晰存在。相机恢复/借用0、Editor EditMode/Console0错、双Scene SHA稳定。只改Editor诊断，无生产/资源/Scene/非战斗改动。完整Game view、物理键、子体寿命和正式EXE同条件GPU仍待；Q07/D-024开放，详COMPOSED-CAMERA-ANALYSIS。覆盖下条预登记前态。

> Q07 Lee正常背景世界相机门 `NTSD28-Q07-LEE-CHILD-COMPOSED-WORLD-CAMERA-001 / IN_PROGRESS`：隔离中央相机像素已证，场景层遮挡未证。Task/Change/Ledger已在脚本前登记，下一只扩既有Editor探针的独立可选模式，原Battle Scene普通J→L出生前后保留世界相机原层/清屏/HDR/MSAA并读回，独立解码背景与首子体ROI；旧模式、生产、DAT/图片、Scene/非战斗保持。Q07及正式EXE同画面未关闭。

> Q07 Lee子体像素限定 `NTSD28-Q07-LEE-CHILD-CAMERA-PIXEL-001 / VERIFIED_SCOPED_UNITY_ISOLATED_CENTRAL_CAMERA_PIXEL`：原Editor Battle Scene普通J→L自然OID204/pic28五命令；tick11/12隔离中央世界相机PNG在首子体ROI58×58由0→574变化非黑，独立Pillow读回288/574精确正式pic28 RGB、471/574色距≤5；正式/暂存cha.png同SHA。相机/RenderTexture恢复，关闭借用0、Editor非Play/Console0错、双Scene SHA稳定。首编译CS0165已修后再Play；无生产/DAT/Scene/非战斗改动。正常背景合成、物理键、子体寿命/交互及正式EXE同相机GPU A/B未证，Q07/D-024仍开。详PIXEL-ANALYSIS；覆盖下条待像素快照。

> Q07下一 `NTSD28-Q07-LEE-CHILD-CAMERA-PIXEL-001 / IN_PROGRESS`：五个OID204自然出生/中央命令已证但像素未证。已先建Task/Change/Ledger；仅扩Lee Editor探针可选双帧相机读回、正式图格/命令ROI和恢复，独立PNG分析后判实际可见。原Editor当前Battle EditMode/Scene SHA稳定；不改生产/DAT/Scene/非战斗，不用computer-use/第二Editor。Q07/D-024总出口及正式EXE画面仍待。

> Q07 Lee Battle Scene Play限定通过：`NTSD28-Q07-LEE-JL-BATTLE-PLAY-001 / VERIFIED_SCOPED_BATTLE_PLAY_NATURAL_CHILD_PUBLICATION` 原Editor run3正式内容45完整Driver tick PASS，J@8/InputPhase0、L@9–10令Lee frame145/PP300，tick12五个OID204源(437,651)、物理(403,-35,651)、五条中央Entity命令；有序关闭/借用0、退出Play、Console0错、双Scene SHA稳定。run1相位/run2夹具源坐标问题保留FAIL。下一 Q07 可转同链真实相机像素或物理输入门；不得把命令当像素或关闭Q07/D-024。生产/DAT/Scene/非战斗不改，详ACCEPTANCE；覆盖下条run2待run3快照。

> Q07 Lee原Battle Scene run2：J@完成tick8/InputPhase0、L@9–10自然到frame60→61→145→146→164，tick12五个OID204/五条中央Entity命令已证；终态FAIL只在新探针未按AppManager生产出生设置源规则坐标。探针记录MP500是错误字段，实际成本/正式raw是PP；不得判生产漏扣费。run1/run2保留，原Editor退出Play、关闭完整/借用0/双Scene SHA稳定。下一仅修探针出生与PP记录并独立run3；生产/DAT/Scene不改，Q07/D-024及像素总出口仍待。

> Q07 Lee原Battle Scene首次Play `NTSD28-Q07-LEE-JL-BATTLE-PLAY-001`：编译0错，45tick完整Driver跑完但首结果FAIL。probe从tick5开始，J落在完成tick7且Lee仍frame0，L@8进入frame110，OID204未生；前包native/Unity EditMode的J在完成tick2。此为待核输入相位夹具差异，不报生产bug。Play退出、有序关闭/借用0、双Scene SHA稳定，run1结果保留。下一只调整新探针为至少一个idle后偶数完成tick送J、随后的L，记录InputPhase并独立run2；不改生产/资源。Q07/D-024整体开放。

> Q07下一运行 `NTSD28-Q07-LEE-JL-BATTLE-PLAY-001 / IN_PROGRESS`：原Editor在干净保存的Battle Scene；Lee J→L原始状态限定出口已证。新Editor-only Play探针的Task/Change/Ledger已先建，下一编译后仅在原项目完整Driver中测自然OID204出生/源/表现载体/有序关闭；严禁帧注入、DAT/生产/Scene/非战斗修改或第二Editor。Play/物理键/正式像素仍待，Q07/D-024总出口开放。

> Q07当前：`NTSD28-Q07-LEE-JL-UNITY-RAW-FIRST-DIFF-001 / VERIFIED_SCOPED_EDITMODE_JL_ENTITY_RNG_PARITY` 原项目Editor第二轮45tick完整Driver PASS；正式EXE LFR对照的输入相位/占用实体映射2557/2557同值，同步RNG 45/45 tick的counter/index/calls/site同值；tick6五个OID204/action20/owner0子体slots51–55、坐标(437,-35,651)同值。Run1额外随机BGM抽数仅为新诊断夹具前置，已只在Lee schema校正并保留旧场景。原Temp结果、双Scene SHA稳定，生产/DAT/非战斗未动；根LFR默认CRT seed0与source/Unity seed682973786不可据此报CRT初态对齐。下一同序列进入原Battle Scene Play，验证自然发布/回收和限定像素；物理键/正式画面及Q07/D-024总出口开放。详同ID ACCEPTANCE，覆盖下条run1待run2快照。

> Q07 `NTSD28-Q07-LEE-JL-UNITY-RAW-FIRST-DIFF-001 / IN_PROGRESS` run1：原Editor已编译新schema并完成45tick EditMode完整Driver，2557个映射输入相位/实体字段与正式EXE trace全同；tick6五个OID204同slot/action/坐标。已定位诊断夹具随机初态偏移（旧raw reset多一次BGM随机选择，native显式BGM2；LFR回放CRT默认seed0），下一仅为本Lee schema改用无BGM draw的ResetFromSeed，保留旧场景与run1，做唯一run2。生产/Scene/DAT未改，Play和Q07整组开放。详RUN1-ANALYSIS。

> Q07下一执行 `NTSD28-Q07-LEE-JL-UNITY-RAW-FIRST-DIFF-001 / IN_PROGRESS`：原Editor raw capture已查可用正式内容/完整Driver但旧schema锁定3或26tick；Task/Change/Ledger先建，下一仅加精确45tick Lee J@2/L@3–4 schema和独立结果路径，保留既有Temp结果及旧场景。先取得EditMode原Unity首差，之后才考虑生产修复与真实Battle Play；Q07仍开。

> Q07/D-024 `NTSD28-Q07-LEE-NATURAL-NONSOUND-CHILD-001 / VERIFIED_SCOPED_FORMAL_SOURCE_RELEASE`：Lee普通J@tick2、L@tick3–4从完整GameSession经frame145→146→164，在tick6自然生成五个OID204/action20/owner0；根正式EXE回放45tick通过，source/release输入相位、Lee动作/MP和owned子体数180/180相等。下一同输入在原Unity Editor完整Driver检子体位置、表现及清理；物理键和更广正式画面仍待。旧AI600tick只到音效OID814，Q07/D-024总出口开放；详同ID ACCEPTANCE。下条“待编译/回放”已被覆盖。

> Q07/D-024当前执行 `NTSD28-Q07-LEE-NATURAL-NONSOUND-CHILD-001 / IN_PROGRESS`：正式Lee frame60/61等的hit_ad145经native J→L历史可选，145→146→164有非音效OID204/action20；先前自然AI600tick只达纯音效OID814、frame22 OID706仅受控。已先建准确Task/Change/Ledger，下一只新增一个Tools诊断CPP，以正常输入搜有界自然链并回放根EXE；无帧注入/生产/DAT/Scene/非战斗修改。Unity同条件Play和Q07总出口仍待。

> Q07/R17最新越图集帧门 `NTSD28-Q07-OUT-OF-RANGE-FRAME-VISIBILITY-AUDIT-001 / VERIFIED_STATIC_VISIBILITY_SCOPE_ONLY`：正式根EXE身份重核；330份正式/暂存对象DAT逐SHA同值、最大累计图格容量560；旧清单84个非999越界base pic在21份DAT中重算84/84越界。所检playable图片偏移生产写者仅零或正值，无法把这些pic移回范围，正式resolver不发body sprite；不作为待补图。六个范围内但超PNG格子、自然可达性与根EXE/Unity同条件像素仍待，Q07/R17/BATCH-04开放。详同ID Task/REPORT；本包只改诊断与进度文档。

> Q07/R17最新静态图片门 `NTSD28-Q07-REMAINING-SPRITE-CONSUMER-AUDIT-001 / DELIVERED_SCOPED_STATIC_CLASSIFICATION`：正式缺114张`sprite/*`已逐路径、正式SHA和DAT声明归属；106张为受保护原生菜单/加载/结果/HUD或P-14例外，8张仅声明无已证选中playable PNG消费者（`UI/extra/*`7、`PAUSE.png`1）。已选战斗WORDS六张/SPARK一张正式与暂存逐SHA相等。禁止把声明当消费或按数量导入；8张的根EXE可见性、Q07旧动态reader/521旧资产、自然技能和R17像素继续开放。详同ID REPORT/CSV。原Editor项目及用户资源未改。

> Q09相机诊断最终文件：原项目程序集晚于最后一行脚本，第三次唯一请求`platform-shadow-camera-03`在原Battle Scene给出PARTIAL/相机恢复累计true/平台载体与中央命令同值/独占像素0；清理/双Scene SHA稳定。显式选中原Editor核到非Play、Console错误0。MCP默认连接曾短暂落到别的已开Editor，未对其执行Play或Scene/内容写入；以后工具调用须显式核实例。Q09仍开放，详同ID ACCEPTANCE。

> Q09 `NTSD28-Q09-FORMAL-PLATFORM-SHADOW-CAMERA-WITNESS-001 / RUNTIME_PENDING / PIXEL_OWNERSHIP_UNPROVEN`：原Editor编译/两次受控正式OID56完整Driver+中央命令Play完成；第二次JSON顶层PARTIAL。1920×1080白底前后相机PNG已查看，阴影ROI53×11被其它命令框覆盖，独占像素0，不能认定目标阴影GPU归属。相机/fixture/World/池恢复、双Scene SHA不变、Editor非Play、Console错误0；后继需独立可分辨像素见证，不可修改10C后伪造同tick帧。Q09/总目标开放，详同ID ACCEPTANCE。

> Q09 `NTSD28-Q09-FORMAL-PLATFORM-SHADOW-CAMERA-WITNESS-001 / CODE_WRITTEN / COMPILE_PENDING`：已仅扩既有opt-in Editor探针的前后相机读回、其他命令投影排除与独占像素计数，旧请求默认不变；原Editor编译/Play待。DAT/Scene/生产/非战斗未动，Q09/总目标开放。

> Q09 `NTSD28-Q09-FORMAL-PLATFORM-SHADOW-CAMERA-WITNESS-001 / IN_PROGRESS`：前一子Task已证原Editor正式OID56完整tick产10C=-5和目标中央阴影命令；下一仅扩同一Editor诊断脚本的可选相机基线/后帧白底读回与保守命令排除。Task/Change/Ledger先建，脚本/编译/Play待；DAT/Scene/生产/非战斗不改，Q09/总目标开放。

> Q09 `NTSD28-Q09-FORMAL-PLATFORM-DRIVER-PLAY-001 / VERIFIED_SCOPED_FULL_DRIVER_CENTRAL_COMMAND`：原Editor正式OID56 frame130→异组OID2受控完整Driver tick6写平台slot50/10C=-5，中央目标Shadow命令1条、Z287/Y(Z-5)；对象/slot/池及双Scene哈希稳定，已退出Play。五条UnityMCP重连disposed-object日志另记；自然/Legacy Play/相机/EXE仍待，Q09总出口开放。原始JSON及范围见同ID ACCEPTANCE。

> Q09 `NTSD28-Q09-FORMAL-PLATFORM-DRIVER-PLAY-001 / CODE_WRITTEN / COMPILE_PENDING`：原Battle Scene正式OID56操作30受控完整Driver Play证10C/中央阴影；仅新增opt-in Editor探针/Meta。脚本前Task/Record/Ledger已注册，编译/Play待；不动DAT/Scene/生产/非战斗，Q09/总目标开放。详同ID Task。

> Q09下一受控Play入口：`NTSD28-Q09-FORMAL-PLATFORM-REACHABILITY-AUDIT-001 / STATIC_FORMAL_CONTENT_CANDIDATE`只读核到正式OID56 Hidan `rea.dat` 17条kind50平台ITR（5条被抓、12条下落），DAT/data.txt与原版逐SHA相同。不同组type0/3可归一operation30，仍须完整Driver证实际frame/snapshot、10C写入、双表现出口与相机像素；自然输入/EXE画面未证。详同ID REPORT。Q09/总目标开放。此前批准的三文档v3恢复已安装，现有后续增量不得被旧候选覆盖。

> Q09影子offset `NTSD28-Q09-PLATFORM-SHADOW-OFFSET-PUBLICATION-001 / RUNTIME_PENDING`：Legacy `UpdateShadow`与中央snapshot/Shadow命令只消费现有10C字段，实体Z/排序/ground foot-marker anchor保持。原Editor编译0错，定向Legacy+中央2/2；相邻类6/7一项旧com-label反射前置FAIL，Battle Play画面/正式EXE未验。Scene SHA稳定，DAT/PNG/Scene/非战斗未改。详同ID `ACCEPTANCE-20260926.md`。
...
> 2026-09-24 D-024最新：原Editor Play d024-ai-child-play-10 证明 DataOrientedCanonical indexed 最近目标检索在tick76/90/120/180各执行1次，但Unk360仍-1、无子体；提交phase call计数0不代表未提交（生产未记录call）。停止重复180tick Play，下一步先做同状态双profile候选/提交首差，读取现有committed/fallback计数。Battle Scene两Camera当前磁盘Enabled=1，原Editor退出Play后可重新加载clean Battle；Scene哈希保持。详NTSD28-USER-D024-AI-CHILD-SCENE-PLAY-001 Task/Record。

> 2026-09-24 最新恢复点：用户确认 Battle Scene 两 Camera 默认启用；当前磁盘 HUDCamera/ScenesCamera m_Enabled=1，原 Editor 重新载入 Battle 时 Scene clean，Menu/Battle SHA 为 785F828...81E13 / 471396E7...7B9。D-024 原项目 Play 探针 d024-ai-child-play-9 完成正式内容与 180 个完整 Driver tick，AI 诊断计数每 tick 1、目标始终 -1、无子体；tick76 后玩家 HitStop0/HP500/RelationTeam1/type0。需同状态 Legacy/DataOrientedCanonical 首差，不得判生产修复或 Q07 解锁。Task/Change NTSD28-USER-D024-AI-CHILD-SCENE-PLAY-001 仍 RUNTIME_PENDING；只改测试探针与文档，未改 Scene、DAT 或生产战斗脚本。OID219/fusion/普通复活已有聚焦修复、整链待验，旧未修措辞被本条覆盖。
...
> 2026-09-24 latest D-024 AI full-Driver: controlled formal-content two-participant held-right 12-tick original Editor job `c1b53cb922a142ab880a9cf2d7100f81` PASS. The legacy replay fixture bypassed AppManager, so its initial missing source carriers were seeded only inside the new test; later full ticks kept every active source carrier initialized and approved physical X diverged from source X. This is not natural Battle Scene AI participant, dynamic child birth or formal EXE parity. Prior two-profile AI4/4, adjacent13/13, zero-allocation1/1 and SelfCheck PASS remain; noncharacter X-edge decision pending; Q07 paused. See Task/Change.

> 2026-09-24 latest D-024 character AI: `NTSD28-USER-SOURCE-CHARACTER-AI-DOMAIN-001 / FOCUSED_TEST_PASS`; production DataOriented/Legacy input, sensing/index, nearest-facts and decision/MoveMode shadow use one source-rule X/Z domain when complete or physical X/Z for the entire pass otherwise. Direct Legacy facts RED observed source X120 versus physical X200; strengthened original Editor 4/4 actual input/facts, adjacent 13/13, warmed 128 inputs 0 B allocation 1/1 and post-fix standard BattleRuntimeSelfCheck local 13:02:44 PASS. Full Driver/natural Play/EXE and all source writer coverage remain open; noncharacter X-edge preference pending. Q07 paused. See Task/Change.

> 2026-09-24 current D-024 focused state: original-project Editor PID10576/bridge6403, C: free recovered; independent test-project Editor PID14952 is excluded. Kind14 integer source correction exact job23375124 11/11 PASS; OID998 source-birth matrix job0a019ffb 4/4 and final C07 class job6f5089ef 7/7 PASS after test-only stale phase assertion correction. Old broad job2ed78c0b lost with PID288224 without trusted terminal result. `NTSD28-USER-SOURCE-COORDINATE-FRAME-MOTION-001` remains untested; D-024 source physics/flags, OID219/fusion, full Driver/Play/EXE and Q07 open.

> 2026-09-24 C07 phase-fixture correction FOCUSED_TEST_PASS: `NTSD28-USER-C07-REVIVAL-PHASE-ASSERTION-ORDER-001` changed only old diagnostic slot28/count34 expectation to FrameAdvance27/Stage28/count33; focused1/1 and final C07 class7/7 PASS. No production edit under this ID. Source birth Play/EXE and ordinary revival average remain open.

> 2026-09-24 bounded D-024 package: `NTSD28-USER-SOURCE-COORDINATE-FRAME-MOTION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE` owns only `LF2Entity` source X/Z writes for linked-platform and frame dx/dz. Original Editor PID10576 exact frame/platform14/14 and adjacent full tick/replay3/3 PASS. Physical D-024 scale, DAT, Y, Scene and nonbattle remain unchanged; source readers stay inactive until physics/teleport/bounds/flags are complete. Formal EXE/Play still pending. See Task/Change.

> 2026-09-24 D-024 kind14 precise-position correction (historical, superseded): original Editor job cbe5118f 2/2 characterized X5.5/int5 missing block; formal-expected RED 7e9ebbb7 then active/projection/three fallbacks corrected to precise X/Z, GREEN 7415d299 7/7 plus direct C32 b2274720 1/1. The expected precise formal rule was wrong; these passes do not prove parity. Source-rule scaled-view coordinate/flag carrier, OID219/fusion, full Driver/Play/EXE remain open; Q07 paused. See successor correction Record.

> 2026-09-24 D-024 non-perceptual kind14 gate: formal playable compares precise X/Z and publishes once-consumed directional block flags; Unity active BoundaryWriter/ECS hit projection compares battle integer X/Z. Factor1 rounding and scaled-view source/battle flags are static mismatch candidates, not yet an official-content runtime hit witness. A source-rule carrier must own counterfactual collision flags and their lifecycle, not merely raw coordinate fields. OID219/fusion first differences remain, Q07 paused. See KIND14-COORDINATE-DOMAIN-AUDIT.md.

> 2026-09-24 D-024 CPoint throw-position two-view witness FOCUSED_TEST_PASS: original-project Editor exact job a75fa674dcb1434d9324c5a5fa351453 2/2; formal row0 actual throw writer gives catcher/caught integer X148/159 and173/184, raw local gap11, caught Y-24/Z200 and Vx1.5/Vy-2.25. Dual-coordinate carrier, OID219/fusion and full Driver/Play/EXE open; Q07 paused. See CPOINT-THROW-POSE-TWO-VIEW-WITNESS.md.

> 2026-09-24 D-024 CPoint held-pose two-view witness FOCUSED_TEST_PASS: original-project Editor exact job b1e0ecd2071448b580d1b62e66b45f0a 2/2; formal row1 actual held writer gives catcher/caught integer X348/373 and373/398, same raw local gap25 and caught Y4/Z249. Throw and nonzero CPoint Z unverified; OID219/fusion two-domain history and full Driver/Play/EXE open, Q07 paused. See CPOINT-HELD-POSE-TWO-VIEW-WITNESS.md.

> 2026-09-24 D-024 WPoint type1 weapon-component witness FOCUSED_TEST_PASS: original-project Editor exact job 13805c76522f442cac6bc7917647d39b 2/2; formal row21 active `LF2WeaponBase` writer has holder/held integer X348/359 and373/384 at factor1/configured view, same raw local gap11 and Z251. Focused pose arithmetic only; CPoint, OID219/fusion two-domain history and full Driver/Play/EXE remain open, Q07 paused. See WPOINT-WEAPON-COMPONENT-TWO-VIEW-WITNESS.md.

> 2026-09-24 non-character early StageZ correction FOCUSED_TEST_PASS: paired playable twice clamps all active entities with type0/other margin0/±1; Unity had filtered the matching early passes to type0 and deferred nonchar to PreFrame. Original Editor RED type1 Z500 vs expected351; default DataOriented plus Legacy/Shadow production correction, initial 4/4 and final related StageZ/StageBounds/WPoint 16/16 PASS. DAT/Scene/camera/nonbattle unchanged; full Driver/hit/Play/EXE and source-rule carrier pending, OID219/fusion open, Q07 paused. See NONCHAR-EARLY-STAGE-Z-ACCEPTANCE.md and Task/Record.

> 2026-09-24 D-024 WPoint non-weapon-component pose witness FOCUSED_TEST_PASS: original Editor job bdb426058e5f448da3a79d711c11d9f4 exact 2/2, unchanged formal row1 holder raw Vx48 then actual HeldObjectProcessAll; factor1 holder/held XInt348/359 and configured373/384, same local gap11, ZInt251. Active LF2WeaponBase path through LF2WeaponHeldStateResolver remains a separate writer. Read-only pass audit also found paired playable twice clamps stage depth for all active entities, while Unity's corresponding early passes filter to type0 and nonchar waits for PreFrame; successor NONCHAR-EARLY-STAGE-Z-001 has now focused-corrected that first difference. Test/doc only in this historical phase; OID219/fusion open, Q07 paused. See WPOINT-POSE-TWO-DOMAIN-WITNESS.md and POSITION-WRITER-OWNERSHIP-GATE.md.

> 2026-09-24 D-024 independent StageZ witness FOCUSED_TEST_PASS: original Editor final strengthened job ddce615b36c54e6b902682f627431db7 exact 2/2 with independent view-factor and pre-bound numeric Z assertions; earlier b4333ba62f464c2385c206f329e05bc7 was 2/2 before strengthening. Same raw startZ300/Vz40 gives factor1 Z340 inside far350, configured physical Z363.123... before default StageZ clamp to350. Future source-rule reference needs independently ordered Z clamp and mirror; test-only change in BattleEcsCharacterStageZPassEditorTests. DAT/Scene/camera/nonbattle unchanged; non-character Z/Play/formal EXE and carrier pending, OID219/fusion open, Q07 paused. See STAGE-DEPTH-TWO-DOMAIN-WITNESS.md and Task/Record.

> 2026-09-24 D-024 kind1 OPoint random X/Z birth outlet FOCUSED_TEST_PASS: original Editor RED job 126a974eb91243fba35b1eaa740971b0 raw X delta147 vs configured expected225.848462, then shared writer one-time X/Z output factor, strengthened job 99d1c0184d6245dfb8232342eb223661 1/1 and original-source World/component row0 job 840455c81a6349c89a9efcc35f418ab3 2/2 PASS. DAT, Y/action and native RNG order unchanged; Scene SHA unchanged; Ledger732/22 PASS. Actual Yagura418 Play/EXE and source-rule carrier pending; OID219/fusion open, Q07 paused. See KIND1-OPOINT-RANDOM-BIRTH-RATIO-ACCEPTANCE.md.

> 2026-09-24 D-024 kind1 OPoint random-birth package PLANNED before script edit: NTSD28-USER-OPOINT-KIND1-RANDOM-BIRTH-RATIO-001 owns only BattleNativeOpointBirthWriter and its existing Q06 materializer test. Formal playable kind1 order is random X/Y/Z/action, official Yagura418 has nonzero X/Z amplitudes. Run configured-view RED before changing the shared final birth outlet; preserve DAT/RNG/Y/action and factor1 behavior. OID219/fusion and coordinate-history carrier open; Q07 paused.

> 2026-09-24 D-024 grab-pose focused witness: original Editor job 6fe9428e5d1f45ea9bd5e4049b3eb22e exact two cases 2/2 PASS. Target raw Vx48 from X140 yields factor1 pre-grab X188 and post-grab attacker/target X148/140; configured Sx2048/1333 yields pre-grab X213 and post-grab X160.5/152.5. Both local gaps -8. This resolves the grab pose writer formula gate only, not OID219/fusion or visual contact. Test-only; Q07 paused. See GRAB-POSE-TWO-DOMAIN-WITNESS.md and Change Record.

> 2026-09-24 D-024 grab-pose witness PLANNED: NTSD28-USER-GRAB-POSE-VIEW-WITNESS-001 predeclares one kind3 grab Editor test after factor1/configured target motion, with source local centerx/cpoint.x anchor and half-gap formula. Production, DAT, Scene, camera and nonbattle code are outside this package; OID219/fusion defects remain open and Q07 paused.

> 2026-09-24 D-024 stage-edge two-domain witness: original Editor job 57c29991fd964a428e6e856c41e1c209 focused 2/2, exact registered character + production mechanics + ECS bounds. Absolute stage width2048, startX2000, rawVx40: factor1 final2040; configured 2048/1333 pre-boundary2061.46 then clamp2048. This is the approved physical-distance exception, not a production bug; source-rule reference must retain independent raw position2040/bound rule. Only BattleEcsCharacterPreFrameBoundsPassEditorTests test changed under NTSD28-USER-STAGE-EDGE-HISTORY-WITNESS-001. OID219/fusion remain open, Q07 paused.

> 2026-09-24 D-024 stage-edge test PLANNED: NTSD28-USER-STAGE-EDGE-HISTORY-WITNESS-001 predeclares one BattleEcsCharacterPreFrameBoundsPassEditorTests method, factor1/configured width2048 start2000 raw Vx40, production mechanics plus ECS bounds. Expect formal raw2040 inside edge while configured battle reaches2048 clamp. Test-only; no production change. Carrier snapshot/reset/checksum and OID219/fusion remain open; Q07 paused.

> 2026-09-24 D-024 coordinate-history writer gate read-only: POSITION-WRITER-OWNERSHIP-GATE.md now traces active absolute/physical/relative/relation/attachment/bounds writers, PS.x/z and direct task aliases, excludes uncalled legacy grab/Coincide helpers, and specifies NTSDEntityRuntime copy/reset/checksum ownership. Scaled battle body can hit unchanged stage edge before formal raw-reference body; boundary flags require explicit two-domain contract before source-history carrier code. OID219/fusion confirmed defects OPEN; Q07 paused. No script/DAT/Scene/camera edit in this pass.

> 2026-09-24 D-024 state9996 child ratio focused exit: source-row parent raw20px movement plus five valid OID217/218 births in original Editor; configured strict RED child X327 vs target325.39084771192796 (job f15b213dffd9458a8d5fd73e3606f0d1). Shared BattleLateEntityLifecycleModule writer now scales only relative X and projected Z at task birth; final five-child/default+configured job f1dc8c0b632a43d4a36c33380a865e53 2/2, default source-row immediate/following eae331fdc81a48958c91e511d9da16be 2/2. No DAT/Scene/camera/nonbattle diff. Play and source-rule reference history pending; OID219/fusion confirmed defects keep Q07 paused. See NTSD28-USER-STATE9996-CHILD-RATIO-001.

> 2026-09-24 D-024 state9996 child ratio fix PLANNED: first-difference report and original Editor 2/2 characterization confirm configured child relative X remains -3 pixels (-3/2048 versus formal -3/1333). New NTSD28-USER-STATE9996-CHILD-RATIO-001 Task/Change/Ledger exists before script edits, scope only BattleLateEntityLifecycleModule shared spawn and declared Q06 test. Test precise X/Z RED first, then one final relative-birth scale, preserving RNG/Y/velocity/parent absolute. OID219/fusion history still open; Q07 paused.

> 2026-09-24 D-024 state9996 child birth witness PLANNED: one test method in NTSD28Q06State9996DirectSpawnEditorTests.cs will use exact source row0 and native clone boundary after production raw20px parent step, compare relative child0 X=-3 screen fractions at factor1 and 2048/1333. Task/Change/Ledger exist before script edit. No production/DAT/Scene/camera change. OID219 and fusion defects remain open; Q07 paused.

> 2026-09-24 D-024 fusion proximity first difference: original Editor job 42504562540d48f29a0f42433c24eced exactly 2/2 passed as defect-characterization. Exact formal fusion row0 OID7+8 after production 20px character step: factor1 primary/partner XInt340/300 gap40 merges, configured 2048/1333 primary/partner350/300 gap50 fails production fusion scan. Playable source expects raw gap40 merge. Only one Editor test method added under NTSD28-USER-FUSION-VIEW-DISTANCE-WITNESS-001; production unchanged. Do not patch threshold alone; OID219 same-current-gap histories require shared source-rule reference coordinate. Q07 paused. See FUSION-VIEW-DISTANCE-FIRST-DIFFERENCE.md.

> 2026-09-24 D-024 fusion witness PLANNED: NTSD28-USER-FUSION-VIEW-DISTANCE-WITNESS-001 limits the next script edit to one focused test in NTSD28Q06FusionRecordTransactionEditorTests.cs using exact formal fusion bytes, valid source row, factor1/configured World, and production fusion scan. Capture raw gap40 versus scaled integer gap50 and actual merge result; do not change production under this Record. OID219 history defect remains open; Q07 paused.

> 2026-09-24 D-024 writer follow-up: same-current-gap OID219 first difference remains confirmed by original Editor; no scalar current-gap correction can close it. X-WRITER-SEMANTIC-INVENTORY.md traces PS.x/SetPos aliases, active fusion X<50/Z<8 proximity/midpoint and state9996 parent-relative spawn. Fusion is static parity risk, not runtime-proven failure. Next: focused fusion/clone witness, finish writer/caller matrix, then Task/Change for dual position-history carrier with reset/snapshot/checksum and Play. No script/DAT/Scene/camera change this pass; D-024 OPEN, Q07 paused.
...
> 2026-09-24 D-024 attachment read-only classification: active BattleHeldObjectWriter/BattleCpointWriter and paired playable BattleWorld28 use holder position plus frame-local wpoint/cpoint geometry; keep raw local anchors pending focused pose test, because scaled holder travel already carries the held entity. Stage bounds and Y/floor still open. This does not release Q07.
...
> 2026-09-23 用户试玩反馈：此前鸣人落地奔跑和螺旋丸续按攻击体感基本正常，允许接续Q07。此为用户确认，不替代正式EXE可见或全实体D-024证明。Q07重新成为当前游标；已知错过转招后OID434 motionX首差、全实体D-024剩余项保持独立回访，不阻断Q07内容包。当前磁盘复核正式DAT405/暂存338/缺67，正式PNG1255/暂存1031/缺224（背景110、sprite114）；先按战斗可达消费者和资源owner分配，不盲补Menu/UI或删旧资源。DAT数据不改、禁computer-use/第二项目、非战斗和双Scene保护。
...
> 2026-09-24 D-024当前恢复点：用户要求先排查非体感、全实体位移比例同类问题，确认无问题才继续Q07。已实测OID219历史速度与fusion距离两项未修首差，不能解锁Q07；九条通用位移出口与state9996子体已有定向修复，但不代表全域完成。新只读`artifacts/diagnostics/NTSD28-USER-ALL-ENTITY-MOTION-RATIO-001/COORDINATE-CARRIER-STATE-VERSION-GATE.md`确认运行时、entity/raw快照、aggregate、checksum和版本17/28/31的原状；后续双坐标载体必须同包覆盖复制/重用/校验，先补writer历史及stage/attachment语义。此轮无脚本/DAT/Scene/相机/非战斗修改。
> 2026-09-24 D-024 read-only carrier gate: `POSITION-WRITER-OWNERSHIP-GATE.md` now records `NTSDEntityRuntime.SyncIntegerPosition` battle-only early return, `PhysicsState.BindRuntime` rebind history, explicit OPoint initial-int materialization early return, common kind1 random draw, multiple-task compatibility reachability, and the separate default `BattleEcsCharacterStageZPass` precise/int writer. Paired playable `clamp_type0_stage_depth` is in the same formal stage-bound pass; source EXE SHA rechecked B1E13AE1...19033. Carrier acceptance must test battle-int-stable/reference-int-changing, rebind with history, explicit birth integers, plus non-character Z and held-pose reachability. No script/DAT/Scene/camera edits; OID219/fusion still defective, Q07 paused.
> 2026-09-24 D-024 StageZ witness PLANNED before script edit: NTSD28-USER-STAGE-DEPTH-HISTORY-WITNESS-001 owns one parameterized test in existing BattleEcsCharacterStageZPassEditorTests. Paired playable `clamp_type0_stage_depth` and Unity default StageZ route traced; raw Z300+40 remains under far350, configured physical `1152/730` step should clamp to350. Next run exact two cases in original Editor. No production/DAT/Scene/camera/nonbattle changes; OID219/fusion open, Q07 paused.
> 2026-09-24 D-024 WPoint pose witness PLANNED before script edit: NTSD28-USER-WPOINT-POSE-VIEW-WITNESS-001 owns one parameterized test in existing NTSD28Q06HeldNativeFrameBindingEditorTests. Reuse formal source row1: static holder X300 yields held X311/Z251; move holder by raw Vx48 at factor1/configured view, then actual HeldObjectProcessAll, checking local holder-held gap11. No DAT/production/Scene/camera/nonbattle edits; original Editor pending, OID219/fusion open, Q07 paused.
> 2026-09-24 non-character early StageZ package IN_PROGRESS before script edit: NTSD28-USER-NONCHAR-EARLY-STAGE-Z-001 owns default/Legacy early depth clamp and two existing Editor test files. Paired playable twice scans all active entities at depth bounds (type0 no margin, others ±1); Unity's matching two passes filter to type0 and nonchar waits for later PreFrame. First capture registered-World RED, then correct only those battle passes and rerun focused/held regressions. DAT/Scene/camera/nonbattle and D-024 scale unchanged; OID219/fusion open, Q07 paused.
> 2026-09-24 D-024 WPoint type1 weapon-component view witness PLANNED before script edit: `NTSD28-USER-WPOINT-WEAPON-COMPONENT-VIEW-WITNESS-001` declares one existing Q06 Editor test, unchanged formal row21/type1, production holder physical travel then actual held pass via `LF2WeaponHeldStateResolver`, and factor1/configured local gap11. No production/DAT/Scene/camera/nonbattle edit. OID219/fusion open, Q07 paused.
> 2026-09-24 D-024 CPoint held-pose two-view witness PLANNED before script edit: `NTSD28-USER-CPOINT-HELD-POSE-VIEW-WITNESS-001` declares one existing Q06 CPoint Editor test, unchanged formal row1, production catcher raw Vx48 travel followed by active `RunWeaponSyncHeldStep10`; expected local gap25/Y4/Z249 in factor1/configured views. No production/DAT/Scene/camera/nonbattle edit. OID219/fusion open; Q07 paused.
> 2026-09-24 D-024 CPoint throw-position two-view witness PLANNED before script edit: `NTSD28-USER-CPOINT-THROW-POSE-VIEW-WITNESS-001` declares one existing Q06 throw Editor test, unchanged formal row0, production catcher raw Vx48 travel then actual `RunCpointAdvanceStep10`; expected local gap11/Y-24/Z200 in factor1/configured views. No production/DAT/Scene/camera/nonbattle edit. OID219/fusion open; Q07 paused.
> 2026-09-24 D-024 kind14 test-only package PLANNED before script edit: NTSD28-USER-KIND14-PRECISE-THRESHOLD-WITNESS-001 adds one existing hit-plan Editor test method for precise X5.5/int5 and control X6.5/int6 with a real candidate and production dispatch. Formal source precise condition predicts a block at5.5; Unity integer producer predicts none. Compile/focused runtime pending; OID219/fusion and Q07 remain open.
> 2026-09-24 D-024 kind14 precision correction package PLANNED: NTSD28-USER-KIND14-PRECISE-COORDINATES-001 predeclares active BoundaryWriter, ECS projection, three fallbacks and focused test. Original Editor job cbe5118f13804bc0ad603aa023dba18d 2/2 characterized X5.5/int5 current no-block versus formal precise block. Test-first RED and correction pending; source-rule history/OID219/fusion and Q07 remain open.
> 2026-09-24 D-024 handoff 恢复：本轮追加状态行的字节写入操作损坏了本文件及 STATE.md；损坏文件已另存备份，正文从 03:18 完整 Git 对象快照恢复，以下依据 03:18 后的 Task/Change Record 与实际测试补记。恢复后须重跑 Change Ledger validator；请勿把载体聚焦通过解释成全链对齐。

> 2026-09-24 D-024 当前门禁：Q07 暂缓；用户要求先查非体感和原始像素/视口比例问题。OID219 hit_Fa5 与 fusion 首差仍开放；独立 source-rule 坐标 carrier 已建立但规则读者未激活，完整 Driver/Play/正式 EXE 对照未闭。DAT 数值不得修改，Camera/Scene/非战斗框架保持。下一步先完成源坐标写入/读取的聚焦包和真实原项目 Play 证据，再回 Q07。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-WEAPON-FRAGMENTS-001 / FOCUSED_TEST_PASS`：武器内置及 DAT 碎片源整数出生=父源整数+原始 dx/dz；RED 初始化4例失败、GREEN 12/12，编译和 Console 0 error，物理比例输出保持。源读者未激活，详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-STATE9996-BIRTH-001 / FOCUSED_TEST_PASS`：state9996 五子源坐标出生修正，RED 初始化2例失败、GREEN 6/6；详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-STATE18-BIRTH-001 / FOCUSED_TEST_PASS`：state18/19 粒子源坐标出生修正，RED 初始化2例失败、GREEN 6/6；详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-PARTICIPANT-BIRTH-001 / RUNTIME_PENDING`：菜单入战斗 participant 源绝对出生写入，RED 2/2失败、GREEN 2/2通过；真实 Menu→Battle Play 待验，详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-KIND1-RANDOM-001 / FOCUSED_TEST_PASS`：kind1 原始随机源整数更新，RED 3例失败、GREEN 14/14，RNG/物理输出保持；详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-BIRTH-FOUNDATION-001 / FOCUSED_TEST_PASS`：OPoint task、World/component、stage 绝对出生源坐标写入；Editor 22/22，原 Battle Scene Play 的真实 Renderer 池化出生 PASS，源/战斗整数分别(-42,126)/(312,259)，tick/checksum/借用数不变，Scene SHA 不变。详 Task/Record 与 renderer-play-pass.json。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-JSON-CHECKSUM-001 / FOCUSED_TEST_PASS`：源坐标 JSON/lockstep 诊断投影聚焦通过；载体未启用规则读者，详 Task/Record。

> 2026-09-24 `NTSD28-USER-SOURCE-COORDINATE-STATE-001 / FOCUSED_TEST_PASS`：源坐标和边界 flag runtime/snapshot/checksum 历史聚焦通过；OID219/fusion 仍开放，详 Task/Record。
> 2026-09-24 `NTSD28-USER-HITFA7-RAW-SLOT-TARGET-001 / FOCUSED_TEST_PASS / FULL_SELFCHECK_PASS / EXE_EMPTY_SLOT_PENDING`：OID875/action55 预分配空槽目标 raw X/Z 路径已由原 Editor RED（-1.4 versus0）、GREEN 2/2及最终完整 SelfCheck PASS 验证；正式 EXE 空槽可见行为和 D-024 双坐标/Driver 未闭。详 Task/Record 与 ACCEPTANCE.md。

> 2026-09-24 `NTSD28-USER-HITFA7-NONCHAR-SELF-CHECK-CORRECTION-001 / FOCUSED_TEST_PASS / FULL_SELFCHECK_PASS / TEST_ONLY`：修正 FL-02/HITFA-7 对非角色行为7凭空生成 clone 的旧自检期望；生产逻辑未动，原 Editor 完整 SelfCheck PASS。详 Task/Record。
> 2026-09-24 next D-024 package `NTSD28-USER-SOURCE-COORDINATE-REVIVAL-EFFECT-BIRTH-001 / PLANNED / CARRIER_NOT_ACTIVE`: formal playable continuation OID998/action6 child source precise/int comes from parent source integer X/Z with Z+1. Unity immediate task currently carries only physical position. Exact Task/Change exists before script edits; test-first actual factory RED/GREEN next. Normal revival average/RNG and N30 remain separate. Q07 paused.
> 2026-09-24 D-024 continuation effect source birth `NTSD28-USER-SOURCE-COORDINATE-REVIVAL-EFFECT-BIRTH-001 / CODE_WRITTEN / COMPILE_PENDING`: exact two-script Task/Change/ledger established. Actual logic-only factory fixture and source-integer OID998/action6 task write are in worktree; no formal DAT/Scene/camera/nonbattle edit. The original Editor has live accidentally unfiltered EditMode job `2ed78c0b98214a5ba14ec92250d3b9f8` (8420 total) because `run_tests` filter was incorrectly placed outside `params`. Do not run another test or call refresh until that same job is terminal; poll `get_test_job` by ID, then refresh compile and run groupNames regex for C07 only. Broad results are not package acceptance. Ordinary revival source sum_x/RNG gate awaits continuous source motion, OID219/fusion open, Q07 paused.
> 2026-09-24 kind14 authority correction: `NTSD28-USER-KIND14-INTEGER-OPERAND-AUTHORITY-CORRECTION-001 / CODE_WRITTEN / COMPILE_PENDING` supersedes the earlier precise-operand conclusion. Formal playable Position28 x/z are int; live kind14 uses those mirrors. Six gameplay branches and focused expectations corrected; Editor compile/NUnit blocked by still-running broad job `2ed78c0b98214a5ba14ec92250d3b9f8` and C: free 0. No second Editor/computer-use; await terminal handle and user response about generated log. D-024/Q07 open.
# 2026-09-24 D-024 source physics writer package

`NTSD28-USER-SOURCE-COORDINATE-PHYSICS-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: original Editor exact source physics7/7, weapon identity/type3 extras12/12 and adjacent ratio11/11; ledger755 PASS, Battle Scene SHA unchanged. Next source-first package is kind14 independent integer-coordinate directional flag production across active and fallback/ECS paths, then teleport/stage/relation writers. Source gameplay readers, OID219/fusion and Q07 remain gated; formal EXE/full Driver/Play evidence absent.
# 2026-09-24 D-024 source kind14 flag producer

`NTSD28-USER-SOURCE-KIND14-DIRECTION-FLAGS-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: central plus four fallback source integer ±5/±2 writers, source physics consumption, original Editor focused7/7 and ECS adjacency2/2 PASS. Physical flags remain unchanged. Next audit source-rule teleport/stage/relation X/Z writers; source gameplay readers, full Driver/Play/EXE and Q07 remain gated.
# 2026-09-24 D-024 source teleport writer

`NTSD28-USER-SOURCE-TELEPORT-WRITER-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: both state400/401 teleport outlets write independent source X/Z; final original Editor C05 class13/13 PASS. Physical scaled offset remains. Candidate ranking still uses physical X/Z and requires separate source-reader proof; Q07 paused. Next higher-priority writer: all-active StageZ and later PreFrame X/Z source-rule bounds, then relation/respawn and OID219/fusion.
# 2026-09-24 D-024 source stage-depth writer

`NTSD28-USER-SOURCE-STAGE-DEPTH-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: default/Legacy/Shadow StageZ and later PreFrame fallback independently clamp source Z with type0 margin0/other margin1. Original Editor new6/6 and existing9/9 including no allocation. Physical Z, DAT, Scene unchanged. Next bounded writer is source stage X; source gameplay readers and Q07 remain gated.
# 2026-09-24 D-024 source character stage X

`NTSD28-USER-SOURCE-CHARACTER-STAGE-X-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: exact and compatibility type0 source X clamp, original Editor new11/11 plus adjacent6/6 (no-allocation) PASS. Physical X stays unchanged. Noncharacter boundary death may diverge under approved physical scaling and remains a separate lifecycle gate; source readers/Q07 paused. Next audit relation/respawn/remaining birth and noncharacter death before switching OID219/fusion readers.

Noncharacter X-edge death product choice pending async user reply from 2026-09-24: formal source-rule edge keeps native lifetime but may go beyond visible edge; physical edge keeps visual boundary but shortens lifetime. Do not change noncharacter destruction until answered. Independent relation/respawn audit may continue.
# 2026-09-24 D-024 source kind8 relation position

`NTSD28-USER-SOURCE-KIND8-RELATION-POSITION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: active canonical kind8 writer now copies independent source precise X/Z with integer mirrors deferred to physics; final original Editor production class15/15 PASS. Legacy direct resolver reachability and other relation writers remain separate. Q07 paused.
2026-09-24 current D-024 cursor: `NTSD28-USER-SOURCE-GRAB-RELATION-POSITION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`. Shared kind1/kind3 source-rule X pair is written and original Editor final catch class23/23 PASS; source-domain facing decision, CPoint/WPoint follow-up relation writes and Q07 remain open. Next inspect formal held/throw attachment writes before activating OID219/fusion readers.
2026-09-24 D-024 `NTSD28-USER-SOURCE-CPOINT-HELD-POSITION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: source-rule held X/Z writer added, original Editor full settlement class7/7 PASS. Native nonzero CPoint Z versus Unity is a static branch difference with official DAT reachability unresolved, not a proven live defect. CPoint throw, WPoint held, source-facing/selection, OID219/fusion, Driver/Play/EXE and Q07 remain open.
2026-09-24 D-024 `NTSD28-USER-SOURCE-CPOINT-THROW-POSITION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: formal raw integer CPoint throw source X written and original Editor throw class8/8 PASS. Physical position/Y/Z unchanged; WPoint/source-facing, full Driver/Play/EXE, OID219/fusion and Q07 remain open.
2026-09-24 D-024 `NTSD28-USER-SOURCE-WPOINT-HELD-POSITION-001 / FOCUSED_TEST_PASS / CARRIER_NOT_ACTIVE`: canonical generic/weapon WPoint source X/Z added, original Editor two-view4/4, adjacent full tick2/2 and incomplete1/1 PASS. Direct character-link production reachability, cover2 official-content branch, source decision readers and Play/EXE remain open; Q07 paused.
2026-09-24 D-024 `NTSD28-USER-SOURCE-REVIVAL-AVERAGE-001 / FOCUSED_TEST_PASS / PLAY_PENDING`: formal source peer `sumX` now gates one RNG pair and writes independent source/physical precise placement; original Editor focused revival11/11 PASS. Real participant Driver/Play/EXE history and Q07 remain open.
2026-09-24 D-024 `NTSD28-USER-SOURCE-HITFA5-TARGET-VELOCITY-001 / FOCUSED_TEST_PASS / PLAY_PENDING`: OID219 hit_Fa5 source integer quotient and child source birth now give formal moved-history Vx3/4 instead of physical-gap4/5; original Editor staged-content8/8 PASS. Full Driver/Play/EXE and Q07 remain open.
2026-09-24 D-024 `NTSD28-USER-SOURCE-FUSION-DISTANCE-001 / FOCUSED_TEST_PASS / PLAY_PENDING`: formal row0 source X gap40 versus approved physical gap50 now merges; source Z gap7 versus physical8 also merges, source/physical midpoints and split copy tested. Original Editor X2/2, adjacent records4/4, Z1/1 PASS. Full Driver/Play/EXE and Q07 remain open.
2026-09-24 D-024 current next gate: source-rule readers are now partially active for ordinary revival average, indexed OID219 hit_Fa5 and OID51/52 fusion, each with original-Editor focused evidence only. Do not interpret older `CARRIER_NOT_ACTIVE` notes as current global state. Next prove real participant full Driver/Play for these paths, then remaining teleport ranking/catch-facing/other position writers and formal EXE comparison; noncharacter X-edge death awaits the existing user preference. Q07 remains paused.
2026-09-24 D-024 broad self-check: existing original-Editor `Temp/NTSD_BattleRuntimeSelfCheck.request` completed, fresh `.result` local12:11:00=`PASS`; Editor was idle/non-Play on Battle Scene afterward. This supports cross-pass self-check only, not full Driver/Scene Play/formal EXE parity. Scene SHA unchanged; Q07 paused.
2026-09-24 D-024 revival scoped Play: original Battle Scene Editor consumed the existing Play request and wrote fresh `Temp/NTSD28-B4-RevivalNormalFloorRng-Play-v1.result` local12:13:41 `state=Passed/logicalCases=10`, including new source-vs-physical RNG gate cases. Editor returned to non-Play/idle and Scene SHA unchanged. Controlled temporary World only; natural participant Driver and formal EXE remain open, so Q07 paused.
> 2026-09-24 active D-024 package `NTSD28-USER-SOURCE-TELEPORT-TARGET-001 / IN_PROGRESS`: formal teleport target rank uses source integer X/Z; Unity physical rank may choose another target after view scaling. Task/Change registered before script edits. Run divergent C05 RED/GREEN in original Editor; Q07 paused; DAT/Scene/nonbattle protected.
> 2026-09-24 D-024 teleport target ranking `NTSD28-USER-SOURCE-TELEPORT-TARGET-001 / FOCUSED_TEST_PASS`: formal source integer X/Z target selection now owns state400/401 when all eligible source histories are initialized, physical ranking fallback otherwise. Original Editor RED both divergent cases; GREEN C05 15/15 job551d9463. Full Driver/natural Play/EXE pending. Q07 paused; no DAT/Scene/nonbattle changes.
> 2026-09-24 D-024 active grab facing package `NTSD28-USER-SOURCE-GRAB-FACING-001 / IN_PROGRESS`: source and physical X order can differ after approved full-view physical travel, reversing kind1/kind3 action/facing selection. Task/Change registered; original Editor divergent RED/GREEN pending. Q07 paused.
> 2026-09-24 D-024 grab facing `NTSD28-USER-SOURCE-GRAB-FACING-001 / FOCUSED_TEST_PASS`: source-complete kind1/kind3 paired relation now takes facing from source integer X, physical fallback if incomplete. Original Editor RED both cases, GREEN B6 25/25 job2ac7226b. Full Driver/natural Play/EXE pending. Q07 paused.
> 2026-09-24 next D-024 P0: AI snapshot `rows.X/Z` from physical runtime ints versus playable native_ai source-position distance/threshold. Static candidate only; inspect live producer/consumer, trace and RED before editing AI. Teleport target C05 15/15 and grab facing B6 25/25 focused passes retain natural Play/EXE gates. Q07 paused.
> 2026-09-24 active D-024 `NTSD28-USER-SOURCE-OBJECT-AI-TARGET-001 / IN_PROGRESS`: playable noncharacter hit_Fa target scan source X/Z versus Unity physical X/Z. Add divergent B0 RED, then source-complete/fallback fix; original Editor GREEN pending. Q07 paused.
> 2026-09-24 D-024 noncharacter AI target `NTSD28-USER-SOURCE-OBJECT-AI-TARGET-001 / FOCUSED_TEST_PASS`: object hit_Fa nearest scan now chooses source integer X/Z when all eligible histories initialized, physical fallback otherwise. Original Editor RED slot1 vs expected slot0; final B0 9/9 GREEN jobc87c4059. Character AI snapshot remains distinct; Q07 paused.
> 2026-09-24 D-024 character AI mapping: GameConfig sets DataOrientedCanonical; source/physical mismatch spans sensing rows, decision owned rows, refresh/fallback and world.X/Z consumer. Next create bounded contract and divergent nearest-target RED in configured profile plus legacy parity; do not patch only one row. No character AI script edit yet; Q07 paused.
> 2026-09-24 active test-only D-024 `NTSD28-USER-SOURCE-CHARACTER-AI-NEAREST-WITNESS-001 / IN_PROGRESS`: create source/physical inverted nearest-target witness in DataOriented and Legacy profiles; no production AI edit. Next original Editor focused RED and position contract. Q07 paused.
> 2026-09-24 D-024 character AI first difference confirmed: `NTSD28-USER-SOURCE-CHARACTER-AI-NEAREST-WITNESS-001 / FOCUSED_TEST_PASS / CONFIRMED_DEFECT`. Original Editor formal-expected RED in both DataOriented/Legacy profiles: slot1 source-nearest versus slot2 physical-selected. Characterization 2/2 PASS is not parity. Production AI unchanged; next source domain contract/Task must span sensing, decision, refresh/fallback, unified snapshot and broadphase. Q07 paused.
> 2026-09-24 D-024 character AI continuation: read `artifacts/diagnostics/NTSD28-USER-ALL-ENTITY-MOTION-RATIO-001/CHARACTER-AI-SOURCE-POSITION-CONTRACT.md` before production edits. Two-profile RED confirmed; current 2/2 characterization is not parity. Source-position projection must cover sensing, decision/unified publisher, refresh/fallback and indexed broadphase coherently, while keeping physical frame-motion/render untouched. Create production Task/Change before scripts; Q07 paused.
> 2026-09-24 active `NTSD28-USER-SOURCE-CHARACTER-AI-DOMAIN-001 / IN_PROGRESS`: production Task/Change created before AI script edits. Both DataOriented/Legacy `CharacterInputAll` selected physical-nearest slot2 in original Editor; formal source nearest is slot1. Implement one latched AI position domain for snapshots/legacy/broadphase and source-active unified full refresh; no physical store/DAT/Scene changes. Q07 paused.
> 2026-09-24 latest D-024 character AI: `NTSD28-USER-SOURCE-CHARACTER-AI-DOMAIN-001 / FOCUSED_TEST_PASS`; production DataOriented/Legacy input, sensing/index and decision rows use one source-rule X/Z domain when all active carriers are complete, or physical X/Z for the entire pass otherwise. Original-project Editor 4/4 actual AI input and nearest, adjacent 13/13 and 128 warm inputs with 0 B allocation PASS. Complete full Driver/natural Play/EXE and all source writer coverage are not established; noncharacter X-edge preference remains pending. Q07 remains paused. See Task/Change.
> 2026-09-24 D-024 非角色 X 边界新静态差异：正式 playable 普通接地物体在精确 X `<100`/`>width` 销毁，OID122/123 合法例外夹取 `[100,width-100]`；Unity 当前分别是 `<0`/`>width`、`[10,width-10]`，battle mode 1 的特殊门控也未覆盖。倍率1就不同，尚无运行首差。下一先做定向 RED，源规则还是画面坐标决定离场的用户选择仍待；不改生产边界。详 D024-CURRENT-GATE.md；Q07 后置。
...
> 2026-09-24 D-025 update: `NTSD28-USER-NONCHAR-WALKABLE-TTL-001 / RUNTIME_PENDING`. Worker-safe map polygon snapshot and 304-elapsed-tick timer are implemented; original Editor focused 8/8, adjacent 29/29+9/9 and fresh standard BattleRuntimeSelfCheck PASS. Actual Battle Scene map capture, worker/F5 and observable removal Play witness remain; Q07 resumes after that gate. Exact paths/evidence in Task and Change Record. DAT/Scene/nonbattle paths untouched by D-025.
> 2026-09-24 Q09/P-04 next bounded probe `NTSD28-Q09-SAME-Z-FORMAL-SPRITE-PIXEL-WITNESS-001 / PLANNED`: formal OID120/121 frame0 sprites, original Battle Scene production World and natural central publication, controlled original-camera A/B/overlap pixel comparison. Protect user-confirmed enabled HUDCamera/ScenesCamera; before-script Task/Change/Ledger/STATE recorded. No script/Play result yet; Legacy and formal EXE A/B still separate exits. See `docs/ai/TASKS/NTSD28-Q09-SAME-Z-FORMAL-SPRITE-PIXEL-WITNESS-001.md`.

> 2026-09-24 `NTSD28-BATTLE-UI-SKELETON-001 / IN_PROGRESS`：当前用户任务转入 `NTSD_Battle` UI 骨架阶段。变更边界为 `Assets/NTSD/Scripts/UI/Battle/` 下的三个 presentation-only 脚本；不接触现有场景/Prefab/输入资产/战斗逻辑，BattleBootstrap 的 Canvas 代码仍视为临时桥接。仓库当前没有 TEngine UPM/source 闭包，先保留 TEngine UIModule 接入面，等待用户创建 `BattleMainUI` Prefab 和节点绑定后再实现窗口资源加载与 Runtime snapshot 接线。详见 `docs/ai/TASKS/NTSD28-BATTLE-UI-SKELETON-001.md` 与 Change Record。
> 2026-09-24 `NTSD28-BATTLE-UI-SKELETON-001 / CODE_WRITTEN`：`BattleUiContracts`、`BattleCharacterSlotView`、`BattleMainUIView` 和 `Battle/` 文件夹 meta 已写入；静态 stub compile、台账校验通过。用户下一步在 Unity 创建 `BattleMainUI` Prefab 并绑定实际 Image/TMP/GameObject；之后再实现 TEngine `UIWindow`/`UIModule` 资源加载和 Runtime snapshot source。Unity compile、Prefab import、UIModule 和真实 Battle Scene Play 尚未验证，且未修改 Scene、Input Actions、DAT、BattleBootstrap。
> 2026-09-24 `NTSD28-BATTLE-UI-SKELETON-001 / SUPERSEDED`：用户确认当前 HUD 没有八个角色 Slot，旧 `BattleMainUIView` 聚合视图不匹配当前场景。
> 2026-09-24 `NTSD28-BATTLE-UI-HUD-REWORK-001 / IN_PROGRESS`：当前目标改为 `HUD/HUDBg` 单角色资源条、`ComboPanel` 连击区、`BattleControls` 动作区。保留用户刚整理的场景布局，移除旧 UI 组件挂载；新 view 脚本写入后由用户绑定节点，再接 TEngine UIModule。
> 2026-09-24 `NTSD28-BATTLE-UI-HUD-REWORK-001 / CODE_WRITTEN`：`BattleHudView`、`BattleComboView`、`BattleControlsView` 和单 HUD snapshot 已写入，旧 `BattleMainUIView`/Slot 脚本不再 active，`HUD` 上的旧组件挂载已移除。用户下一步在 Inspector 绑定 `HeadImg/HP/HpXu/MP`、Combo 节点和三个动作 Image；之后再接 TEngine UIWindow/UIModule。Unity compile、实际 Runtime source 和 Play 尚未验收。
> 2026-09-24 `NTSD28-BATTLE-UI-SCENE-HUD-COMBO-001 / IN_PROGRESS`：用户确认 `NTSD_Battle` Canvas 是场景常驻且默认显示，不转 prefab。`HUD`/`ComboPanel` 保持场景 `MonoBehaviour` view；本包处理 HUD/Combo 的状态与现有 ComboBg/ComboTemp/ComboArraw 引用，暂不猜测 `BattleControlsView` 的输入逻辑，等待用户下一步说明。当前不接 TEngine `UIModule` 到这组常驻 Canvas。
> 2026-09-24 `NTSD28-Q09-SAME-Z-FORMAL-SPRITE-PIXEL-WITNESS-001 / CODE_WRITTEN`：新 Editor-only OID120/121 原 Scene 四阶段 GPU 遮挡探针已写，利用诊断表现 tick 避免暂停下同 tick plan 缓存；原 Battle Scene Play、编译、像素和清理尚未验。只改新测试脚本及治理记录；不动相机 enabled、DAT、生产或场景。下一步原 Editor 导入、聚焦 Play、Scene SHA 前后及 Change Ledger。
> 2026-09-24 Q09/P-04：`NTSD28-Q09-SAME-Z-FORMAL-SPRITE-PIXEL-WITNESS-001 / VERIFIED_CONTROLLED_CENTRAL_GPU_ONLY`。原Battle Scene生产正式OID120/121的同Z body命令大slot先绘；实际受控原世界相机GPU四图在body交集11/11可区分像素均取后绘slot51颜色，0反例。原Editor编译0错、Play退出、对象/slot/两池/暂停/Scene SHA与diff恢复。旧全屏匹配历史PASS无效，已保留修正记录。下一出口为自然完整tick/Legacy/正式EXE同条件A/B及Q09其他P项；不因本包关闭P-04/Q09。详本包ACCEPTANCE/Change。
> 2026-09-24 Q09/P-04 `NTSD28-Q09-SAME-Z-NATURAL-TICK-PIXEL-WITNESS-001 / RUNTIME_PENDING`: original Editor compile0; real tick5→6 formal OID120/121 frame1/pic1 equal-Z central body command order observed. The corrected four-separate-Play method obtained only `baseline` PASS_CAPTURE under Scene SHA9C7EDD...BA6CFD7D (RNG/slot/cleanup/Scene hash recorded); before `a`, parallel UI work saved Scene to 711FE77B...E5D605457. Four-run same-Scene pixel comparison stopped; controlled 11/11 remains independent. Both Cameras remain enabled. Resume four variants only after Scene stabilizes, preserving UI edits. Legacy/EXE A/B open. See PROGRESS/Record.
...
> 2026-09-24 D-024 AI Play 最新：test-only `d024-ai-child-play-11/-12` 在原Editor显式mode0后 AI tick75 选中人类；600完整Driver tick 的 Lee 帧61/486 OPoint 指向正式 OID814 的 `pic999/wait0` 纯音效，故没有持久 owner1 子体是夹具预期错误，不能报生产生成缺陷或子体验收。三个 OID120/owner-1 为其他随机武器。源carrier、ordered shutdown、pool零借用、Scene磁盘SHA都通过。下一步换正式可达的非音效 AI OPoint 见证；D-024未闭，Q07总门槛仍后置。详 Task/Record/request12。
> 2026-09-24 Q07 Sasuke旧图引用回访：旧owner清单的Scene BMP GUID引用1已是前快照；当前Battle Scene禁用预览指向正式`sasu.png` GUID及y881。原项目Editor干净Battle Scene现场菜单预览PASS、图像已查看，正式图/角色/阴影/脚标/血条可见，原Editor预览类11/11、Console0、双Scene SHA保持。`NTSD28-Q07-SASUKE-EDITOR-PREVIEW-FORMAL-IMAGE-001 / VERIFIED`仅关闭作者预览子包；旧BMP保留、动态引用/自然技能、Q07整组未闭。证据见同包PROGRESS与归档JSON/PNG/test job。旧文“原Editor Reload待”为历史快照。
> 2026-09-24 Q07旧图片调用链新审计：正式根配置下Menu/Battle预热走Logan候选；原Editor正式type-0 DAT/图片部署测试job `ed1e857c29b84159bea5bbbe3c583f5c` 1/1 PASS。旧索引图片383 GUID在NTSD指定序列化资产扩展0命中；但空根legacy内容分支仍会读取旧`data.txt`、DAT和图片，Editor刷新也有该入口。旧报告把当前部署测试写成旧资源消费者已过期；仍不得删除旧文件。详Q07 `CURRENT-DYNAMIC-REACHABILITY-20260924.md`、summary/job JSON及GUID扫描；Q07和D-024未闭。
> 2026-09-26 Q09/P-08 `NTSD28-Q09-BPOINT-BLEED-SCENE-PIXEL-001 / IN_PROGRESS`: new opt-in original Battle Scene high/low-HP formal Ita central camera witness is declared before script edit. No Play/pixel result yet; use original Editor only, verify camera/Scene restoration and mark ownership. Legacy and formal EXE same-state comparison remain open.
> 2026-09-26 Q09/P-08 `NTSD28-Q09-BPOINT-BLEED-SCENE-PIXEL-001 / VERIFIED_CONTROLLED_CENTRAL_GPU_ONLY`: original Battle Scene formal Ita frame0 high/low HP final camera A/B found 3 independently verified new red pixels inside the mark ROI. Original Editor compile0, exited Play, camera/World/slots/pool and four protected SHA stable. Two initial probe false negatives retained. Next P-08 natural/Legacy/formal EXE same-state; P-08/Q09/full goal still open. See `artifacts/diagnostics/NTSD28-Q09-BPOINT-BLEED-SCENE-PIXEL-001/ACCEPTANCE.md`.
> 2026-09-26 Q09/P-08 Legacy read-only owner audit: `LF2ObjectRenderer` is the live body SpriteRenderer path but has no bleed materializer; `BattleEntityOverlayRenderer.RenderAll` has only test/self-check callers, while production `SimulationTickDriver.LateUpdate` calls `SparkRenderer.RenderAll`. Do not patch the unused overlay class alone and claim Legacy parity. A separately governed production owner with pool/release/shutdown and Legacy Scene pixel evidence remains; see `artifacts/diagnostics/NTSD28-Q09-BPOINT-BLEED-SCENE-PIXEL-001/LEGACY-OWNER-AUDIT.md`. Central controlled pixel pass stands, P-08/Q09/full goal remain open.
> 2026-09-26 Q07减伤资源修复 `NTSD28-Q07-REDUCED-HIT-RESOURCE-TRANSACTION-001 / FOCUSED_TEST_PASS / SCOPED_MP_PARITY_POSITION_PENDING`：原Editor RED→GREEN聚焦2/2、编译0错；正式根EXE/Unity同初态X550 1428映射字段剩12位置差、双方MP已同500/404，X1200 2100/2100全同。前后Unity raw只变X550双方tick16–26当前MP，domain与input/RNG均不变。tick20目标水平位置差单独追，Q07/D-024/总目标开放。Scene/配置四保护SHA稳定，恢复v3三文档NUL-free；下方PLANNED是实施前记录。详同ID资源修复验收。
> 2026-09-26 Q07位移首差后继 `NTSD28-Q07-REDUCED-HIT-HALF-DVX-PRECISION-001 / PLANNED`：正式命中解析器用浮点`dvx/2.0`，Unity通用减伤地面writer用`int dvx/2`；X550在tick19目标Vx8.4/8、tick20 X558.4/558。Task/Change/Ledger先行，下一先RED再仅修共用公式并复跑X550/X1200。D-024比例例外、DAT、Scene、非战斗保持；下方资源修复已限定通过，Q07/总目标开放。
> 2026-09-26 Q07减伤奇数dvx精度 `NTSD28-Q07-REDUCED-HIT-HALF-DVX-PRECISION-001 / FOCUSED_TEST_PASS / SCOPED_RAW_PARITY`：原Editor有效RED3.6/3.1→GREEN2/2、编译0错；正式根EXE/Unity X550 1428/1428、X1200 2100/2100映射字段相同。X550目标tick19 Vx8.4、tick20精确X558.4两边同值；前后仅目标速度/位置变，显式v2 domain与input/RNG不变。首轮漏v2导出保留且未用于同schema比较。D-024场景比例、自然Play和Q07总出口开放；详同ID运动验收。下方PLANNED为历史。
> 2026-09-26 Q07 ShadowCompare后继 `NTSD28-Q07-HIT-PROJECTION-RESOURCE-MOTION-SYNC-001 / PLANNED`：现有减伤观察原Editor 2/2 FAIL，mask0x20000000000000为目标PP/相关字段组；生产命中X550/X1200 raw已同正式，不应据观察误报 gameplay。投影另有奇数dvx整数半量。Task/Change/Ledger先立，下一加奇数RED并修标准/减伤投影；DAT/Scene/非战斗不动。
