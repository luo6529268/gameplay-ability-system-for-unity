...
- **当前 formal 输入窄门槛（2026-08-25，部分 Server-only 实现）：** Server [`PENDING-S1-FRAME-INPUT-EDGE-OWNERSHIP-001.md`](../../../../NTSD_Server/docs/ai/DECISIONS/PENDING-S1-FRAME-INPUT-EDGE-OWNERSHIP-001.md) 已将上位设计的`held/pressed/released`与C++ release由prior/current held派生edge的事实收敛。`S1-SERVER-HUMAN-FRAME-INPUT-VALUE-001`现已完成immutable seven-action triple、pure edge derivation/order与equality/hash；仍待确认/实现的是Client capture、wire malformed-payload disposition、full formal `FrameInputSet`/world integration。它只阻塞后者，**不暂停**Server-first目标；不改Client，也不选择missing-input或wire/ABI行为。
...
- **当前执行门禁（2026-08-25）：** 最近的Server policy包已在其Server-only范围关闭；当前无活跃源码包。后续实质源码路径仍需未批准的formal input capture/wire、missing-input产品规则、retention/recovery、formal Kernel/S5 Host合同或明确Client授权；S6+ transport/public/control-plane也未获授权。此状态不是完成或Server test失败；不要用placeholder Server/Client代码绕过。恢复时先读取Server `CURRENT-HANDOFF.md`与S0-S9 matrix，并在获得所需决定后新建独立Task/Change Record。
