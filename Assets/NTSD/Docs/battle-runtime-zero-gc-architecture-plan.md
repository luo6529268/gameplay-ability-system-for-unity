...
本计划的首要目标是在资源加载、对象池预热和战斗世界初始化完成后，使正式战斗窗口不再产生托管堆垃圾，也不在战斗过程中触发托管垃圾回收。优化只能改变数据所有权、容器复用和执行方式，不能改变权威 C++ release 的 pass 顺序、slot 顺序、RNG 消费、输入边沿、碰撞候选、命中、opoint 或生命周期结果。
