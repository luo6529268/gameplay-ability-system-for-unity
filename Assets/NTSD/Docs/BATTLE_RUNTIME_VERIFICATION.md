...
- PreFrame X 逻辑边界已完成并通过 fresh Unity 运行时自检：entity pass 分离 `BaseStageWidthPx` 与 `XMaxOverride`，按 current DAT type/OID 中央分派，并覆盖 `RelationTeam`/`HitStop`/`Unk344`/`YInt`/strict edges/`XInt`/free lifecycle。
- OPOINT-VIS 已通过 fresh runtime matrix：pre-advance/natural-drop/逐实体 late 三个 flush 边界、real factory queue、父回收、高/low slot 可见性均已覆盖。验收过程修复 pending destroy 被 active-only 采集过滤的问题。
- STEP10 已通过 fresh runtime matrix：duration/mismatch 后 throw/dir、non-character/shared-DAT cpoint、input priority、KillStats/DamageStats 与 `HPLost` 契约均已覆盖。
- TRANSFORM-SHELL / FRAME-ADV / FRAME-TICK 已通过 fresh runtime matrix：过程修复 character/weapon `PS.BindRuntime`、cross-SimOrder unregister、current-DAT landing 分派，并确认 state4000/8000 用目标 DAT `weapon_hp` 刷新 `WeaponFlightCounter`而不改 `WeaponCount`。矩阵覆盖 SpecialAttack 单次 physics/frame tick/type3 drain、PpDisplay、state14、negative next、type1/2/4/6/oid999 落地、state8000 hit-stop 顺序及 transformed pending destroy 只注销一次。release 无 oid9 Amaterasu 专属 drain，仍不新增猜测分支。
- COLLISION-SNAPSHOT 权威审计未发现普通生产路径差异：slot 顺序、Prev2、双向 pair、itr 顺序、20 candidate cap 与 cache 隔离均有现有映射。保留对象引用 cache 对比 C++ slot cache 的未来风险；只有 snapshot 消费期间出现同 slot 即时复用 producer 时才需升级修复。
...
2026-07-15 12:36:59 最新验收：fresh `dotnet build Assembly-CSharp.csproj /v:minimal /m:1` 为 `0 errors / 18 warnings`；UnityMCP 执行最新 `BattleRuntimeSelfCheck` 后，`ntsd_selfcheck_unity.log` 明确包含“战斗运行时自检通过/自检完成”。OPOINT-VIS、STEP10、TRANSFORM-SHELL、FRAME-ADV、FRAME-TICK 的上述矩阵已通过，并回归 T0-T9 及其他既有检查。该 PASS 是针对性断言证据，不是完整对局逐帧等价证明。COLLISION-SNAPSHOT 权威审计未发现生产差异，但未来 pass 内同 slot 即时复用风险仍保留。最终 architect 已基于最新 diff、build 与 Unity runtime 证据完成 fresh 复核，结论 PASS、无 blocker。T8 默认 `stage.dat` 仍由用户明确暂缓，不进入当前 backlog。
