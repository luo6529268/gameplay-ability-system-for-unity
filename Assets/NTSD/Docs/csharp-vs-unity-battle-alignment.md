...
> **2026-08-30 authority correction — BA7-F5:** 本表的“StageSpawnAt应保留复用槽ARest/VRest”来自已经废止的C# authority口径。当前唯一authority的C++ release live path为`spawn_stage_immediate_entry_slot -> spawn_at -> spawn_into_slot -> reset_cooldowns(slot)`，明确清ARest、VRest victim row与attacker column。因此BA7-F5的旧“预期保留”结论已被`NTSD_Server/docs/ai/AUDITS/S0-STAGE-SPAWN-REST-ALIGNMENT-PREREQUISITE-001.md` supersede；当前代码仍未修复、未运行时验收，不能把旧SelfCheck PASS当作C++对齐证据。
...
| BA7-F5 | **历史C# authority结论，已由2026-08-30 C++ authority审计supersede**；旧结论称stage spawn误清复用槽ARest/VRest | 历史C# `SimulationWorld.Registry.SpawnAt`不清；当前C++ release `spawn_at`成功后明确`reset_cooldowns(slot)` | Unity `StageSpawnAt`当前仍保留rest，ordinary registration才清 | 前置：later stage spawn复用非零rest槽。当前C++预期：ARest、victim row、attacker column全清；Unity实际：保留 | `CLIENT-CPP-STAGE-SPAWN-REST-ALIGNMENT-001`待新授权；默认`stage.dat`不需要；**权威差异已确认 / 代码未修复 / 未运行时验证** |
