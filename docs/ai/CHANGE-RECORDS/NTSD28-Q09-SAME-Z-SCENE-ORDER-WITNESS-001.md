...
Camera correction after user clarification: user says both Battle Scene cameras default enabled and attributes the disabled save to this task. Restored only HUDCamera/ScenesCamera serialized m_Enabled to 1; Scene hash 9409F2...3B3A39 and git diff empty. Reloaded original Scene with no unsaved changes. Existing central visibility Play probe captured ScenesCamera enabled/active true, valid central plan/submission, source commands4/draws6 and no diagnostic first difference; post-exit hash unchanged. BattleBootstrap.OnDisable can disable these cameras but separate Play exit did not reproduce disk write, so root cause of earlier save remains unproven; no production script changed.
