<!-- CHANGE-RECORD
id: NTSD28-Q06-SNAPSHOT-MUTABLE-DAT-IDENTITY-RESTORE-001
status: VERIFIED
change-kind: SNAPSHOT_MUTABLE_DAT_IDENTITY_RESTORE
...
code-path: Assets/NTSD/Scripts/Test/Editor/NTSD28Q06LockedKindNativeFrame40AdmissionEditorTests.cs
authority: User-approved battle alignment and retained-shell replay; formal kind transition source2.
evidence: Parent replay job eb4986b07a5d4f4abc247d7f6e345405 FAIL EntityIdentityMismatch; binding audit pending.
-->
...
Measured RED job f602595525964e1aa23c6c2fd707b3a6: executed3, valid type0/type3 restore2 FAIL EntityIdentityMismatch; late-slot StableId rejection PASS. XML focused-red/results.xml. Test-only compilation completed. Exact production decision: preserve EntityKind/StableId/current DAT type checks; changed DAT id additionally requires expected catalog definition/config identity/type and FrameCache. Validate saved frame IDs against expected data, then Load expected wrapper after raw/entity runtime copy and before base-shell restoration. Existing same-ID/no-catalog compatibility retained. Cross-DAT-type restore remains rejected and is an explicit unclosed follow-up, not covered by this same-type repair. No ObjectId setter needed because TryCopyEntityRuntime restores Runtime.ObjectId first; identity callback repopulates metadata for restored wrapper.

Implementation written: only BattleStateSnapshotRestore.cs ValidateBattleStateSnapshotRestore changed-DAT gate, new CanRestoreLocalDataDefinition, HasSnapshotFrameData expected-definition selection, and pre-base-shell FrameCache.Load. Parent fixture unchanged. Editor domain reload complete; focused snapshot class plus original parent replay job cfffb63e14c74ea0b4f5eb2d92715aa5 running. No SelfCheck/Play rerun yet.
...
Measured payload RED94bd29dfed0e411aafc5834eb6d83c24:8 cases,6PASS/2FAIL; corrupted entity/runtime type cases incorrectly accepted. Config id/missing data/expected999 absence rejected correctly. Original XML payload-red. Implement declared readonly consistency helper now, no allocation/schema/public API changes.

Actual production addition now written: readonly HasConsistentDataIdentity checks existing entity/raw type fields, changed-DAT preflight returns EntityPayloadMismatch on mismatch. No schema change or allocations. Joint snapshot restore/native descriptor/local+transfer and original parent replay job84364ef670d3421baa4ca9cc3df6c5af running after Editor reload.

Correction after joint84364ef670d3421baa4ca9cc3df6c5af: last observed progress36 had4 failures including valid type3 and original parent replay, all EntityPayloadMismatch. Final XML unavailable after external Editor termination; old62860 absent, MCP6403 refuses, Temp result missing. Do not report a final count for that interrupted observation.

Root source audit proves prior raw-type equality requirement WRONG: RuntimeSlotTable.Page independently allocates RawRuntimes and Entities; snapshot validation explicitly rejects ReferenceEquals(view.Entity.Runtime, view.RawRuntime). DamageWriter resolves entity.Runtime first and falls back to raw only absent entity. FrameCache callback only writes entity.Runtime/IdentityWriter, not rawRuntime. Thus require expected type consistency only for ENTITY payload subject to that callback; preserve raw payload exactly as captured without imposing entity type. Existing CaptureCopiesEveryCanonicalRuntimeFieldAndKeepsRawSlotDistinct supports this ownership contract. Correct helper name to HasConsistentEntityDataType, preserve raw-domain independent payload. Replace erroneous raw-corruption rejection case with pre-capture distinct raw-type preservation control; keep original failed evidence and explain correction, not weaken entity rejection. Current type3 failures are introduced by overly strict new check, not preexisting battle behavior.
...
Existing project Editor33236 recovered; no new Editor launched. Added shared replay runner/Play run-restore only as predeclared. Refreshed and observed idle/domain reload complete. Joint b834ddc108714f14ae50ab78d3b76f48 running snapshot class + parent original replay + native frame restore + Q05 independent raw restore. Latest source remains entity-only type guard.

Latest corrected production joint42/42 PASS jobb834ddc108714f14ae50ab78d3b76f48, SelfCheck2026-09-21T04:31:47.9738179Z PASS, renderer run-restore2/2 and Q05close PASS archived play-pass. Raw Play JSON inherited old descriptive scope text wrongly mentioning Mobile; restoreOnly=true/passedCases2/source branching show actual Authority/DataOriented renderer replay only. Corrected only descriptive scope conditional after run; no assertion or production change, no repeated SelfCheck/Play required. Archive remains original with this correction. Editor returned idle/notPlaying; Scene hash unchanged.
