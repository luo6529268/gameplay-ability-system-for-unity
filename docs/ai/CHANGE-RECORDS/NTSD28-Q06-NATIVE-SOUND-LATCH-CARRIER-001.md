<!-- CHANGE-RECORD
id: NTSD28-Q06-NATIVE-SOUND-LATCH-CARRIER-001
status: VERIFIED
change-kind: NATIVE_SOUND_LATCH_CARRIER
...
code-path: Assets/NTSD/Scripts/Test/Editor/NTSD28Q06NativeSoundLatchEditorTests.cs
...
authority: Formal BattleWorld28 append_native_frame_sounds and EntityState28 sound_action_latch lifecycle; source witness2676 confirms independent latch.
evidence: Pre-change read-only source writers/defaults and absence in Unity Runtime; exact copy/checksum/version migration.
-->
...
事前补充第22路径：Q05TraceIdentityEditorTests仅同步三项schemas头断言；直接JSON schemas读取未被首轮SchemaVersion名称检索捕获。生产修改尚未开始。

8项RED均失败（6缺字段/2前一版尚被接受），red-8.xml留证。准确22脚本已CODE_WRITTEN，NativeSoundActionLatch新建/reset -1和copy、checksum/ECS/full parity、14/22/25/2/2及两个工具头已同步；等待编译和回归。尚未接声音producer。

版本夹具补充：Q05 checksum旧历史测试原文件LF，首轮CRLF签名替换未命中，已显式改成TestCase23/24+oldVersion参数；在运行测试前纠正。dotnet工具51/51通过。

初轮32项31PASS/1FAIL：新Mobile测试调用仅支持Authority400的full parity API，非生产恢复错误。已保留两profile完整checksum/claimed+raw恢复与fingerprint断言，将full parity仅在既有支持的Authority400执行，并新增raw-only checksum独立变化断言；不扩大API权限或删除Mobile覆盖。

联合回归344/344 PASS，job ba3c4d3bc21d4983b07c71044775c3b1，362.246秒，含新8和前版checksum24拒绝。工具88/88；新fresh native/Unity 14/22/25/2/2与正式内容identity一致，raw仍44既有字段相等/6MISSING，本字段未纳入raw50。当前FOCUSED_TEST_PASS，SelfCheck已请求，真实Play待。
