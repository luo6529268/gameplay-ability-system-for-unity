...
Measured plan refinement before production patch: choose approved batch SetSubMeshes candidate; local value-only descriptor staging buffer, upload current finite vertices first, batch only count/range changes; stable active-prefix/inert-tail work preserved. See Record for RED counts and buffer teardown ownership.
