...
状态 FOCUSED_TEST_PASS / SCOPED_PLAY_PASS / JOINT_SCHEMA_PENDING。父五reserved步骤2继续，GrabbedBy/TrackerFlag、Mass/Oscillate、2F8/raw已限定验证，不重做。当前参考上一两flag artifact release-tick-next-references.txt，再以当前源逐项确认。
...
限定出口：267/267、完整SelfCheck（旧JSON断言修正后复跑）与四当前数据Play前后有效字段一致，对象4→4；记录/artifact同ID。下一唯一 `NTSD28-Q05-WEAPONSTATE-CARRIER-RETIREMENT-001`，其余联合阶段保持。
