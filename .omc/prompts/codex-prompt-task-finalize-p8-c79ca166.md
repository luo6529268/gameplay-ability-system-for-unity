...
files:
  ...
  - "I:\\GitHub\\Unity_GAS\\gameplay-ability-system-for-unity\\Assets\\NTSD\\Docs\\csharp-vs-unity-battle-alignment.md"
  - "I:\\GitHub\\Unity_GAS\\gameplay-ability-system-for-unity\\Assets\\NTSD\\Docs\\HANDOFF-codex-battle-alignment.md"
timestamp: "2026-07-23T10:24:25.872Z"
---
...
- **当前验证边界：**本次文档更新开始时，完整 `BattleRuntimeSelfCheck` 的最新已知结果为 `2026-07-23 17:43:38 PASS`。此后 benchmark -> self-check 顺序回归正在由执行者重跑，因此这里不虚构该重跑的最终结果，也不宣称新的 Architect PASS。P8-E Android/Adreno/Mali 仍由用户负责且不在本轮范围内；T8 默认 `stage.dat` 部署继续排除。
...
**状态（2026-07-23 当前结论）：P8-A/B1/B2 已完成；P8-C 正确性/像素验收矩阵已完成并有 EditMode 与真实 Play pool 证据；P8-D 的 Editor 表现工作负载 harness 与四档结构化报告已完成；P8-E Android/Adreno/Mali 真机验证继续由用户负责并排除。** P8-D 不等于真实 `SimulationWorld` active-entity 容量或 logic tick 性能结论：该矩阵只覆盖冻结的确定性 presentation snapshot/command 工作负载。诊断只读取 immutable presentation 数据和中央后端结果，绝不回写战斗 runtime。编译通过、self-check 通过、Play Mode 定向验证、像素对照和性能测量是不同证据，不能互相替代；Editor 结果也不能推出 Android、Adreno、Mali 或 Standalone Player 实测结论。
...
**2026-07-23 P8-D 当前证据（覆盖本节此前“待实施/待新证据”的历史快照）：**`Temp/P8-D-presentation-100-ab-rerun.json`、`Temp/P8-D-presentation-300-ab.json`、`Temp/P8-D-presentation-500-ab.json` 与 `Temp/P8-D-presentation-1000-ab.json` 均为 **PASS**。每档均执行 CentralOnly/benchmark-only Legacy compatibility 的 A/B 表现工作负载，严格校验实际 `100/300/500/1000` presentation entities、`200/600/1000/2000` commands、render workload ownership、`256x256` render target，以及 600-frame soak 后资源数、benchmark-owned memory、full-GC retained managed heap 和 graphics memory 的增长阈值。P2 已关闭 EditMode 将 mesh segment 冒充 `Graphics.DrawMesh` submission 的回归：`presenterSubmissionDrawCalls` 现在明确为 `unavailable`，Play 只在实际调用提交后才计数。其余 `mainThreadTimeMs`、`renderThreadTimeMs`、`gpuFrameTimeMs`、`drawCalls` 等不可用指标也显式报告 `unavailable`，绝不伪报零；冻结表现工作负载不执行 `SimulationWorld` logic tick，也不注册真实战斗实体。故 P8-D 完成的是 benchmark 工具与该受控 presentation 矩阵，不宣称全面性能收益、真实 active-entity 容量、生产 atlas 性能或本轮 Standalone Player 实测。

**额外 current-scene production 覆盖：**`Temp/P8-D-current-scene-ab-v2.json` 为 **PASS**。真实 `NTSD_Battle` Play 在退出前的 `SimulationWorld` 为 `ObjectCount=12`、`tick=3847`；冻结并发布的表现帧为 `EntityCount=6`、`CommandCount=12`。CentralOnly 与 benchmark-only Legacy compatibility 均实际请求/物化 `6` 个 presentation entities、`12` commands，使用同一 fingerprint `f3aaf429518f46ec` 与同一 `256x256` target；600-frame retained 检查中 Central/Legacy managed heap 分别为 `+28672 B`/`+49152 B`，graphics `+0`、owned bytes `+0`、resource count 不变。presentation build 与 GPU 数据只记录为这一次 Windows Editor 当前场景样本，`mainThreadTimeMs`、`renderThreadTimeMs` 和 frame draw count 仍为 `unavailable`，不据此推断全面性能收益。该生产覆盖是对 P8-D 的额外证据，不是新的 P8 独立 gate，也不改变“不测 logic tick/真实 active capacity/Standalone Player”的边界；本次运行后已退出 Play。
...
| P8-B1 | reason enum、allocation-safe records、runtime-handle/slot 查询、focused self-check | **已完成；fresh full self-check PASS，最终架构复核已纳入本轮验收** |
| P8-B2 | Editor 可视查询/导出与按实体定位工具 | **已完成；EditMode、真实 Play slot 查询与确定性 JSON 导出通过** |
| P8-C | 自动正确性/稳定性矩阵，以及 Play/像素证据入口 | **已完成矩阵范围；EditMode 像素报告 PASS、真实 Play pool 扩容 PASS、11/11 EditMode 通过与 fresh full self-check PASS。非本矩阵的生产资源/设备覆盖不作推断** |
| P8-D | Editor/Player benchmark harness 与结构化报告 | **Editor presentation synthetic workload 已完成四档 A/B 结构化报告；不等于 SimulationWorld 容量/logic tick、全面性能收益或本轮 Standalone Player 实测** |
...
--- File: I:\GitHub\Unity_GAS\gameplay-ability-system-for-unity\Assets\NTSD\Docs\csharp-vs-unity-battle-alignment.md ---
...
本次文档更新开始时的最新完整 self-check 是 `2026-07-23 17:43:38 PASS`。benchmark -> self-check 顺序回归正在重跑；在结果产生并复核前，本节不声称该重跑已 PASS，也不虚构新的 Architect PASS。
...
--- File: I:\GitHub\Unity_GAS\gameplay-ability-system-for-unity\Assets\NTSD\Docs\HANDOFF-codex-battle-alignment.md ---
...
- **未扩大范围：**P8-E Android/Adreno/Mali 继续由用户负责并排除；T8 默认 `stage.dat` 部署继续排除。文档更新开始时的 full self-check 是 `2026-07-23 17:43:38 PASS`；benchmark -> self-check 顺序回归仍在重跑，因此不把未完成重跑或未提供的 Architect 复核写成 PASS。
